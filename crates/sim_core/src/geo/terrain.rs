...
/// v1.50.40：4 -> 5（S7-02 新增 `grassland_plain_v1` 低幅高程场/残丘/泉溪洼地分支；
///           旧 T1/T2 路径逐位不变，按「生成算法面变更必须递增」规则递增）
    ...
    pub fn generate_with_profile(&mut self, seed: u64, profile: &str, config: &SimConfig) {
        ...
        for gy in 0..self.grid_height {
            for gx in 0..self.grid_width {
                ...
                let wave_large = ((wx * 0.006 + p1_x).sin() * (wy * 0.006 + p1_y).cos()) * 5.0 * wave_scale;
                let wave_medium = ((wx * 0.014 + p2_x).cos() + (wy * 0.014 + p2_y).sin()) * 2.5 * wave_scale;
