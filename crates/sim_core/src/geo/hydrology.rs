...
impl RiverCenterline {
    ...
    fn visit_nearest_segment(&self, node_index: usize, p: Vec3, best: &mut (f32, f32, f32, usize)) {
    ...
    pub fn lateral(&self, p: Vec3) -> f32 {
        self.distance(p).2
    }
    ...
    pub fn s_at_y(&self, y: f32) -> f32 {
        ...
        for i in 0..self.points.len().saturating_sub(1) {
            ...
            let t = if dy.abs() < f32::EPSILON {
                ...
                ((y - a.y) / dy).clamp(0.0, 1.0)
            };
            let err = (a.y + dy * t - y).abs();
            if err < best.0 {
                best = (err, self.cumulative[i] + a.distance_to(&b) * t);
            }
        }
        best.1
    }
    ...
    pub fn meander_windows(&self, half_width: f32) -> Vec<MeanderWindow> {
        ...
        for i in 0..self.points.len().saturating_sub(20) {
            ...
            if chord / arc <= 0.71
                && neck >= 0.5 * half_width
                && neck <= 3.0 * half_width
                && swing >= 4.0 * half_width
            {
                out.push(MeanderWindow {
                    s0: self.cumulative[i],
                    s1: self.cumulative[j],
                    arc_length: arc,
                    chord_length: chord,
                    chord_over_arc: chord / arc,
                    neck_width: neck,
...
impl RiverGeometry {
    ...
    #[inline]
    pub fn distance(&self, p: Vec3) -> (f32, f32, f32) {
        self.centerline.distance(p)
    }

    #[inline]
    pub fn point_at_y(&self, y: f32) -> Vec3 {
        self.centerline.sample(self.centerline.s_at_y(y))
    }
}
...
pub(super) fn plan_river_geometry(seed: u64, cfg: &SimConfig, size: f32) -> RiverGeometry {
    ...
    for i in 0..=256 {
        ...
        let x = size * 0.155 * (t * std::f32::consts::TAU * 3.0 + phase * 0.35).sin() * taper;
...
impl TerrainMap {
    ...
    fn generate_river(&mut self, geom: &RiverGeometry, cfg: &SimConfig) {
        ...
        for gy in 0..self.grid_height {
            ...
            let gx_lo = ((((center(row_y) - span) / size + 0.5)
                * (self.grid_width - 1).max(1) as f32)
                .floor() as isize
                - 3)
            .max(0) as usize;
            let gx_hi = ((((center(row_y) + span) / size + 0.5)
                * (self.grid_width - 1).max(1) as f32)
                .ceil() as isize
                + 3)
            .min((self.grid_width - 1) as isize)
            .max(gx_lo as isize) as usize;
        ...
        for (i, y) in [-size * 0.24, size * 0.24].into_iter().enumerate() {
            ...
            let mut tx = ahead.x - cp.x;
            let mut ty = ahead.y - cp.y;
            ...
            let a = land_endpoint(-1.0).unwrap_or_else(|| {
                Vec3::new(
                    fallback_a.x,
                    fallback_a.y,
                    self.sample_elevation(fallback_a.x, fallback_a.y),
                )
            });
            let b = land_endpoint(1.0).unwrap_or_else(|| {
                Vec3::new(
                    fallback_b.x,
                    fallback_b.y,
                    self.sample_elevation(fallback_b.x, fallback_b.y),
                )
            });
            let crossing = TerrainConnection {
                id: i as u32 + 1,
                start: a,
                end: b,
                width: cfg.terrain_crossing_width.max(12.0),
                node_a: None,
                node_b: None,
            };
            for gy in 0..self.grid_height {
                for gx in 0..self.grid_width {
                    let p = self.grid_pos(gx, gy);
                    if ((p.x - cp.x) * tx + (p.y - cp.y) * ty).abs() <= crossing.width * 0.5
                        && self.cells[gy * self.grid_width + gx]
                            .water_body_id
                            .is_some()
                    {
                        let c = &mut self.cells[gy * self.grid_width + gx];
                        c.surface_kind = SurfaceKind::ShallowWater;
                        c.elevation = level - 0.25;
            ...
            self.features.push(TerrainFeature {
                id: 10 + i as u32,
                kind: TerrainFeatureKind::ShallowFord,
                vertices: vec![a, b],
                elevation: level,
        ...
        self.hydrology.water_bodies.push(WaterBody {
            id: 1,
            level,
            flow_direction: Vec3::new(0.0, -1.0, 0.0),
            resource_pool_id: 1,
            vertices: outline.clone(),
        });
        self.features.push(TerrainFeature {
            id: 1,
        ...
        for i in 0..n {
            ...
            self.hydrology.access_points.push(WaterAccessPoint {
                id: i as u32 + 1,
                water_body_id: 1,
                resource_pool_id: 1,
                pos: p,
...
pub(super) fn apply_river_cliff(
    ...
    for i in 0..=n_slots {
        ...
        for j in 0..=16u32 {
            ...
            if ford_ys
                .iter()
                .any(|fy| (y - fy).abs() < ford_half + CLIFF_FORD_MARGIN_M)
            {
                ok = false;
                break;
            }
        ...
        let next = geom
            .centerline
            .sample((cs + 1.0).min(geom.centerline.total));
        ...
        for j in 0..=16u32 {
            ...
            min_clear =
                min_clear.min(signed - (geom.half_width(py, size) + geom.bank + geom.terrace));
        }
    ...
    let gx_hi = (((bmax_x / size + 0.5) * (gw - 1).max(1) as f32).ceil() as isize + 2)
        .clamp(0, (gw - 1) as isize) as usize;
    let gy_lo =
        (((bmin_y / size + 0.5) * (candidate.grid_height - 1).max(1) as f32).floor() as isize - 2)
            .clamp(0, (candidate.grid_height - 1) as isize) as usize;
    let gy_hi =
        (((bmax_y / size + 0.5) * (candidate.grid_height - 1).max(1) as f32).ceil() as isize + 2)
            .clamp(0, (candidate.grid_height - 1) as isize) as usize;
    ...
    for &idx in &overlay_idx {
        scratch
            .surface_overlays
            .push(super::geometry_transaction::SurfaceOverlay {
                cell_index: idx,
