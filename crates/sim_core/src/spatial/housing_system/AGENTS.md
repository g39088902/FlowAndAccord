...
`world.tick()` 中"房屋系统"环节调用本管线，顺序固定：冬季供暖 → 折旧坍塌 → 丧偶解婚 → 修缮结算 → 施工计时与竣工 → 成婚/改嫁 → 空置房登记（户主亡故→无主→营地列表）→ 金币遗产继承（`world.rs`）→ 营地行政区升级。
...
### 4.4 空置房登记规则（v1.10.0 取代父系继承）

户主故去 → 房屋 `owner_id`/`spouse_id` 置空，登记到所属营地 `vacant_houses` 列表（附带受益人：在世子女+在世配偶，按 agent.id 升序去重）；无主空置房正常风化（与有主同速率），坍塌后从列表移除，大门路网节点可被新立宅复用。房屋转让/继承逻辑留待后续迭代。
