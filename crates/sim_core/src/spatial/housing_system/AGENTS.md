...
`world.tick()` 中"房屋系统"环节调用本管线，顺序固定：冬季供暖 → 折旧坍塌 → 丧偶解婚 → 修缮结算 → 施工计时与竣工 → 空置房登记（户主亡故→无主→营地列表）→ 二手房屋估价与麦穗拍卖（`auction.rs`）→ 金币遗产继承（`world.rs`）→ 营地行政区升级。
...
1. **动态物化成本折算 $C_{const}$**：0 级地基价值保底 0.1 金，加各级累进水粮木石，水粮取自 POI #60 榷场互市动态市价纯函数，木/石/金榷市暂未承载记 0 单价；
