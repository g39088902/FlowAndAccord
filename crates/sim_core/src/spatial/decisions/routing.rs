...
use super::strategy::{ActiveTask, ExecutionStrategy, ResidenceTarget, ResourceStage, ReturnStage};
...
impl<'a> Decisioner<'a> {
    ...
    pub fn has_available_node(&self, agent: &Agent3D, pool: NodePool) -> bool {
        pool.nodes(self.ctx)
            .iter()
            .any(|target| agent.poi_is_seekable(target.poi_id))
    }

    /// 评估节点周边连接道路的通行质量（踏路加成系数，加权平均）
    pub fn estimate_path_quality(&self, node: NodeId) -> f32 {
        let Some(&idx) = self.network.node_map.get(&node) else {
            return 1.0;
        };
        ...
        for edge in self.network.graph.edges(idx) {
            ...
            let factor = (self.config.road_level_factor_base
                + self.config.road_level_factor_wear_coef * quantized_wear)
                .clamp(
                    self.config.road_level_factor_min,
                    self.config.road_level_factor_max,
                );
            total_factor += factor;
            count += 1;
        }
    ...
    pub fn home_target(&self, agent: &Agent3D) -> NodeId {
        if let Some(house_id) = agent.home_house_id {
            ...
            self.ctx
                ...
                .iter()
                .min_by(|(_, a), (_, b)| {
                    a.distance_to(&agent.world_pos)
                        .partial_cmp(&b.distance_to(&agent.world_pos))
                        .unwrap()
                })
                .map(|(nid, _)| *nid)
                .unwrap_or(agent.home_camp_node)
    ...
    pub fn dispatch(
        ...
        if let Some(path) =
            self.network
                .find_path_3d_with_preference(start, target, agent.is_covert, self.config)
        {
            if !path.is_empty() {
                agent.state = state;
    ...
    pub fn turn_around_and_route_to(
        &self,
        agent: &mut Agent3D,
        target_node: NodeId,
        state: PrimitiveActionState,
            ...
            if let Some(&edge_idx) = self.network.edge_map.get(&lane_id) {
                ...
                if let Some(rev_edge_idx) = self.network.graph.find_edge(to_idx, from_idx) {
                    ...
                    let route = if from_node == target_node {
                        vec![rev_lane_id]
                    } else if let Some(remaining) = self.network.find_path_3d_with_preference(
                        from_node,
                        target_node,
                        agent.is_covert,
                        self.config,
