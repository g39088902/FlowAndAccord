...
> 完整机制与里程碑见 `docs/plan-ledger-refactor.md`（账本与仓库重构计划，M1~M4 已完成，M5 收尾规划中）。
...
分家权重分母 `W = 1(父) + 1(母) + n` 中 `n = 父亲 children_ids.len()`（胎儿已作为 agent 实体计入），父在世权重 1、母在世权重 1；分家/继承只记账本余额，不动物理库存；`Inheritance` 先于 `Split` 执行（丧父之子由继承直接立户，Split 幂等跳过已立户者）。
