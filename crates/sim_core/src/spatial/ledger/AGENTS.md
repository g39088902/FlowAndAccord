...
> 本目录局部操作指南。全局规则以根目录 `AGENTS.md` 为准（§4.3 确定性节拍 / §4.10 测试策略 / §4.12 超参集中化），本文件只收录本目录的职责边界、文件清单与局部易踩坑。
> 当前机制见 `../../../../../docs/current/tech/07-ledger-and-polity.md`；

---

## 1. 📂 目录职责

独立经济**账本层**（制度账本：记录"归谁、谁付的、谁收的"），与物理仓储层（`house.rs` pantry_* / `agent.rs` carried_* / `ecology/` 装卸）**完全分离**：
...
- **M4 地区**：每营地一册 `Region`（政体 `Kingdom`、继承制 `Primogeniture`）；初王 = `arrival_order` 中第一个**物理抵达营地**（距营地 < 交互半径 22m，且非远征别营过客）的在世男性（v1.22.0，杜绝 tick0 秒封未抵营始祖）；国王死亡按 长子→长孙→arrival_order 下一男性 继承，绝嗣王位空悬账本冻结；公仓税每 `ledger_tax_interval_ticks` 征收（账面余额 × `ledger_tax_rate`，有国王才征），救济有公仓门槛与冷却；**★ 国王内帑与帝国公帑**：每 `royal_privy_interval_ticks`（7200 tick = 120 游戏小时）从地区公仓各品类物资（水/粮/木/石/金）提取 `royal_privy_rate`（1%）转入现任国王家户私库（若无家户则进入随身，`tick_royal_privy`，`RoyalPrivy` 流水）；皇帝每 `imperial_privy_interval_ticks`（7200 tick = 120 游戏小时）从每个下属王国公仓各品类物资抽取 `imperial_privy_rate`（0.5%）转入皇帝家户私库（`tick_imperial_privy`，`ImperialPrivy` 流水）；在 Agent/Region/World 三层累计供调试模式展示。
...
分家权重分母 `W = 1(父) + 1(母) + n` 中 `n = 父亲 children_ids.len()`（胎儿已作为 agent 实体计入），父在世权重 1、母在世权重 1；分家/继承只记账本余额，不动物理库存；`Inheritance` 先于 `Split` 执行（丧父之子由继承直接立户，Split 幂等跳过已立户者）。**★ v1.44.0 `Agent3D.is_household_head` 为纯性能缓存（非业务真相源）**：`Split` 循环首部 O(1) 短路在世户主；新增字段默认 false，旧档/继承立户/婚姻建户等全部路径靠 Split 幂等判断（`hh.head == agent.id`）**首拍自愈回填**。业务分支一律不得读写该字段作为归属/身份判断真相源，真相始终是 `household_registry`。
...
临时单测验证后删除，长期验证依赖 `node tools/test-wasm.js`（根 AGENTS.md §4.10）。
