...
- **M1~M4 已完整落地**：
  - M1（v0.9.72~v1.0.0）：账本内核 + 团体基类 + 婚姻登记簿 + 家户体系（家庭跟着男人走）+ 胎儿预分配 ID；
...
- **M2 分家权重**：`W = 2(父) + n(子一代，含胎儿)`，分家男子抽走各类资源 `1/W`；份额按**每类资源独立计算**，只记账本余额不动物理库存。
...
分家权重分母 `2 + n` 中 `n` 含胎儿（父亲配偶怀孕且已预分配 ID）；分家/继承只记账本余额，不动物理库存；`Inheritance` 先于 `Split` 执行（丧父之子由继承直接立户，Split 幂等跳过已立户者）。
