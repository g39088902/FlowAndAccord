...
impl World3DEngine {
    ...
    pub fn seed_primitive_ecology(&mut self, _agent_count: usize) {
        ...
        let mut find_spaced_pos =
            |rng: &mut WorldRng, terrain: &crate::geo::TerrainMap, radius_ratio: f32| -> Vec3 {
                for _ in 0..100 {
        ...
        for i in 0..self.config.count_camps {
            let mut pos = find_spaced_pos(
                &mut self.rng,
                &self.terrain,
                self.config.poi_spawn_radius_camp,
            );
            pos.z += 0.5;
        ...
        for i in 0..self.config.count_water_sources {
            let pos = find_spaced_pos(
                &mut self.rng,
                &self.terrain,
                self.config.poi_spawn_radius_resource,
            );
            let node_id = self.network.add_node(pos, NodeType::GroundIntersection);
        ...
        for _ in 0..self.config.count_terrain_transition_nodes {
            let x = self.rng.gen_range(
                -half_size * self.config.poi_spawn_spread_ratio,
                half_size * self.config.poi_spawn_spread_ratio,
            );
        ...
        let valid_spawn_nodes: Vec<u32> = road_nodes
            .iter()
            .copied()
            .filter(|&nid| {
        ...
        for agent in &mut self.agents {
            ...
            match agent.state {
                PrimitiveActionState::DrinkingAtWater => {
                    ...
                    if let Some(poi) = self.pois.iter_mut().find(|p| {
                        p.poi_type == PoiType::WaterSource
                            && p.pos.distance_to(&agent_pos) < self.config.poi_interaction_radius
                    }) {
                        let need = (self.config.agent_thirst_capacity - agent.thirst).max(0.0);
                        if need > 0.01 {
                            let extracted = poi.extract(need.min(rate_res * dt));
                            agent.thirst =
                                (agent.thirst + extracted).min(self.config.agent_thirst_capacity);
                        }
                        if agent_hid.is_some()
                            && agent.carried_water < carry_cap
                            && poi.current_stock > 0.01
                        {
                ...
                PrimitiveActionState::ForagingFood => {
                    ...
                    if let Some(poi) = self.pois.iter_mut().find(|p| {
                        ...
                        if need > 0.01 {
                            let extracted = poi.extract(need.min(rate_res * dt));
                            agent.hunger =
                                (agent.hunger + extracted).min(self.config.agent_hunger_capacity);
                        }
                        if agent_hid.is_some()
                            && agent.carried_food < carry_cap
                            && poi.current_stock > 0.01
                        {
                ...
                PrimitiveActionState::BuyingAtMarket => {
                    ...
                    if let Some(poi) = self.pois.iter_mut().find(|p| {
                        ...
                        let p_water =
                            market_unit_price(poi.current_stock, poi.max_stock, &self.config);
                        let p_food = market_unit_price(
                            poi.secondary_stock,
                ...
                PrimitiveActionState::RestingAtCamp => {
                    ...
                    if agent.home_house_id.is_some() {
                        if let Some(hh_hid) = self.household_registry.household_of(agent.id) {
                            ...
                            // —— 吃喝：从家户账本真实扣减 ——
                            let ledger_water = self
                                .household_registry
                                .get(hh_hid)
                                .map(|hh| hh.group.ledger.balance(ResourceKind::Water))
                                .unwrap_or(0.0);
                            if agent.thirst < self.config.agent_thirst_capacity
                                && ledger_water > 0.05
                            {
                                let drink_amount = (self.config.agent_thirst_capacity
                                    - agent.thirst)
                                    .min(ledger_water)
                                    .min(self.config.camp_home_consume_rate * dt);
                                agent.thirst = (agent.thirst + drink_amount)
                                    .min(self.config.agent_thirst_capacity);
                            ...
                            let ledger_food = self
                                .household_registry
                                .get(hh_hid)
                                .map(|hh| hh.group.ledger.balance(ResourceKind::Food))
                                .unwrap_or(0.0);
                            if agent.hunger < self.config.agent_hunger_capacity
                                && ledger_food > 0.05
                            {
                                let eat_amount = (self.config.agent_hunger_capacity - agent.hunger)
                                    .min(ledger_food)
                                    .min(self.config.camp_home_consume_rate * dt);
                                agent.hunger = (agent.hunger + eat_amount)
                                    .min(self.config.agent_hunger_capacity);
                                if eat_amount > 0.001 {
                                    if let Some(hh) = self.household_registry.get_mut(hh_hid) {
                                        hh.group.ledger.record_consumption(
