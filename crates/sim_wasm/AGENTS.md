...
- 快照只有 FABS 二进制一条通道（JSON 通道已于 v1.50.35 移除）；导出变更必须同步 `rustworld.js`、Worker 与工具调用方。
...
- **★ v1.50.35 JSON 快照通道彻底移除**：T1（v1.46.0）时 JSON 曾收敛为 test-only 真值源（`world_snapshot_json_debug_ptr/len`）供 `test-snapshot-bin.js` 对拍；现该导出、`SNAPSHOT_JSON_DEBUG_BUF` 与门禁脚本 `tools/test-snapshot-bin.js` 均已删除，快照仅剩 FABS 一条通道，tools/ 统一走 `tools/snapshot-reader.js`。
