...
> 状态：实施规划，未实施、未排期。2026-09-13 对照当前源码复核。
...
先等订单簿、委托人身份、真实履约与最小成交单位落地。订单保留具体 principal AgentId，委托人死亡／分家等失效按市场事务处理，不把家户 ID 当人。

以买单依价格、时间、ID 的确定序扫描；对每一买单，在所有可交叉的卖单中按卖价升序、创建时间升序、`min(A→B, B→A)` 降序、卖单 ID 升序选取。无合格对手继续下一买单，沿用领域原有数量、预算与资产核验。关系只破同价同时间平局；不静默修改成熟市场优先级。效果关闭时关系键为 0，恢复价格／时间／ID。紧急选策仍由融合契约先比较救命时效，不因关系主动等待更晚的朋友挂单。
...
| AF-10 | P2／中 | AF-09 + 市场真实履约 | 普通冷却字段及配置；订单事实去重；存档及失效清理 | 拆单、跨资源、零删除、驱逐、保存恢复不绕冷却；满冷却只拒计分；普通上限 +3 |
| AF-11 | P2／中 | AF-10 + 市场撮合 | 委托人方向查询、双方较小值匹配破平、取消旧信任 bool | 同价同时间平局改变交易对象；未改价佣；饥渴自救时效与资产守恒通过 |
...
依赖主链：AF-01 → AF-02 → AF-03 → AF-04 → AF-05 → AF-06 → AF-07 → AF-08 → AF-09；之后市场就绪才进入 AF-10/11。AF-12 每个事件还必须等其领域独立就绪。记忆、激素不在 P0/P1 前置链上。
