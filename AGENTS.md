...
- `sim_core` 是确定性模拟内核；`sim_wasm` 只做桥接、tick、配置注入、快照和存档；`frontend` 负责 Worker、快照解码、配置 UI 与渲染（浏览器 UI (版本: v1.60.1)）。
...
常用运行命令：`node frontend/server.js`（默认 `http://localhost:3000`）。每次重编译 WASM 后强制刷新；页面顶部标题栏显示版本徽章 **`v1.60.1`**，版本升版统一使用 `node tools/bump-version.js`。
