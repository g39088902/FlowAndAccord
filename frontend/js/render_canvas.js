...
const NEED_KIND_REASON = {
  ...
  Rest: '正在归宿静坐休养，体力恢复速率 = 8.0%/h × 睡眠效率/100，属性越高休息越快，恢复至 100% 满值后方可结束。',
  ...
  HomeMeal: '★ v1.47.0 风烛残年：健康值低于衰弱阈值，不再外出就源与储备劳作，返家从家户账本取用饮水食物安度晚年。',
};
...
if (isCameraFollow && sim.selectionType === 'agent') {
  ...
  if (shouldUseWebgl) {
    if (!webglTerrainRenderer) {
      ...
      if (!window.WebGLAccentLayer) {
        ...
        window.WebGLShadowPass.strength = (window.RENDER_CONFIG && Number.isFinite(window.RENDER_CONFIG.accentWebglShadowStrength))
          ? window.RENDER_CONFIG.accentWebglShadowStrength : 0.38;
    ...
    if (webglTerrainRenderer && webglTerrainRenderer.isReady() && sim.terrain && sim.terrain.cells) {
      // ★ 先更新阴影图（光向可能逐帧缓变），地形渲染随后采样
      if (window.WebGLShadowPass && window.WebGLShadowPass.isReady()) {
        window.WebGLShadowPass.render(sim);
      }
