...
window.RENDER_CONFIG = {
  // —— 目标渲染帧率 ——
  // 支持 30 与 60 档位，默认 60 追求流畅显示；在性能受限设备上可热调为 30
  targetFps: 60,

  // —— 地形离屏分块瓦片烘焙缓存（★ v1.50.72 性能优化）——
  // 60x60 地形网格划分为 6x6=36 个 Chunk 瓦片离屏烘焙；
  // 平移/跟随帧 100% 缓存复用（零重绘），仅在旋转/缩放/光照变动时置脏重烤；
  // 有动态实体穿插的山地分块自动回退逐格排序，兼顾 60 帧满帧吞吐与绝对深度遮挡精度。
  terrainChunkCacheEnabled: true,
  terrainChunkGridCount: 6,
