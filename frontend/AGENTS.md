...
>
> 本目录是原生静态前端：57 个 JS 文件（含 ★ TA-06 人工验收台 `ta06-acceptance.js`，仅 `?ta06=1` 激活）（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / **★ TA-07 `accent-lod.js`** / `render_accents.js` / `render_bush.js` / `render_grass.js` 与 ★ S4-02/S4-03 资源景观套件 `landscape-model.js` / `landscape-mask.js` / `render_landscapes.js`、★ S4-06 标签布局层 `label-layout.js`、★ TA-12-2 地表纹理模型层 `terrain-texture.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
38. ta06-acceptance.js      ★ v1.52.1 植被观察台（控制台按钮或 ?ta06=1 激活；未打开零影响，最后加载）
```
