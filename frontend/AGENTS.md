...
> 本目录是原生静态前端：44 个 JS 文件（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / `render_accents.js` / `render_bush.js` / `render_grass.js` 与 ★ S4-02/S4-03 资源景观套件 `landscape-model.js` / `landscape-mask.js` / `render_landscapes.js`、★ S4-06 标签布局层 `label-layout.js`、★ TA-12-2 地表纹理模型层 `terrain-texture.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
| `js/render_depth_queue.js` | ~550 | ★ v1.50.46 TA-04-6 前置自 render_world.js 拆分（守 §4.6 800 行上限 + 「入队和分发归队列层」单一职责）：**世界统一深度队列层**——DEPTH_* 常量（★ 新增 DEPTH_ACCENT_SHADOW=12 树/灌木贴地投影）、`_depthPool` 对象池、`_ownCellCenterDepth`/`_surfaceDepth`/`_decalDepth` 贴面/足迹感知深度、`MAP_Z_LIFT`/`projectLifted`、`drawWorldEntities()` 收集（地形格/侧壁/水系/游鱼/波光/道路/辖区连线/POI 底座与标记/房屋/装饰/树灌木阴影/族人）与远 → 近分发。**各图元绘制逻辑不进本文件**；树/灌木阴影深度 = 基点/影梢足迹深度取大（世界落点 + 影梢 = 锚点 + 世界阴影方向 × 影长 × 实高，SimLighting.shadowDirInto），不沿用树根/树冠深度。★ S4-06/S4-07：drawWorldEntities 帧首 updateEntityHover 实体悬浮检测（同步 sim.hoveredEntity）+ LabelLayout.beginFrame → POI/房屋/族人收集循环内同步提案（proposePoiLabels/proposeHouseLabels/proposeAgentLabels）→ list.sort() 前 resolve() 安置 → 分发循环结束后 drawSelectedNeedBubbleOverlay() 交互覆盖标签 → LabelLayout.drawClusters/drawLeaderLines/syncFallbackDockDOM 同步渲染与 DOM 停靠区 | 绘制入口（render_terrain/render_world/render_accents/render_shadows/render_grass/render_agents）、标签布局（label-layout） |
