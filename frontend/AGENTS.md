...
> 本目录是原生静态前端：41 个 JS 文件（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / `render_accents.js` / `render_grass.js`、★ S4-02/S4-03 资源景观套件 `landscape-model.js` / `landscape-mask.js` / `render_landscapes.js` 与 ★ TA-12-2 地表纹理模型层 `terrain-texture.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
