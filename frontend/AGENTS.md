...
> 本目录是原生静态前端：41 个 JS 文件（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / `render_accents.js` / `render_grass.js` 与 ★ S4-02/S4-03 资源景观套件 `landscape-model.js` / `landscape-mask.js` / `render_landscapes.js`、★ S4-06 标签布局层 `label-layout.js`、★ TA-12-2 地表纹理模型层 `terrain-texture.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
| `js/config.js` | ~215 | `window.SIM_CONFIG` 全局数值配置（276 字段，含拆分配置合计），按功能分区注释 | 前端配置文件是数值权威，Rust 负责接收契约 |
...
| `js/render_landscapes.js` | ~280 | ★ S4-02（STAGE-04-TODO §3.1/§3.4）**资源景观绘制接入层**：`collectLandscapes`（由 render_depth_queue.js::drawWorldEntities 在装饰阴影段后调用——先 `LandscapeModel.sync(sim)`（静态签名变化才重建几何；遮罩层已在装饰段前同步，此处幂等兜底），被遮蔽子图元（`LandscapeMask.childHidden`，S4-03）连同投影先于预算跳过，再把子图元与树/灌木贴地投影分别以 DEPTH_LANDSCAPE(13)/DEPTH_LANDSCAPE_SHADOW(14) 入队，深度口径与装饰一致走 `_decalDepth`；`landscapeFrameChildBudget` 每帧入队硬上限、固定遍历序截断、视口粗剔除）/ `drawLandscapeChild`（复用 render_accents/render_grass 既有图元与 SimTreeTint 季相，**不复制光照公式**）/ `drawLandscapeShadowGround`（复用 render_shadows.js::drawAccentShadowFor 模型参数化主体）。子图元模型经 `AccentModel.getByKey('L#…')` 解析；未知 modelKind 跳过+计数（debug 限频报警）。**配置关态（`landscapeEnabled=false`）零入队零同步，完整回退原画面路径**；开关为纯渲染开关，严禁经 applyConfig 写模拟事实。★ S4-04：collect 增 `LandscapeModel.childActive` 过滤——`stockRole:'detail'` 可采细节按 q≥qThreshold 显隐（不入队不占预算，骨架恒可见）；新增 `drawLandscapeGroundPatch` 贴地色差片本地图元（GroundPatch 专用，无 AccentModel 模型：地面圆经 rotX 投影椭圆，两遍式外弱内深柔和色差，内圈中心按子图元稳定 rot 微偏；固定贴地色**不走** accentLitFill——贴地片无受光法线；冬季按 `landscapeGroundWinterAlphaRatio` 减弱；**填充样式串按 tone×季节预建常量（`_gpStyles` 缓存），渲染热路径零字符串分配**；湿土 tone=wet α 0.28/0.22、林下暗部 tone=shade α 0.15/0.13）。★ S4-05：增 berry（深浆果红点簇）/ gold（哑光金矿脉斑，**禁发光**）/ quarry（可采面灰斑，globalAlpha=0.06+0.16q 随 q 增强）三 tone——点簇可见点数 = round(10×q) 连续单调；q 相关强度走 globalAlpha + 常量 fillStyle **零字符串分配**；collect 时组 q 镜像 `child._q` | 配方/缓存（landscape-model）、遮罩（landscape-mask）、深度队列调度 |
| `js/label-layout.js` | ~330 | ★ S4-06（STAGE-04-TODO §4.6）**标签候选层与屏幕布局** `window.LabelLayout`：世界画布 10 处文字入口统一抽离为「提案 → 测量 → 矩形 → 优先级 → 固定备选位」布局层。帧内三段式挂载 render_depth_queue.js::drawWorldEntities——① beginFrame(w,h) 重置提案池/冲突网格/UI 矩形；② 收集阶段 proposePoiLabels（render_world.js）/proposeHouseLabels（render_world.js）/proposeAgentLabels（render_agents.js）逐实体提交候选（锚点 projectLifted + 类别）；③ list.sort() 后 resolve()：按 (优先级, 收集序) 稳定排序安置——**pinned**（实体锚定标记：POI 图标/施工🔨/流产🥀/夺位⚔️）恒接受占格不移位，**ordinary**（可省略文字：营地名称/舍数、拍卖🔨/修缮🔧/房屋编号）依次尝试 [首选→锚点镜像→首选同排右→首选同排左] 四个固定备选位（候选数有界 ≤4），屏幕网格冲突检测（labelGridCellSize 分桶 + 链表，非全对扫描）+ UI 禁入矩形（labelUiRectIds，labelUiRefreshFrames 帧节流重测 + resize 立即置脏），全败即省略。绘制侧旧入口只消费 posOf(key)——普通标签仍在**实体深度**落笔，地形遮挡行为不变（山后标签不透山）；布局关态（labelLayoutEnabled=false）posOf 恒 false 完整回退 v1.50.51 旧直接绘制。**overlay 通道** overlayPlace()：选中族人需求气泡（自 drawAgent 迁出为 drawSelectedNeedBubbleOverlay，深度队列分发循环结束后调用）强制安置不省略、不参与地形遮挡，避开 UI 矩形与已接受矩形后钳制画布内。字体测量缓存（font+text→宽，上限 2048 清空）；矩形模型：CJK/拉丁 0.85/0.35em + pad，**emoji 独占文本用墨迹近似框 0.72/0.10em**（胖盒会系统性挤走贴图标排布的普通标签——验收修正①）；左右备选位与首选**同排**（贴锚点左右必撞 pinned 图标框——验收修正②）。提案上限 labelMaxProposals（普通超限按收集序丢弃，pinned 不限）。键位规划 = 实体 id×16 + 槽位（poi 0..3/house 4..6/agent 7..9，气泡 10）。**零内核交互**：纯前端表现层，不写模拟事实，拾取路径（render_inspector.js）未动 | 提案（render_world/render_agents/render_depth_queue）、绘制消费（render_world/render_agents） |
| `js/render_depth_queue.js` | ~460 | ★ v1.50.46 TA-04-6 前置自 render_world.js 拆分（守 §4.6 800 行上限 + 「入队和分发归队列层」单一职责）：**世界统一深度队列层**——DEPTH_* 常量（★ 新增 DEPTH_ACCENT_SHADOW=12 树/灌木贴地投影）、`_depthPool` 对象池、`_ownCellCenterDepth`/`_surfaceDepth`/`_decalDepth` 贴面/足迹感知深度、`MAP_Z_LIFT`/`projectLifted`、`drawWorldEntities()` 收集（地形格/侧壁/水系/游鱼/波光/道路/辖区连线/POI 底座与标记/房屋/装饰/树灌木阴影/族人）与远 → 近分发。**各图元绘制逻辑不进本文件**；树/灌木阴影深度 = 基点/影梢足迹深度取大（世界落点 + 影梢 = 锚点 + 世界阴影方向 × 影长 × 实高，SimLighting.shadowDirInto），不沿用树根/树冠深度。★ S4-06：drawWorldEntities 帧首 LabelLayout.beginFrame → POI/房屋/族人收集循环内同步提案（proposePoiLabels/proposeHouseLabels/proposeAgentLabels）→ list.sort() 前 resolve() 安置 → 分发循环结束后 drawSelectedNeedBubbleOverlay() 交互覆盖标签 | 绘制入口（render_terrain/render_world/render_accents/render_shadows/render_grass/render_agents）、标签布局（label-layout） |
...
2. config.js                  SIM_CONFIG (276 字段，含拆分配置合计)
...
28. render_accents.js         ★ v1.50.23 TA-01 装饰绘制层（drawAccentEntity 等，早于 render_world.js）
...
31a. label-layout.js          ★ S4-06 标签候选层与屏幕布局（window.LabelLayout，须早于 render_depth_queue.js / render_world.js / render_agents.js）
...
- **渲染系列（21-37）最后加载**，`render_canvas.js` 的 `render(now)` 主循环依赖 `window.rustWorld`、`window.dag`、`window.ledgerUI` 等全局对象；共享全局作用域，函数声明可提升，加载顺序为 canvas→river_life→accent-season→accent-model→landscape-model→landscape-mask→terrain→accents→grass→shadows→landscapes→hud→depth_queue→world→agents→inspector
