...
>
> 本目录是原生静态前端：51 个 JS 文件（js/ 根 45 + webgl/ 6；含 ★ TA-06 人工验收台 `ta06-acceptance.js`，仅 `?ta06=1` 激活）（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / **★ TA-07 `accent-lod.js`** / `render_accents.js` / `render_bush.js` / `render_grass.js` 与 ★ S4-02/S4-03 资源景观套件 `landscape-model.js` / `landscape-mask.js` / `render_landscapes.js`、★ S4-06 标签布局层 `label-layout.js`；★ v1.60.1 删除 `terrain-texture.js` / `terrain-mesh-merge.js` / `render_shadows.js` 与 webgl/ 下 `fallback-handler.js` / `render-canvas-patch.js` / `test-grid.js` 六文件）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
| `js/water_gpu.js` | ~1000 | ★ **2026-09-26 水体粒子 GPU 求解器** `window.WaterGPU`（★ **2026-09-27 改为真三维 3D PBF**）：WebGPU compute 推进 PBF（补源 / **3D 重力 + 地面接触**受力预测 / **3D 网格空间哈希**（cellsX×cellsY×cellsZ、3×3×3 邻域、**分层前缀和**）/ 密度-λ / Δp / XSPH / 淘汰 / 打包，14 个 WGSL 入口共享 1 uniform + 3 storage buffer），上限 2 万粒子。**3D 核函数**（poly6 / spiky 梯度 / 自贡献）、`mass = spacing³`、静止密度取 3D 点阵；粒子 `z` 是**真实自由度**，地面接触把重力投影到地形切平面（`g−(g·n)n`）⇒ 顺坡下泄并汇入洼地成水体（**不再有** 2D 动态水深场 / 竖直水壁）。★ 2026-09-27 **删除清泉（`Water` POI）涌水**（粒子唯一水源是**降雨**），并新增**蒸发机制**：`despawn` 里 age 累积速率按「被水环绕程度」插值（邻居越多越慢、孤立/薄水膜越快）。结果经 staging `mapAsync` 每帧一次回读写 `rustWorldSim.fluidParticles` + `fluidParticleIds`，**世代号守卫**防换世界脏写。**纯表现层、放弃确定性、不进快照/存档**（内核水已休眠）；无 WebGPU 由 `main.js` 硬门槛阻断启动（`?watergpu=0` 为 A/B 逃生门）。须早于 `main.js`（其 WebGPU 硬门槛在加载期探测 `window.WaterGPU`；迟于 main.js 会误报「模块缺失」）。⚠️ 三个 storage buffer **必须带 `COPY_DST`**（地形/槽位置空走 `queue.writeBuffer`，缺它会被 WebGPU 静默丢弃 ⇒ 水面恒空）。详见 `docs/current/tech/33-runtime-fluid.md` §8 | 模拟状态写入、WorldRng、快照/存档契约 |
| `js/water_particles.js` | ~200 | ★ **v1.62.0 水体粒子渲染层**（求解已上移到**前端 WebGPU `water_gpu.js`**，权威说明见 `docs/current/tech/33-runtime-fluid.md` §8）——本层只消费 `rustWorldSim.fluidParticles` 的粒子位置，逐粒子视觉抖动由「**槽位号**（`fluidParticleIds`，无则回退下标）+ 世界种子」稳定哈希派生（只算一次 ⇒ 稳态不闪、零堆分配），复用 v1.61.4 的观感常量（配色/透明度/每 5 粒亮点）与 sink 落笔路径；★ **v1.63.0 粒子数逐帧可变**（求解器降雨/泉涌补源、边界出流、坡面下渗）——本层按 `count` 伸缩 `_particles` 视图，粒径仍取求解器下发的 `fluidFillRadius`/`fluidToneRadius`（不随粒子数漂移）；★ **v1.63.0 启用门槛取消**：`init` 不再要求「静态水系特征非空」（`render_depth_queue.js` 也无条件调 `update`），因为开放水循环会在无任何水系特征的地形上造出水——有无水唯一由求解器输出决定（游鱼 `RiverLife` 仍按 `features.length` 启停）；★ **2026-09-27 起粒子 `z` 为三维真实高度**（重力/堆叠/跌落），本层不再消费任何水深字段（原「动态水深场 + 竖直水壁」已删），投影落笔路径不变；**v1.61.3 的前端流场构建 / 渠坐标积分 / 覆盖率折叠选活 / 墙钟累加器已全部删除，勿复活**。以下为历史沿革（已过时，仅供追溯）：`window.WaterParticles`——静态水体外轮廓只作为**场源**：河流取中心线断面表（河心/切向/半河宽/断面高程，倒坡整表倒序 ⇒ `u=0` 恒为上游），湖泊取到岸有符号距离场（扫描线填充 + 双向倒角距离，格距 1~6m、每体 ≤64×64 格，创世期一次）。`init(features,seed)` 用 `_engineSeed` 独立 PRNG 建场，粒子数由面积推导（河流 `area/2.2²`、湖泊 `area/3.0²`，单体上限 1500/1300、**全图预算 3600** 超限等比缩），粒径 = **实际间距 × 1.30** 自适应；`update(timeMs)` 墙钟累加器 + **固定 1/60 步长**（每帧 ≤4 步、长暂停丢弃积压）：河流在渠坐标系 `(u,q)` 积分（顺流速度松弛 × 断面坡度 + 横向回中弹簧 + 湍流 + `q=±1` 岸线反弹 + `u≥1` 回上游重生），湖泊世界坐标积分（出界硬回推 + 近岸软回推 + 慢漂移环流）；`coverage` 收窄展示域（`√coverage` 绕质心/河心）并**折叠选活**（河流从两岸退、湖泊从岸线退，粒子身份稳定不闪烁）；`drawParticle` 向 `WebGLAccentLayer` sink 分发压扁菱形（★ sink 硬门槛：GL 未就绪帧整帧跳过），逐粒子旋转/尺寸/透明度抖动打散栅格感；`stats()` 调试探针。**不写模拟状态、不消耗 WorldRng、不进存档**；入队端屏幕剔除在 render_depth_queue.js；★ v1.61.4 起为**水面唯一来源**（地表水格不承载水色、terrain-style 水格乘色与 2D 多边形水面绘制已删） | 模拟状态写入、WorldRng 消耗、快照契约 |
...
13b. water_gpu.js             ★ 2026-09-26 水体粒子 GPU 求解器（WebGPU compute **3D PBF**；写 rustWorldSim.fluidParticles）。
                              ⚠️ **必须早于 main.js**——main.js 的 WebGPU 硬门槛在**加载期**即探测 `window.WaterGPU`，
                              迟于 main.js 会误报「模块缺失」；亦须早于 water_particles.js 与 render_canvas.js 的每帧 step
