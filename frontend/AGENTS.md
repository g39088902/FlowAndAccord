...
| `js/webgl/layers/accents/stone-renderer.js` | ~300 | ★ v1.50.83 **石头绘制 WebGL 迁移（31 号阶段三石体切片）** `window.WebGLStoneRenderer`（实例 `window.WebGLStoneLayer`，render_canvas.js 与地形渲染器同点惰性创建）：Boulder / RockCluster（含资源景观子石）GPU 绘制。**几何与配色单一同源**——render_accents.js::drawStoneBody / drawAccentRockCluster 在 `sinkOn` 期间把与 Canvas 逐位相同的屏幕空间图元按同一画序分发（`quad`/`polyRing`/`profileStroke`/`ellipseRGBA`），本层只做三角化 + 逐三角形解析式边缘 AA（重心坐标 × 每边像素距离，内部边旗标防接缝）+ 闭环 miter 展开描边条带；**深度测试开、深度写入关**——对 GL 地形读深度（锚点世界深度 + 1 世界单位朝相机偏置，NDC z 系数与 projection-utils sz=-0.0005 逐位一致），石体间保持画家收集序；`beginFrame/endFrame` 由 render_canvas.js 在 drawWorldEntities 前后调用（同一 GL 帧、不清屏）；`boulderGroundShadow()` 为**唯一观感新增**（Boulder 地面投影阴影，Canvas 路径单石无阴影）。⚠️ 画风红线：本层**严禁**复制任何几何/光照公式（顶点与颜色全部来自 drawStoneBody sink 分发）；Canvas 路径修改 drawStoneBody 时 sink 分发必须同步保持。配置走 `RENDER_CONFIG.stoneWebgl*`（`?stonegl=0` 完整回退 Canvas）。**过渡边界**：石体在底层 GL 画布、位于 2D 覆盖层之下（跨画布遮挡阶段四消除） | 几何/配色（render_accents sink 分发）、GL 地形深度（terrain-renderer）、帧编排（render_canvas） |
...
- ★ v1.50.83 `webgl/layers/accents/stone-renderer.js` 注册于 index.html WebGL 块（terrain-renderer.js 之后、render_canvas.js 之前）；实例在 render_canvas.js 与 `TerrainWebGLRenderer` 同点惰性创建，`beginFrame/endFrame` 夹住 `drawWorldEntities()`——GL 地形未接管时 sink 不置位，石体自动回退 Canvas 路径
