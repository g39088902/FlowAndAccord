...
> 本目录是原生静态前端：37 个 JS 文件（含 ★ M4 `snapshot-bin.js` 二进制解码器与 ★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / `render_accents.js` / `render_grass.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
28. render_shadows.js         ★ v1.50.46 TA-04-6 装饰贴地投影绘制层（树/灌木地面图元阴影，紧随 render_grass.js）
29. render_hud.js             HUD/大盘辅助函数（v1.7.1 拆分）
30. render_depth_queue.js     ★ v1.50.46 TA-04-6 世界统一深度队列层（自 render_world.js 拆出，早于 render_world.js）
31. render_world.js           路网/POI/房屋绘制 + lightShadowOffset/shadeHex（★ v1.50.46 深度队列迁出）
32. render_agents.js          族人/特效绘制（v1.7.1 拆分）
33. render_inspector.js       Inspector 面板/点击拾取（v1.7.1 拆分）
34. auction-ui.js             拍卖大盘（最后加载，独立模态）
```
