...
> 本目录是原生静态前端：39 个 JS 文件（含 ★ M4 `snapshot-bin.js` 二进制解码器、★ v1.50.23 TA-01 装饰套件 `accent-season.js` / `accent-model.js` / `render_accents.js` / `render_grass.js` 与 ★ S4-02 资源景观套件 `landscape-model.js` / `render_landscapes.js`）+ index.html + map.html + style.css + map.css + server.js，无构建工具，纯静态文件。
...
| `index.html` | ~900 行 | 单页应用骨架：Canvas 容器 / 顶栏（含存档按钮） / Inspector / 制度大盘 / 决策引擎覆层 / 存档面板 / 族谱模态 / **★ v1.27.0 启动存档门禁层 `#startup-save-gate`**（v1.28.0 起已连接默认存档时自动读档续演；v1.28.1 起权限未持久化不删记录、提供授权按钮重授）/ 37 个 script 标签按序加载（★ M4 含 `js/snapshot-bin.js`） |
...
25. landscape-model.js        ★ S4-02 资源景观模型层（window.LandscapeModel 配方/组缓存，须早于 render_landscapes.js）
26. render_terrain.js         地形网格/水系特征/天空氛围（★ v1.48.0 从 render_world.js 拆出）
27. render_accents.js         ★ v1.50.23 TA-01 装饰绘制层（drawAccentEntity 等，早于 render_world.js）
28. render_grass.js           ★ v1.50.39 GrassTuft 草丛绘制（自 render_accents.js 迁出，紧随其后加载）
29. render_shadows.js         ★ v1.50.46 TA-04-6 装饰贴地投影绘制层（树/灌木地面图元阴影，紧随 render_grass.js）
30. render_landscapes.js      ★ S4-02 资源景观绘制接入层（子图元/阴影入统一深度队列，紧随 render_shadows.js）
31. render_hud.js             HUD/大盘辅助函数（v1.7.1 拆分）
32. render_depth_queue.js     ★ v1.50.46 TA-04-6 世界统一深度队列层（自 render_world.js 拆出，早于 render_world.js）
33. render_world.js           路网/POI/房屋绘制 + lightShadowOffset/shadeHex（★ v1.50.46 深度队列迁出）
34. render_agents.js          族人/特效绘制（v1.7.1 拆分）
35. render_inspector.js       Inspector 面板/点击拾取（v1.7.1 拆分）
36. auction-ui.js             拍卖大盘（最后加载，独立模态）
```
...
- **渲染系列（21-36）最后加载**，`render_canvas.js` 的 `render(now)` 主循环依赖 `window.rustWorld`、`window.dag`、`window.ledgerUI` 等全局对象；共享全局作用域，函数声明可提升，加载顺序为 canvas→river_life→accent-season→accent-model→landscape-model→terrain→accents→grass→shadows→landscapes→hud→depth_queue→world→agents→inspector
