...
> **阶段二退出条件**（06 号 R.3、§5.1、§5.3 末段）：基座自身不改世界；旧 T1/T2 的高程、坡度、地表、flags、肥力、水系、路网、POI **逐字节不变**；改变物理事实 = 否；`TERRAIN_GENERATOR_VERSION` 保持 4 不递增；补齐 R.1 遗留 3 项门禁；加回 `terrainGenerationMaxRetries` 且满足消费点门禁。
...
| **STAGE2-6** | 初始营地生存连通分量与往返成本诊断（补齐 §18.1 门禁） | `geo/query.rs` / `ecology/spawn.rs` | ⏳ 待办 | STAGE2-3, STAGE2-5 |
| **STAGE2-7** | `flat_baseline` 行为等价基线支持（补齐 §18.1 门禁） | `geo/terrain.rs` / `config.rs` | ⏳ 待办 | STAGE2-3, STAGE2-5 |
| **STAGE2-8** | 阶段二代码收口：跨构建差分全等验证 + 遗留门禁销项 + 文档同步 | 全链路 / 验收脚本 / 文档 | ⏳ 待办 | STAGE2-1～7 |

## 未拆分阶段

阶段三～八与阶段九a~九d（首批子特征、D-C、P1 四模板〔含湖畔盆地〕、阶段七插队模板〔河谷聚落/平地草原/半坡林地〕、复杂水系/R0、远期批次〔静态扩展/沙漠绿洲/T4 动态水文/海岛〕）暂未拆分，排期与依赖见 06 号 R.3 阶段计划表（2026-09-13 起 16 张模板全部排期）；完成阶段二后按同一粒度逐批拆入。

开工约束：阶段二按 06 号 §5.3 先迁移旧 T2 陆地公式并完成跨构建物理等价比较（STAGE2-2）；阶段三按 §5.4.E 支脊规格与 §5.4.D 岩壁离散尺度探针准入，候选参数不得当作已验证默认值。这些仍是未实施任务，不随本次方案修订勾选完成。
