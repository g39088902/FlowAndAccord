...
- [ ] **D-B1-8 场景样板：支脊山口 + 岩壁河谷（美术侧）**
    ...
    - **进展（2026-09-14）**：双场景视觉目标草案已交付，用户已明确“接受构图方向”；图稿、提示词、前置源码核对与后续实机证据矩阵归档至 [07 号 §4.4.1](docs/plan/tech/07-terrain-art.md#441-ta-09-构图评审与实机交付记录)。当时阶段三 RiverCliff 尚未完成（TA-08 已删除视为完成）；真实种子/存档及两场景实机验收仍待交付，保留未勾选。
    - **前置复核（2026-09-22）**：TB-01 与 RiverCliff（v1.52.6 / 生成器 14）已落地，可开始种子筛选；这不等于跨层遮挡或实机验收通过。默认仍为混合管线，先做遮挡冒烟，系统性失败转 TC-03/渲染缺陷；执行见 [TA-09 实施方案](TA-09-IMPLEMENTATION-PLAN.md)。
    - **执行进展（2026-09-22/23）**：TA-09-0 冒烟 PASS；L1 几何扫描完成（T1: 60/60 全过；T2: 11 个 RiverCliff 注入种子）；L2 完整世界验证完成（T1: 9/9 不降级；T2: 11/11 保留 RiverCliff+双浅滩）；L3 构图初评与 L5 人工评审均由用户确认通过（G-1 可读性、构图对齐、接地、遮挡无阻断）。候选冻结：T1 主=seed 1 / 备=seed 43；T2 主=seed 1 / 备=seed 38。seed=0 因 worker 入口缺陷排除。证据归档 [TA-09 实施方案](TA-09-IMPLEMENTATION-PLAN.md)。L4 矩阵采集/L6 性能/L7 全量存档为后续技术任务。
    ...
    - 依赖：TB-01 与 06 号阶段三 RiverCliff 均已落地；TA-08 已删除视为完成，但 31 号 §8.5 的实际遮挡结果仍须验证，不能由任务关闭推定管线能力已齐。
...
- [ ] **阶段八 · 复杂地貌模板**
    - 规划模板：火山湖（`lake_basin_v1`）、冲积扇（`alluvial_fan_v1`）等。
