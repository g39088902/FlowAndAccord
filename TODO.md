...
- [x] **TA-06 植被轮廓与物种变体（三乔木 + 三灌木）**
    - 详见 [TA-06 实施方案](TA-06-TODO.md) 与 [07 号文 §6.3/§11.4](docs/plan/tech/07-terrain-art.md)；TA-06 现状 ✔ v1.52.2（2026-09-22 人工验收通过：实现层 v1.50.64 落地 + 植被观察台 `ta06-acceptance.js`（左下角控制台按钮入口）+ seed42 七株身份表 + GL/Canvas 双路径全过 + 景观抽查 18 组 48 株 + 用户逐项走查 §9 清单通过。**仍 NOT_RUN**：Chrome/Edge 真实 LOAD 存档链路（`?nogate=1` 不能替代）、§11.3 性能 A/B 四场景）。
