...
- [ ] **TA-06 植被轮廓与物种变体（三乔木 + 三灌木）**
    - 详见 [TA-06 实施方案](TA-06-TODO.md) 与 [07 号文 §6.3/§11.4](docs/plan/tech/07-terrain-art.md)；TA-06 现状 ◐ v1.50.64（实现与静态/数值验证已落地：物种派生 + 三乔木轮廓 + 三灌木变体 + 花朵图元 + 冠幅单一来源联动）。
    - ★ 2026-09-22 验收补做（内置预览浏览器 + `?nogate=1`，默认 **WebGL** 路径）：6 变体分布/纯函数性/非植被零影响/**景观共用通道物种一致（57 对零失配）**/暖冷缓存一致/三档真实覆盖/四季与花历数值矩阵 全部通过，视觉矩阵（四季 × 四方位 × 两俯角 × 三缩放）与 6 变体近景取证归档 `docs/plan/tech/assets/ta06-evidence-2026-09-22/`；**性能 A/B 按「已无性能问题」取消**（同 TA-07-10 口径）；**Chrome 真实存档 LOAD 链路仍 NOT_RUN**（须 Chrome/Edge 的 File System Access API，可与 TA-05 LOAD 补测合并）。
