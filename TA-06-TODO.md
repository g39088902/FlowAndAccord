...
> **状态：未实施（规划态，2026-09-14 编写）。** 文中全部源码事实为 2026-09-14 逐行核对结论（应用版本 v1.50.57、HEAD `fcbd5dc`）；参数为首轮建议值，实施后以各任务实施记录为准。
...
- **全部叶簇必须挂在真实枝条上**（v1.50.26 悬空簇教训）——补位簇锚定枝段参数位 55%~95%，锥形常绿锚定轮生短枝；禁用抽象球面包络定位。
- 锥形常绿**不新增「不脱落」特例分支**：`evergreen` 曲线叶量 ≥0.94 + `fade 0.09` 下 `accentClusterVisibility` 恒 ≥1（实施时以断言固化该结论）；若实测出现掉簇，优先调曲线而非在可见度函数内加 kind 判断。
- 簇的法线仍由 `attachCrownNormals` 按整副骨架包络求解（受光管线不改）；倾干剪切继续由绘制层施加，不入模型缓存。锥形常绿倾干幅度收敛（`leanShear` 系数按轮廓下调，针叶树读感挺直）。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 落叶多茎 | `multiStem` | 5~7 | 3.4~5.0 | 0.38~0.68 | ×1.0（8） | 1.5~2.4 | `deciduousBush` | 现状形态；冬季枯枝为主、少量宿存叶 |
...
- 花位 = 构建期由 `_accentHash(id, 810+i×3…)` 在**可见簇外围**预生成的固定点（模型层输出 `flowers: [{cx,cy,cz, dot, hue}]`，绘制层只投影），最多 `accentFlowerDotsMax`(9) 点；花量 `flowerAmount` 只决定**可见点数** `round(K×flowerAmount)` 与 alpha，绝不参与位置重抽（对齐 S4-04 点簇「库存不参与几何」的同一纪律）。
- 花色走低饱和三色板（白 `246,242,232` / 淡粉 `226,190,186` / 淡黄 `238,224,168`），按稳定哈希逐点选定；**禁发光、禁径向渐变光晕、禁高饱和**（07 号 §4.1：明亮颜色留给选中对象与紧急事件）。
- 细节分级：仅在 `detailMid` 及以上、且点屏幕半径 ≥ `accentFlowerMinPx` 时落笔；远景整组省略。
- 受光：花朵为小尺度贴簇图元，直接以簇法线走 `accentLitFill` 单一入口，**不复制光照公式**、不新增固定屏幕亮斑。
- 零 GC：花点投影复用 `_ptD` 与专用 `_flowerScratchPool`，稳态零逐帧堆分配（TA-11-6 纪律）。
- 季相边界：`accentFlowerCycle` 现状为 `u=0` 满开、0.16 归零、0.96 回升 0.65——实施时须核对「冬末回升」是否与叶量萌芽曲线协调（若出现冬季满花，收紧 0.88/0.96 两结点，属曲线微调不改结构）。
...
| `accentTreeSilhouettes` | 每轮廓 `{ trunkHBase, trunkHVar, crownR, branchMin, branchMax, subBranchPer, clusterFactor, clusterRBase, clusterRVar, elevMin, elevMax, whorlLayers?, whorlBranches? }` | `treeSkeleton` |
| `accentBushVariants` | 每变体 `{ stemMin, stemMax, heightBase, heightVar, outKMin, outKMax, clusterFactor, clusterRBase, clusterRVar, squash }` | `bushSkeleton` |
| `accentFlowerDotsMax` | 9 | 模型层花位生成上限 |
| `accentFlowerDotRadiusK` | 0.09（相对簇半径） | `render_bush.js` |
| `accentFlowerMinPx` | 1.6 | 同上（远景省略） |
...
模型层输出并作为唯一真相源：`sk.trunkH`（实高）、`sk.crownR`（冠幅）、`sk.footprintR`（深度足迹 = 冠幅 × 轮廓系数）、`model.extent`（锚点上方最大延伸，TA-07 预留）。改造点：
...
- [ ] **TA-06-1 前置基线与样板世界（约 0.5 日）**
  ...
  - 建立**无变体基线**：现状截图（四季 × 四方位 × 三景别）+ 统一队列绘制耗时基线 + 现存落叶/常绿个体清单（按 `AccentModel.get(a).evergreen` 筛选，**不得**用 `accentEvergreenChance` 置零筛样板——模型已缓存该字段，见 09 号 §2）。
  - 交付：基线记录与样板清单；不得从既有截图反推 seed，不跳过正式存档门禁。

- [ ] **TA-06-2 物种派生层与 profile 单一入口（约 1 日，依赖 1）**
...
- [ ] **TA-06-3 三乔木轮廓骨架（约 1.5 日，依赖 2；可与 4 并行）**
...
- [ ] **TA-06-4 三灌木变体骨架与花位（约 1 日，依赖 2；可与 3 并行）**
...
- [ ] **TA-06-5 前置拆分 `render_bush.js`（约 0.5 日，依赖 2；先于 6/7）**
...
- [ ] **TA-06-9 景观共用通道抽查与生命周期断言（约 1 日，依赖 8）**
...
- [ ] **TA-06-10 Chrome 视觉与受光验收 + 证据包（约 1 日，依赖 9）**
...
- [ ] **TA-06-11 性能 A/B（约 0.5~1 日，依赖 10）**
