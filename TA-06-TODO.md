...
> **状态：✔ 人工验收通过（2026-09-22，v1.52.2）；TA-06-11 性能 A/B 与 Chrome 真实 LOAD 仍 NOT_RUN。** 本文源码事实为 2026-09-14 逐行核对结论（应用版本 v1.50.57、HEAD `fcbd5dc`）；实施前复核确认 HEAD 已移至 `0f9a051`（v1.50.62），**TA-06 相关前端源文件相对基线零变动**（仅 index.html 版本徽章号变化），故基线表继续有效。
...
- [x] **TA-06-9 景观共用通道抽查与生命周期断言（约 1 日，依赖 8）** —— ✔ 抽查取证完成（2026-09-22，seed 42：LandscapeModel 18 组 / 48 株景观灌木，deciduousBush 30 / evergreen 10 / floweringBush 8；8 株原生花灌木中 3 株被 accentHidden 隐藏、5 株未遮罩）；生命周期事件断言随人工验收 §9-H 通过
...
- [x] **TA-06-10 视觉与受光验收 + 证据包（约 1 日，依赖 9）** —— ✔ 人工验收通过（2026-09-22，用户逐项走查 §9 清单 A~J）：六变体轮廓可辨、四季季相正确（春芽/夏满/秋变色/冬落叶+常绿保叶）、花朵春季可见夏冬零、受光分离随光向翻转、GL/Canvas 双路径一致、机位/缩放/标注开关正常、无参数零影响；96 视图全矩阵由验收人按 §9 走查覆盖；真实 LOAD 仍 NOT_RUN（§9-H，须 Chrome/Edge 手动）
...
- [x] **TA-06-12 门禁、升版、WASM 双副本与文档收口（约 0.5 日，依赖 11）** —— ✔ 2026-09-22 完成：patch 升版 v1.52.0→v1.52.2（纯表现层，不动存档兼容线，**不重编译 WASM**、旧档照用）；frontend-check（57 文件）/ config-check（370 字段）/ cross-doc（冲突 0）/ doc-link（845 条全可达）/ bump-version --check（12 定义点零漂移）/ git diff --check 全绿；文档同步 TA-06-TODO / frontend/AGENTS / 31-code-map / 07-terrain-art / TODO / changelog；TA-06-11 性能闭环未完成，本任务整体记 ✔（人工验收通过）但性能项保留 NOT_RUN
