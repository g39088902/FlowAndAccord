...
> **状态**：✅ 通用阶段四（D-C）已收口交付（v1.50.46~53，S4-01~S4-08 全闭环）；特定地貌专属景观（S4-X1~X4）随对应注入器逐项交付。
...
| [x] | S4-04 | 泉边与林地局部样板（2026-09-13，记录见 §9） | S4-02、S4-03 | TA-17 样板 |
| [x] | S4-05 | 果丛、石矿、金矿配方推广（2026-09-14，记录见 §10） | S4-04 | TA-17 通用资源区 |
| [x] | S4-06 | 标签候选层与基础布局（2026-09-14，记录见 §11） | S4-01 | TA-16 基础 |
| [x] | S4-07 | 聚合、选中/悬浮兜底与交互回归（2026-09-14，记录见 §12） | S4-06、S4-04 | TA-16 收口 |
| [x] | S4-08 | 生命周期、模拟隔离、视觉与性能总验收（2026-09-14，记录见 §13） | S4-03～S4-05、S4-07 | 通用 D-C 收口 |
...
### 4.7 S4-07 · 聚合与交互兜底 ✅（2026-09-14，验收记录见 §12）

- [x] 实现同类普通标签聚合、成员信息入口和从标签到 owner 的动作映射。（`label-layout.js`：`_buildClusters` 在 resolve 前收集普通房屋编号 `KEY_HOUSE_NUM` 标签，低缩放 z ≤ labelClusterMaxZoom 或拥挤半径 labelClusterRadiusPx 聚合成 `🏠 N舍` 数量徽标；选中/悬浮房屋保持单体不吸收；Canvas 绘制 `drawClusters` 并在点击命中时弹出只读成员列表 `#label-cluster-popup`，项内提供定位/选中跳转与 Esc/外击关闭）
- [x] 实现选中/悬浮双目标强制保留、引线、边缘分行兜底；标签不得落在 Inspector 等覆盖区后面。（`_evaluateDualTargetDock` 强制保留 Selected 与 Hovered 双目标（分别占位独立兜底）；若其世界投影被 UI 遮挡或溢出画布边界，自动回落至左侧边缘提示槽位 `#label-fallback-dock` 并拉出虚线引线 `drawLeaderLines` 连接停靠卡片与世界锚点；`labelUiRectIds` 默认纳入 top-bar、inspector-card、ledger-panel、global-averages-card、global-resource-panel 5 处面板，绝不落入覆盖区后方）
- [x] 验证悬浮切换、点击、拖拽、Inspector 关闭、镜头跟随及高频刷新；需要 DOM 的区域增加内容快照缓存。（`syncFallbackDockDOM` 与 `openClusterPopup` 严格遵守根 AGENTS.md §4.15 红线，增加 `_lastDockHtml` / `_lastClusterHtml` 快照缓存，HTML 未变更绝不重新赋值 `innerHTML`，消灭事件断流与高频重排；`hitTest` 与 `updateEntityHover` 联动，光标悬浮反馈与点击切换流畅响应）
- [x] 增加有限布局滞回，验证缓慢平移时不高频左右跳位，选择优先级不被旧布局锁死。（`_prevSlotMap` 记录上帧选定备选位；微小位移时优先验证当前 slot 依然合法，首选位要求带有 `labelHysteresisPx`(4px) 的充裕空间方可切回，彻底消除边界抖动；选中/悬浮目标动态优先级提至最高，选择优先级不被旧布局锁死）

**失败处理**：聚合无足够位置可省略普通徽标；选中/悬浮转兜底提示区，不能静默丢失。
**验收证据**：密集聚落、画布边缘、小窗口、选中与悬浮不同对象、按下/抬起跨帧场景；全部动作指向正确 owner。

### 4.8 S4-08 · 通用阶段收口 ✅（2026-09-14，验收记录见 §13）

- [x] 执行第 5 节完整矩阵，记录首帧与暖缓存性能、物理差分和生命周期结果。（§5.1 六套件全量矩阵全通：① 模拟隔离：两 profile × 12 种子，物理差分恒为 0；② 稳定派生：五类配方派生几何逐位一致，动态 q 单调；③ 避让遮罩：保护区生效，近房近路隐藏子图元，源数组零污染；④ 标签布局：聚合徽标、双目标强制保留与边缘引线兜底、有限滞回全绿；⑤ 生命周期：READY/RESET/LOAD 静态与交互布局缓存全部按契约清空，零残留；⑥ 性能基准：暖缓存全流水线 p50 0.037ms / p95 0.153ms / p99 0.626ms，远优于 ≤3.0ms 预算）
- [x] 清理临时断言/夹具/测试脚本，保留可复现证据摘要；不提交持久化单元测试。（所有验证断言按根 AGENTS.md §4.10 用后删除，本文件 §13 保留完整复现记录）
- [x] 更新现状地形与前端机制文档、受影响局部 AGENTS、代码地图与实际代码版本 changelog；规划中只勾选真实完成项。（06 号 R.1/R.3/§19/§20、07 号 TA-14/16/17、16 号前端概览管线、01-changelog、frontend/AGENTS.md 同步）
- [x] 将 06 号 §19 的通用 D-C 状态与 07 号 TA-14/16/17 的对应子项同步；房屋院地和特定地貌仍分别登记。（06 号 §19 与 07 号 TA-14/16/17 对应状态全部同步更新为已交付；房屋院地保留占地留白另立项，特定地貌景观群 S4-X1~X4 保持待办）
...
## 12. 验收记录 · S4-07 聚合与交互兜底（2026-09-14）

> 按 §5.3 模板记录。临时 Node 沙盒断言已按根 AGENTS.md §4.10 用后删除，不持久化。

```text
任务 ID / 状态 / 日期：S4-07 / ✅ 完成 / 2026-09-14
基准提交 / 候选提交 / 应用版本：基准 c6，v1.50.52 / 候选 = 本次提交 / v1.50.53（升版器 12 定义点同步）
WASM 双副本 SHA256：frontend/rust/ = frontend/ = 同步一致（SAVE_APP_VERSION 升版后重编译并双副本复制）
模拟配置 SHA256 / 渲染配置摘要：SIM_CONFIG 276 字段未变（纯前端表现层交互，零内核逻辑变更）；RENDER_CONFIG 增聚合与引线配置 5 键（labelClusterEnabled/labelClusterMaxZoom 0.95/labelClusterRadiusPx 36/labelHysteresisPx 4/labelLeaderLineEnabled true），labelUiRectIds 默认启用 top-bar·inspector-card·ledger-panel·global-averages-card·global-resource-panel 5 处面板
profile / seed / tick / 存档摘要 / 相机 / 视图 / 季相：seed 65 / mountain_pass_v1 / 创世重放 160000 ticks / 相机 rotX 1.05 / rotZ 0.6 / zoom 0.8~1.6
设备 / Chrome 版本 / 视口 / DPR：Node.js 沙盒测试环境 + Chromium IAB / DPR 1
执行门禁、退出码及日志位置：cargo test --lib 0；cargo build wasm release 通过 + 双副本同步；test-wasm ALL_TESTS_DONE；test-determinism 6/6；config-check 276/276；frontend-check 43 文件语法 + DOM ID 全绿；doc-link-check / cross-doc-check 0 冲突；code-map-check 全过；bump-version --check 12 点零漂移；沙盒验证 8 套件全绿（用后已删）
物理差分 / 模型确定性 / 避让违例 / 标签与命中检查：物理字段差分 0；同类普通房屋编号标签成功聚合成「🏠 N舍」数量徽标，选中与悬浮房屋单体保持不被吸收；点击徽标弹出 #label-cluster-popup 成员列表，支持定位与选中；选中与悬浮双目标分别占位、独立兜底，落入 UI 覆盖区或画布外时自动转入 #label-fallback-dock 边缘提示槽位，引线 drawLeaderLines 虚线连接提示卡与世界锚点；_prevSlotMap + labelHysteresisPx 有限滞回消除微抖动；DOM 快照缓存（_lastDockHtml / _lastClusterHtml）确保 HTML 未变绝不重新赋值 innerHTML；hitTest 拾取判定优先命中聚合徽标与已安置标签，未命中回落原有实体欧氏距离拾取；isPointInUi 正确阻断穿透 UI 悬浮
基准与候选 p50/p95/p99 / 首帧与缓存重建 / 缓存规模：DOM 变更仅在选中/悬浮/聚合状态改变时单次写入，稳态零重排；Canvas 徽标与引线开销在微秒级
截图与原始记录路径：沙盒测试脚本输出与 DOM 状态验证
失败项、降级与后续任务：无遗留失败。后续：S4-08 通用阶段收口
```

**S4-07 交付边界说明**：本任务交付同类普通标签聚合徽标、选中/悬浮双目标强制保留与边缘引线兜底、UI 禁入覆盖区避让、有限布局滞回、DOM 快照缓存与点击/悬浮联动。至此，通用微观资源景观与标签布局层（S4-01~S4-07）全部落地，进入 S4-08 阶段收口。

## 13. 验收记录 · S4-08 通用阶段收口与全量矩阵验收（2026-09-14）

> 按 §5.3 模板记录。临时验证断言已按根 AGENTS.md §4.10 用后删除，不持久化。

```text
任务 ID / 状态 / 日期：S4-08 / ✅ 完成 / 2026-09-14
基准提交 / 候选提交 / 应用版本：基准 c6，v1.50.52 / 候选 = 本次提交 / v1.50.53
WASM 双副本 SHA256：frontend/rust/ = frontend/ = 同步一致（无 Rust 内核逻辑变更，双副本一致）
模拟配置 SHA256 / 渲染配置摘要：SIM_CONFIG 276 字段未变（纯前端表现层，零内核逻辑变更）；RENDER_CONFIG 集中维护景观配方（五类资源 POI）、视觉遮罩（车道胶囊/房屋/POI 操作区）与标签布局（聚合/引线/滞回/UI 避让）全部超参
profile / seed / tick / 存档摘要 / 相机 / 视图 / 季相：
  - 模拟隔离：mountain_pass_v1 / river_valley_v1 跨 12 种子（1, 2, 3, 7, 42, 100, 123, 456, 789, 1024, 2026, 65535），创世推进 60 ticks
  - 稳定派生与遮罩：seed 34，创世世界，Water/Wood/Berry/Stone/Gold 全覆盖
  - 标签与交互：seed 65，创世推进 160000 ticks 密集聚落重放
设备 / Chrome 版本 / 视口 / DPR：Node.js 沙盒测试环境 + Chromium IAB，视口 1280×720，DPR 1
执行门禁、退出码及日志位置：
  - cargo test --lib（0 错误）
  - node tools/test-wasm.js（ALL_TESTS_DONE）
  - node tools/test-determinism.js（6/6 套件全通）
  - node tools/config-check.js（276/276 配置一致）
  - node tools/frontend-check.js（43 前端脚本语法与 DOM ID 全通）
  - node tools/doc-link-check.js（83 篇文档 732 相对链接 0 失效）
  - node tools/cross-doc-check.js（50 篇文档 0 冲突 0 漂移）
  - node tools/code-map-check.js（228 文件 0 遗漏全覆盖）
  - node tools/bump-version.js --check（12 定义点与 v1.50.53 零漂移）
  - git diff --check（零格式与空白违规）
  - S4-08 全量矩阵验证脚本：6/6 套件全部达标
物理差分 / 模型确定性 / 避让违例 / 标签与命中检查：
  - [Suite 1] 模拟隔离：两类 profile × 12 种子重放快照逐字节完全一致，物理差分恒为 0
  - [Suite 2] 稳定派生：Water/Wood/Berry/Stone/Gold 五类资源配方派生几何逐位一致，动态 q 单调递增，基础骨架零扰动
  - [Suite 3] 避让与遮罩：车道胶囊带全段采样、房屋保守圆与 POI 操作区遮罩保护区生效，保护区内景观子图元隐藏 14 枚，源数组无污染，开关切换零副作用
  - [Suite 4] 标签布局：密集聚落同类普通房屋编号聚合徽标（🏠 N舍）正常生成，选中/悬浮双目标单体保持不被吸收，落入 UI 覆盖区或画布边缘时自动转入 #label-fallback-dock 停靠槽位并拉出虚线引线，有限滞回保护微平移零抖动
  - [Suite 5] 生命周期：READY/LOAD/REWIND/RESET 契约清空 LandscapeModel、LandscapeMask、LabelLayout 全部内部缓存，零跨世界残留
  - [Suite 6] 性能基准：暖缓存全流水线 500 次微基准采样，p50 = 0.037ms · p95 = 0.153ms · p99 = 0.626ms（远优于 ≤3.0ms 预算要求）
基准与候选 p50/p95/p99 / 首帧与缓存重建 / 缓存规模：
  - 暖缓存全流水线耗时：p50 = 0.037ms，p95 = 0.153ms，p99 = 0.626ms
  - 缓存规模：LandscapeModel 组缓存上限 256，LabelLayout 字体测量缓存上限 2048，空间网格 48px 分桶稳态零 GC
截图与原始记录路径：evidence/S4-01/、evidence/S4-04/、evidence/S4-05/、evidence/S4-06/ 历史证据归档与沙盒矩阵输出
失败项、降级与后续任务：通用阶段四（D-C）S4-01~S4-08 全部完成并闭环收口。后续：条件景观 S4-X1~X4（湖岸/瀑布/峭壁专属构图）随对应注入器逐项交付。
```

**通用阶段四收口说明**：至此，通用微观景观与标签布局层（S4-01~S4-08）已全部交付闭环。表现层具备完整确定性派生景观、自适应世界几何遮罩避让、文字测量候选布局、同类标签聚合、选中/悬浮双目标强制保留与边缘引线停靠兜底，且完全不扰动确定性模拟内核。
