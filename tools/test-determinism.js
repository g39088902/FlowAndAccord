...
(async () => {
  ...
  for (const seed of seeds) {
    ...
    runA.worldCreate(0, 764.0, seed, 20, simConfig.countCamps, simConfig);
    runA.ex.world_tick_steps(600, dt);
    const snapA = runA.getSnapshotString();

    const runB = await createInstance();
    runB.worldCreate(0, 764.0, seed, 20, simConfig.countCamps, simConfig);
  ...
  instA.worldCreate(0, 764.0, batchSeed, 20, simConfig.countCamps, simConfig);
  for (let i = 0; i < 600; i++) instA.ex.world_tick(dt);
  const snapBatch1 = instA.getSnapshotString();

  // B: 10 步 * 60
  const instB = await createInstance();
  instB.worldCreate(0, 764.0, batchSeed, 20, simConfig.countCamps, simConfig);
  ...
  instC.worldCreate(0, 764.0, batchSeed, 20, simConfig.countCamps, simConfig);
  ...
  instD.worldCreate(0, 764.0, batchSeed, 20, simConfig.countCamps, simConfig);
  ...
  instMono.worldCreate(0, 764.0, subphaseSeed, 20, simConfig.countCamps, simConfig);
  instMono.ex.world_tick_steps(300, dt);
  const snapMono = instMono.getSnapshotString();

  const instSub = await createInstance();
  instSub.worldCreate(0, 764.0, subphaseSeed, 20, simConfig.countCamps, simConfig);
  for (let t = 0; t < 300; t++) {
    for (let p = 0; p <= 8; p++) {
  ...
  instA4.worldCreate(0, 764.0, sideEffectSeed, 20, simConfig.countCamps, simConfig);
  ...
  instB4.worldCreate(0, 764.0, sideEffectSeed, 20, simConfig.countCamps, simConfig);
  ...
  instSL.worldCreate(0, 764.0, slSeed, 20, simConfig.countCamps, simConfig);
  ...
  for (const pop of [10, 20, 50]) {
    ...
    instPopA.worldCreate(0, 764.0, popSeed, pop, simConfig.countCamps, simConfig);
    ...
    instPopB.worldCreate(0, 764.0, popSeed, pop, simConfig.countCamps, simConfig);
