 ...
 *   2. 生成 docs/current/tech/14-config-reference.md —— 一份带中文说明的参数速查表，降低用户检索与调参成本。
 *
 * 用法：
...
const OUT_MD = path.join(ROOT, 'docs', 'current', 'tech', '14-config-reference.md');

// ---------------------------------------------------------------------------
// 配置字段 → 影响模块映射 (v1.7.0 新增)
//   基于字段名前缀推断，特殊字段显式覆盖。用于 14-config-reference.md 的「影响模块」列。
//   改字段影响面时须同步更新此映射。
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 生成 docs/current/tech/14-config-reference.md
// ---------------------------------------------------------------------------
function generateReference(rs, js, errors) {
  ...
  const lines = [];
  lines.push('# Flow & Accord · 仿真超参数速查表 (14-config-reference.md)');
  lines.push('');
