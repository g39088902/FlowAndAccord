...
function factDrifts() {
  ...
  const impact = readText('docs/current/tech/29-impact-matrix.md');
  ...
  for (const [name, text] of [['29-impact-matrix.md', impact], ['spatial/AGENTS.md', spatial]]) {
    if (!text.includes(`(tick + id) % ${interval} == 0`)) drifts.push(`${name} 决策错峰与 config.js 不一致`);
  }
  for (const [name, text] of [['sim_core/AGENTS.md', core], ['frontend/AGENTS.md', frontend], ['29-impact-matrix.md', impact]]) {
  ...
  for (const re of stale) {
    for (const [name, text] of [['sim_core/AGENTS.md', core], ['frontend/AGENTS.md', frontend], ['29-impact-matrix.md', impact], ['spatial/AGENTS.md', spatial]]) {
