...
> **入口**：[文档导航](./README.md) · [四季与热力学现状](./current/03-seasons-climate.md) · [地形美术规划](./21-plan-terrain-art.md) · [前端现状](./current/07-frontend-ui.md)。
...
`docs/current/03-seasons-climate.md` 已确立「`season_year_length` 是唯一时间基准，单季长度自动派生，不得引入独立季度长度配置」。光照若自建日/季时钟，就会出现“画布上的季节”与“气温、供暖、浆果霜冻所依据的季节”两套事实——直接违反该不变量。本方案把光相定义成季节时钟的**纯函数**，不存在独立相位。
...
**边界**：这里只调“光”，不调地表反照率（草绿/枯黄/积雪属 `21-plan-terrain-art.md` S1-4，见 §10）。
...
**预算**：新增渲染耗时 p95 增量 ≤ 3 ms；同设备仿真吞吐下降 ≤ 5%（对齐 `21-plan-terrain-art.md` §8.2）。
...
**门禁联动**：新文件须登记进 `index.html` 脚本序列、`frontend/AGENTS.md` 文件清单与加载顺序、`docs/current/09-code-map.md`（否则 `code-map-check.js` 报“实际有但文档未登记”）。`config-check.js` 无需改动（不并入 `SIM_CONFIG`）。
...
| 文档 | `frontend/AGENTS.md`（文件清单/加载顺序/DOM 契约）、`docs/current/07-frontend-ui.md`、`09-code-map.md`、`21-ui-page-overview.md`、`11-changelog.md`、`docs/21-plan-terrain-art.md`（S1-4 对接） | — |

`render_world.js` 现约 770 行，逼近 800 行上限：本方案落地时**先把 `drawTerrain`/`drawTerrainFeatures` 拆到 `render_terrain.js`**（`21-plan-terrain-art.md` §9 已列为建议前置），再改光照。
...
> ⚠️ 升版会让 `SAVE_APP_VERSION` 变化，按 v1.37.1 的设计**旧存档自动作废**——这是本项目既有规则，不是本方案引入的副作用（`docs/current/15-save-load.md`）。
