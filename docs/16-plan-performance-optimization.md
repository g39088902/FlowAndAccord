...
### 里程碑 M2：道路衰减稀疏活跃边集合与降频（攻克 30.7% 算力热点）

- **优先级**：P1（高收益，轻量改动）
- **涉及文件**：`crates/sim_core/src/spatial/graph.rs`、`crates/sim_core/src/spatial/agent.rs`

#### 核心痛点与方案：
1. **活跃磨损边稀疏集合（Sparse Active Wear Set）**：
   - **现状**：每拍无条件遍历路网全部边 `self.graph.edge_weights_mut()`。但全图数百条边中，绝大多数荒野小径的 `wear` 长期处于 `0.0`。
   - **优化**：在 `LaneGraph3D` 中维护确定性集合 `active_wear_edges: BTreeSet<EdgeIndex>`：
     - 当族人通行踩踏（`agent.rs`）导致 `wear > 0.0` 时，将该边索引加入 `active_wear_edges`；
     - 衰减扫描仅遍历 `active_wear_edges` 中的边；
     - 当某条边的 `wear` 衰减归零时，移出活跃集合。
2. **衰减慢周期降频批量折算**：
   - **现状**：道路衰减率为 0.5%/小时，单 tick（1/60 小时）的衰减量微乎其微。
   - **优化**：将衰减更新降频至每 $K=10$ 或 $K=30$ tick 执行一次，衰减公式按复利精确折算：`wear * (1.0 - rate * K * dt)`。

- **预期收益**：
  - Phase 4 耗时压缩 75%~85%（从 3.07µs 降至 < 0.6µs）；
  - 单核纯内核吞吐量从 **67,000 TPS 提升至 82,000+ TPS**。
- **确定性风险**：极低（使用 `BTreeSet` 保证边遍历顺序确定；降频需保持同种子固定周期触发）。
...
| **M2: 道路衰减** | 1 天 | `sim_core/graph.rs` | Phase 4 耗时从 30.7% 降至 <8%；确定性全通 | **82,000 ~ 88,000 TPS** (+25%) |
