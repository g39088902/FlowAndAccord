...
- [地图模板总纲：地图模板、地形要素与实现技术方案（T0/T1/T2/D-A 已落地，D-B/D-C 与 T4 待办）](./22-plan-terrain-features.md)：以 14 种地图模板为核心（山口聚落/两岸河谷已落地，其余见模板总表），八类基础地形为拼装词汇，地表查询、合法走廊、水系资源池、装饰系统、快照、存档与 Canvas 均为支撑模板落地的共用基座。
...
- [Agent 神经内分泌调制层：四轴十一激素系统（设计方案，未排期）](./30-plan-agent-hormone-system.md)：奖赏-情绪轴 D.O.S.E + 应激轴三激素 + 生殖轴（雄/雌/孕：求偶竞争、受孕窗口、妊娠安稳、产后崩落、更年期）+ 代谢轴（甲状腺素 BMR 总闸门、代谢气质遗传、饥荒自适应节流）；多巴胺阈值漂移（淘金成瘾）、冬季血清素低落（SAD）、皮质醇慢性折寿与应激禁孕、催产素信任联结（29 号关系的生理载体）；纯确定性、效果开关关闭逐字节等价。
- [人际好感度系统（设计方案，未排期）](./31-plan-affinity-system.md)：每人 36 条、取值 -7~+7 的有限容量态度表（16 档等级命名）、陌生人默认 0、溢出驱逐 \|值\| 最小者的确定性规则、事件源清单、决策只读调制、存档与 FABS 快照契约及 P0~P3 落地建议。
