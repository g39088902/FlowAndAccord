# 26. CI/CD 自动部署指南（GitHub Actions → 腾讯云 COS）
...
本状态机描述「一次部署运行」的状态迁移：从 push `master` 后排队，经 WASM 构建、双副本同步、确定性/跨文档/文档链接三类门禁，再到预检 Secrets 与 COS 上传，最终成功或终止。边界是门禁不过绝不部署（P3）。
...
stateDiagram-v2
    ...
    D --> E : test-wasm.js + cross-doc-check.js + doc-link-check.js 全绿
...
| D | 门禁 | 双副本就绪 | `test-wasm.js`、`cross-doc-check.js` 与 `doc-link-check.js` 跑完 |
| E | 部署上传 | 三类门禁全绿，Secrets 预检通过 | COS 上传与 MIME 覆写完成，输出摘要 |
...
graph TD
    ...
    E1 --> E2["node tools/doc-link-check.js 文档链接可达性门禁"]
    E2 -->|全绿| F["upload-artifact frontend/"]
...
| 门禁 | `node tools/test-wasm.js`（确定性/越界/NaN/长程）+ `node tools/cross-doc-check.js`（跨文档事实指纹）+ `node tools/doc-link-check.js`（Markdown 相对链接可达性，★ 2026-09-12 新增），任一不通过则不上线 |
