...
- 默认端口 `3003`，内置 `.wasm` MIME（`application/wasm`）
- **若 3003 已被占用，说明用户已手动启动服务，Agent 不要重复启动**——直接访问 `http://localhost:3003`
...
| 端口 3003 占用 | 用户已启动服务，直接访问 `http://localhost:3003`；不要重复 `node frontend/server.js` |
...
启动本地 HTTP 服务（默认端口 3003）：
```bash
node frontend/server.js
```
访问浏览器：`http://localhost:3003`

> 若 3003 端口已被占用，说明前端服务已在运行，直接访问即可，无需重复启动。
