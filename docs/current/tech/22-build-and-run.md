...
> ⚠️ CI 中**禁止**使用便携链——CI 运行在 ubuntu-latest，用标准 rustup（见 [./26-cicd.md](./26-cicd.md)）。`.toolchain/` 与 `.cargo-home/` 已被 gitignore。

### 1.2 ★ 宿主 linker 环境（MSVC）：Git Bash 下必配 `LIB` 与 linker 优先级
...
通过时输出字段数对比，并自动刷新 `./05-config-reference.md`（参数速查表，**勿手改**）。
...
- 默认端口 `3004`，内置 `.wasm` MIME（`application/wasm`）
- **若 3004 已被占用，说明用户已手动启动服务，Agent 不要重复启动**——直接访问 `http://localhost:3004`
...
| `failed to get petgraph / unable to update registry crates-io` | 网络被代理/沙箱阻断。改走离线 vendor 源（§1.3）+ `--offline` |
| `link: extra operand '...rcgu.o'` | Git Bash 的 coreutils `link` 抢先；把 MSVC `bin/Hostx64/x64` 置于 `PATH` 最前（§1.2） |
| `LNK1181: 无法打开输入文件“kernel32.lib”` | `LIB` 未含 MSVC / Windows SDK 的 x64 库目录（§1.2） |
...
| 端口 3004 占用 | 用户已启动服务，直接访问 `http://localhost:3004`；不要重复 `node frontend/server.js` |
...
启动本地 HTTP 服务（默认端口 3004）：
```bash
node frontend/server.js
```
访问浏览器：`http://localhost:3004`

> 若 3004 端口已被占用，说明前端服务已在运行，直接访问即可，无需重复启动。
...
node tools/config-check.js
```
输出「字段集、类型、默认值完全一致，无漂移」即代表前后端配置同步。同时自动刷新 `./05-config-reference.md`。
