...
> **UGC-01 实施状态（2026-09-24）**：新增 `geo::procedural` Field IR、确定性二维字段算子、`grassland_plain_v1`/`mountain_pass_v1` 数据配方、约束报告和 `geo::backend::HeightfieldBackend`/稀疏 Voxel 接口。旧 `TerrainMap` 生成器仍是默认兼容路径；新编译器通过 `TerrainGenerator::compile_procedural` 使用，后续波次再迁移水文和体素静态重建。

> **UGC-02 实施状态（2026-09-25）**：`geo::procedural::processes` 已提供固定 D8 汇流（含平地稳定 tie-break）、热松弛、液压侵蚀/沉积和 priority-flood 水位求解；`procedural::hydrology` 将河道连通分量、岸带、静态水体 ID 和深水/河岸语义投影到 `SemanticGrid`。这些过程只由 recipe 水文参数驱动，旧 `TerrainMap` 默认生成路径保持不变。

> **UGC-03 实施状态（2026-09-25）**：已注册河谷、台地、盆地、冲积扇、火山湖、半坡以及前两波的草原/山口共八个静态 recipe。迁移校正后，六个 profile 进入 Field Compiler；冲积扇和盆地暂时保留旧 `FanGeometry`/`BasinGeometry` 专用生成路径，因为通用 Cone/Depression recipe 尚未表达扇形干沟、盆地出口谷地和模板锚点。`random` 仍使用原有 seed→profile 分区，选中这两个 profile 时同样走专用路径；其余 profile 通过 `HeightfieldBackend` 投影到 `TerrainMap`。编译失败、flat baseline、未知配方或降级覆盖开启时回退旧生成器。`plateau_v1` 和 `volcanic_lake_v1` 的 Field Compiler 结果带有后端标记，路网不再套用只适用于旧专用几何的台地/湖泊门禁。`TERRAIN_GENERATOR_VERSION` 已推进到 25；旧版本存档按既有门禁拒绝。适配器复制新高程、坡度、地表、水体 ID 和 flags，并补回 TerrainAccent；保留旧特征 ID/水文多边形协议不伪造；生态布局在没有旧水文多边形但检测到新深水格时，仍建立共享水池并绑定水源 POI；`compare_recipe_to_legacy` 提供高程最大误差、地表差异和水体 ID 差异报告。
...
1. ★ 世界统一深度队列 drawWorldEntities()  按 depth = ry·sinX + z·cosX 升序（远 → 近）
   ...
   └─ ★ 装饰实体（WORLD_ENTITY_ACCENT：Bush / Boulder / Tree，v1.50.2 起并入；sink 硬门槛进 GL 装饰层）
2. 调试网格线（普通视图隐藏，G 键切换）      drawTerrainGrid()
```
