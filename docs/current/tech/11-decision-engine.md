...
> **模块索引**：[← 返回 ../README.md 全景索引](../README.md) · 主要源码：`crates/sim_core/src/spatial/decisions/`（模块地图见局部 AGENTS）· 深度拆解见 [`./11-decision-engine.md`](./11-decision-engine.md)
...
> 上篇回答「机制是什么」，本篇回答「为什么这样设计」。原为独立文档 `./11-decision-engine.md`，已内聚至此。
...
> **本文定位**：**设计思路深度解析（为什么这么设计）**，与 [`./11-decision-engine.md`](./11-decision-engine.md) 形成互补——06 讲"机制是什么"，本文讲"为什么这么设计"。
> **版本**：v1.3.2
...
根 AGENTS.md §4.11 明确：系统只当"物理规则执行者"，一切"盖不盖、何时盖、在哪盖"来自 agent 自己的 `arbitrate_sustained_task` 输出。三条自主触发链路（立宅/升级/修缮）均为确定性触发，详细门槛见 [./11-decision-engine.md](./11-decision-engine.md)。
...
| 维度 | 当前实现 | 愿景（[../../plan/design/01-roadmap.md](../../plan/design/01-roadmap.md) M6/M7/M8） |
| :--- | :--- | :--- |
