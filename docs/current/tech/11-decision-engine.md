...
- 详见 `spatial/AGENTS.md` §二（tick 顺序与不变量）与根 AGENTS.md §4.3。
...
当前 Agent 的"AI"是**纯确定性规则系统**——层次化动机有限状态机 (FSM) + 加权 A* 寻路 + 踩踏拓路涌现 (Stigmergy) + 生理/家庭/房屋生命周期闭环。**不包含任何 LLM / 神经网络 / 学习成分**。
