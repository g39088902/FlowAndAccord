...
> **现状**：三层解耦已全量落地，`ActiveTask` 控制器是持续任务单一真相源。Inspector 决策行动中枢三栏看板、分级任务抢占与平滑改道、先天禀赋与家资个性化选策、多品类采收预排队列与 TSP 行程优化均在线。
> **本文构成**：前半为架构机制（三层职责、持续任务生命周期、连续采收、状态所有权、外部契约），后半（H2「技术规格」起）为类型、分支映射、tick 时序与快照契约的详细权威定义。
> **关联**：[决策引擎](./11-decision-engine.md)（马斯洛层与 16 条分支） · [核心系统状态机全景](./02-core-systems-fsm.md) · [决策局部规则](../../../crates/sim_core/src/spatial/decisions/AGENTS.md)

---

## 1. 为什么三层解耦

旧架构中 `Need { level, kind, target_state }` 同时描述需求与动作，`seeking.rs` / `harvest.rs` / `market.rs` 分别处理选点、断流、返航，修改一条兜底规则常要改多个入口。三层解耦后，需求仲裁（L1）、策略选择（L2）、执行适配（L3）各有明确所有者与反馈接口，而不是删除跨模块调用：

- **瞬发通道**（近距成婚、门前受孕、竞拍出价等）只写既有 pending，不占持续任务槽位、不改运动状态、不扣资源、不消费 RNG；
- **持续任务**由 `ActiveTask` 控制器统一安装/推进/退出，`agent.state` 降为只读投影；
- **连续采收**保留原动态再评估语义，在采收完成/断流的原时点请求 L1 重选，不在 L1/L2 之间无限互调。

立宅 RNG 在 `fulfill_resting_need` 内消费，并非分支条件函数直接掷点。
...
L2 在采收完成/断流的原时点请求 L1 的"连续采收仲裁入口"；该入口仍按配置顺序检查现有允许分支（b1/b2/b5/b6/b7/b9/b10）。每次转换必须重新检查私宅有效性、家户补货触发器、自身生理、各品类独立行囊容量、POI 私有可用性与体力。金币保留无限容量、单趟采集目标和两类冷却的区别。回到家门只表示抵达，卸货仍按速率逐 tick 入家户账本。

**多品类预排队列与 TSP 行程**：`Agent3D::harvest_queue: [Option<BranchId>; 4]`（零堆分配、`#[serde(default)]` 兼容读旧档）在出发时由 `harvest.rs::plan_harvest_itinerary` 按最近邻贪心 TSP 规划 1~4 站顺路链路。采满现场转站优先消费预排队列，候选失效自动跳过并回退 `branch_order` 兜底；任务抢占、回宅卸货、体力耗尽（<15%）与死亡时清空队列。预排行程经 `ActiveTaskSnapshot.itinerary` 透传 Inspector，以"🗺️ 预排行程"微光条呈现。

## 5. 状态所有权与运行时频率

### 5.1 状态所有权现状

`PrimitiveActionState` 被代谢、运动到达、POI 交互、房屋结算和快照共同读写，不是单纯 UI 字段。三层解耦完成后，所有权划分如下：

- **持续任务真相源**：`ActiveTask` 控制器，仅 `transition.rs` 可修改；
- **位置 / 车道 / 路线进度**：仍归既有运动系统，规划器只发命令不复制第二条路线；
- **pending 与世界结算**：仍归世界系统（生态 / 房屋 / 账本 / 生命周期）；
- **`state`**：执行兼容只读投影，由 `transition.rs` 在动作安装、到达、提交消费和生命周期事件时同步，其他模块不得自由覆写；投影只读、不调用导航、不清车道、不消费 RNG。

死亡和胎儿生命周期优先于任务投影；返家阶段必须投影为返家；到达/离路/结算结果不能等到下一个 120 tick 相位才同步。具体映射与合法组合见下文技术规格 §6。
...
### 7.1 Inspector 决策行动中枢三栏看板

Inspector 角色卡片顶部的「🧠 决策行动中枢」自适应卡片分三栏：
...
⓪ 瞬发高亮微光 Pill：加冕结算（`coronation_pending`）、门前受孕（`raise_child_pending`）、就近求偶（`courtship_pending`）、房屋竞拍（`pending_bids`）、立宅选址（`pending_house_pos`）等待结算事件以动态微光胶囊呈现在中枢顶栏。`AgentSnapshot.active_task: Option<ActiveTaskSnapshot>` 经快照四处同步链路下发。

### 7.2 分级任务抢占矩阵

`crates/sim_core/src/spatial/decisions/preemption.rs` 按分级裁决规则平滑中断常规任务：

| 当前任务 | 可被谁抢占 |
|---|---|
| 娱乐淘金（`GoldWealth`） | 任何生理需求（饱食/饮水/休养）、安全补料、私宅修缮、远征登基 |
| 建材采收（`StockStone` / `StockWood`） | 临界饥渴、冬季暴雪断柴（木材 < 10）、危房耐久告警（耐久 < 20%） |
| 远征登基（`SeekThrone`） | 仅濒死饥渴危象 |
| 施工修缮（`ConstructUpgrade` / `RepairHouse`） | 求生、王位空缺加冕（施工进度在房屋实体冻结保全，不回滚） |

抢占由 `transition::preempt_task` 统一调度，沿原车道连续反向掉头（U-turn），严禁瞬移；已有采获行囊保全。

### 7.3 先天禀赋与家资个性化选策

- **力量 (Strength)**：高力量（≥110）野外重体力采收与伐木采石装载速率 +25%；低力量（≤90）装载速率 -15%，倾向轻体力活动，且房屋耐久低于 90% 即主动修缮（常规族人 80%）。
- **智力 (Intelligence)**：高智力（≥110）具备市场牌价感知，综合权衡路途耗时与榷场现货牌价，野外过远或市场更近时直接赴榷场；寻路偏好高等级通衢主干道，规避低限速泥泞小径。
- **家户财富阶层**：豪绅家户（金币 ≥ `market_wealthy_family_gold`，默认 200）户主面临家庭物资缺口时 80% 几率直接赴榷场现货采购；平民家户保持自力更生。
...
本表不替代现有分支的全部条件函数；改分支时须以源码条件函数为准，不能按表中摘要重写守卫。表中状态表示兼容状态，不意味着所有状态都物理静止。
...
以下记录 `world_tick.rs::tick()` / `tick_subphase()` 的实际调用顺序。
...
`state` 写入、`enter_stationary_state`、路线到达、死亡、房产/婚姻/政体变化和 pending 消费点，分布在 `agent.rs`、`decisions/`、`ecology/`、`housing_system/`、`world_tick.rs`、`ledger/`、初始化/出生及存档。三层解耦完成后：

- 任务修改只走 `transition.rs`，其他模块不得直接改写持续任务控制字段；
- 兼容 `state` 在动作安装、到达、提交消费和生命周期事件时立即同步，快照读取无副作用；
- 物理结果入口（运动到达、POI 交互、房屋结算、账本变化）通过 `transition.rs` 反馈，不另起旁路指挥。
...
WASM 签名、FABS 布局/码位、现有 `current_need`、目标字段和派生统计保持稳定。`state` 相同不代表远征统计、需求标签等其他字段也相同，须完整比较。新增字段时同步 `snapshot.rs`、`world_snapshot.rs`、`snapshot_bin/encode.rs`、`snapshot-bin.js`、`rustworld.js` 及消费方（快照四处同步硬约束，见根 AGENTS §4.5）。

### 7.2 持久化与恢复

`SAVE_FORMAT_VERSION` 当前为 7（含 `ActiveTask` 完整持久化）。应用版本按统一升版器更新并同步 WASM 双副本，旧应用版本存档仍拒绝加载。

必须保存：ActiveTask（含用途、目标、阶段、原语、`itinerary`）、所有无法从世界事实唯一重建且会影响下一步的结果、既有 pending、路线/车道进度、RNG、私有触发器和冷却。派生兼容视图可以重建，但只能从完整控制状态纯计算，不能重新选目标、重新消费 RNG、重复出价或重放已结算提交。
...
新增控制对象采用定长结构/枚举，不引入 `Box<dyn Strategy>`、按 Agent 每相位分配的策略列表或不受限队列。既有上下文、路径和竞拍集合存在分配，不能把"新控制对象不分配"写成全引擎零分配。性能以 `tools/profile-benchmark.js` 配对相对基线衡量（同机器、同 seed/config/tick/快照频率，预热后取中位数）。

## 8. 改动决策层时的回归场景清单

改决策层代码时，除 `test-wasm.js` / `test-determinism.js` / `snapshot-check.js` 等长期门禁外，应逐项核对以下场景：
...
| 状态同步与导航 | 车道中改道、节点派发、返家到达、离路、死亡、立宅定位例外——状态/路线/任务组合合法，原到达时点同步；辅助诊断用 `node tools/diagnose.js --check all` |
...
临时差分脚本按根 AGENTS §4.10 验证后删除，不提交持久化单元测试。

---

## 9. 意图观察 API 与领域词汇

> 源码：`crates/sim_core/src/spatial/decisions/{intent,strategy,primitive,observation}.rs`
>
> 本节描述 `observation.rs` 提供的两个只读观察入口，供 Inspector 三栏看板与外部诊断使用；它们不驱动物理。
...
`ActiveTask` 控制器挂载于 `Agent3D`，作为进行中持续任务单一真相源，通过 `transition.rs` 统一管理生命周期与兼容视图同步；存档格式版本 `SAVE_FORMAT_VERSION` 为 7，完整持久化持续任务，读档零重新规划、零重新掷点。

当前决策系统平铺于 `crates/sim_core/src/spatial/decisions/`，L1 的 16 条活动意图分支、瞬发通道与连续采收候选仲裁解耦；L2 在水粮木意图下选择野外采集或采购，L3 原语派发收口。
