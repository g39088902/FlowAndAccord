# 24. Flow & Accord · 确定性无头诊断与 Bug 排查使用指南
...
无头诊断从「世界创建 → 无头推进到目标 tick → 异常嗅探 → 结论」流转，命中告警则微观下钻复现锁定嫌疑对象。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> A
    A --> B : diagnose.js 无头 JIT 推进至 --tick T
    B --> C : 每 --sample 步嗅探 Rule 1/3/5/6
    C --> E : 命中告警，追加 --agent/--house 下钻
    E --> B : 调整 --trace-window 重推进采样
    C --> D : 无异常或采样完成
    B --> D : 到达 --tick 终态导出 JSON/报告
    D --> [*] : 输出报告，进入五步 SOP 修复
```

| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| A 世界创建 | `diagnose.js --seed S` 加载 `sim_wasm.wasm` 建世界 | 命令行调用 | 开始无头推进 |
| B 推进到目标 tick | `world_tick` 无头推进至 `--tick` 截止 | 世界就绪 | 到达终态/嗅探 |
| C 嗅探异常 | Rule 1 生理险境死锁 / Rule 3 建房停滞 / Rule 5 移动阻断 / Rule 6 死亡潮 | 每 --sample 步 | 命中下钻/无异常 |
| E 微观下钻 | `--agent/--house --trace-window` 采集中枢生理与决策时序 | 命中告警 | 重推进采样 |
| D 结论/报告 | 导出 `--export-json`/`--export-report`，告警或正常 | 推进完成 | 进入修复 SOP |

**不变量**（违反即出 bug）：
- 强确定性铁律：同种子 + 同配置输入 → 逐字节快照一致，诊断轨迹与生产环境完全相同。
- 构造前置条件阈值必须从 `SIM_CONFIG` 读取（如 `carryCapacityResource`=100.0），严禁写死常量。
- 修复后必须双副本同步 + 同参数靶向回归 + `test-wasm.js`/`config-check.js` 门禁校验双绿。
...
2. **门禁回归**：
   ...
   ```
3. **版本管理**：更新版本号（`index.html` 与 `AGENTS.md`），在 `docs/current/01-changelog.md` 记录修改。
