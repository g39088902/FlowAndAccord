...
> **入口**：[文档导航](../../README.md) · [四季与热力学现状](./41-seasons-climate.md) · [地形美术规划](../../plan/tech/26-terrain-art.md) · [前端现状](./50-frontend-overview.md)。
...
- 图层铁律：天空背景与大气色洗只落在 §3-L4 两个固定插入点；阴影是贴地图元；立体实体仍走 `drawWorldEntities()` 统一深度队列。

## 0. 口径确认：什么叫“运动周期频率与四季一致”
...
| 昼夜 + 年调制 | 光在一“日”内东升西落，年周期再调制高度/时长 | ✗（见 §0.3） |

内核的“一年”已经是 240 模拟秒（`seasonYearLength`），因此本方案的周期**直接复用既有季节时钟，不新增任何时间基准**——这正是“周期频率与四季一致”的工程含义。

### 0.2 为什么不自己做一套时间

`./41-seasons-climate.md` 已确立「`season_year_length` 是唯一时间基准，单季长度自动派生，不得引入独立季度长度配置」。光照若自建日/季时钟，就会出现“画布上的季节”与“气温、供暖、浆果霜冻所依据的季节”两套事实——直接违反该不变量。本方案把光相定义成季节时钟的**纯函数**，不存在独立相位。

### 0.3 如果日后要昼夜（变体口径）

当前内核**没有“日”这一层**（无日钟、无昼夜行为）。若将来要昼夜，必须先在 `SimConfig` 增加日长字段并进存档，属另一条技术路线。本方案为此预留的唯一接口是 §3.1 的**相位函数 `phaseFromSnapshot()`**：把昼夜叠上去只需改这一个函数，§4 的全部渲染层不动。**本期不实现。**
...
| 位置 | 现状 | 对“运动光照”的阻碍 |
|---|---|---|
| `math.js::computeElevationColor` L57–66 | 硬编码 `L = normalize(-0.45,-0.60,0.66)`（**西北 41°**，见 §3.2 轴向约定），`k = clamp((0.54+0.46·N·L)·ao, 0.52, 1.22)` | 光向是常量，无法运动 |
...
**关键结论：本方案可以做到零 Rust / 零 WASM / 零快照 / 零存档改动。** 唯一缺口是 `rustworld.js` 补一行 `this.seasonProgress = snap.season_progress ?? 0`（§4 L0-1）。这也意味着**不触发 `SAVE_FORMAT_VERSION` 变更**，旧档不作废（但见 §8 的版本号连带影响）。
...
**边界**：这里只调“光”，不调地表反照率（草绿/枯黄/积雪属 `../../plan/tech/26-terrain-art.md` S1-4，见 §10）。

### 2.6 阴影向量（世界空间，不是屏幕空间）
...
- **读档 / 重置 / 时光倒流**：时间可能倒退，限速器会“倒着走”。因此在这三处**立即 `resync()`**（§4 L0-3），不做平滑。

---

## 3. 渲染分层实施

### L0 · 光相内核（新增 `frontend/js/lighting.js`，约 260 行）
...
- `drawHouse` 落影椭圆：偏移改用 §2.6 的 `screenDelta`，长度随 `shadowLen` 缩放，透明度 `opacity = shadowOpacityBase + shadowOpacityGain·(1-sin e)`（低日头更浓）。
...
- `render_hud.js` 调试监视器新增 `#dbg-light-ms`（重着色耗时）与 `#dbg-light-phase`（u 与档位），供 §8 性能验收取证；
...
**预算**：新增渲染耗时 p95 增量 ≤ 3 ms；同设备仿真吞吐下降 ≤ 5%（对齐 `../../plan/tech/26-terrain-art.md` §8.2）。
...
| 相机旋转/缩放 | **不重算**（阴影按 §2.6 每帧投影，颜色与相机无关） |
| 主题切换 | 只影响 UI 与可选氛围亮度 |
...
1. **零内核改动**：不动 Rust、不动 WASM、不动快照字段、不动存档结构（`SAVE_FORMAT_VERSION` 保持 7）。唯一碰 Rust 的连带影响是版本号自增会改 `world_save.rs::SAVE_APP_VERSION`（§8）。
...
4. **图层铁律**：天空背景与大气色洗只能落在 §3-L4 的两个固定插入点；阴影是贴地图元；立体实体仍走 `drawWorldEntities()` 统一深度队列（`frontend/AGENTS.md` §5.9）。
...
## 6. 配置项（新增 `frontend/js/config.lighting.js`）
...
**门禁联动**：新文件须登记进 `index.html` 脚本序列、`frontend/AGENTS.md` 文件清单与加载顺序、`./90-code-map.md`（否则 `code-map-check.js` 报“实际有但文档未登记”）。`config-check.js` 无需改动（不并入 `SIM_CONFIG`）。
...
| 文档 | `frontend/AGENTS.md`（文件清单/加载顺序/DOM 契约）、`./50-frontend-overview.md`、`./90-code-map.md`、`./53-ui-implementation.md`、`../11-changelog.md`、`../../plan/tech/26-terrain-art.md`（S1-4 对接） | — |

`render_world.js` 现约 770 行，逼近 800 行上限：本方案落地时**先把 `drawTerrain`/`drawTerrainFeatures` 拆到 `render_terrain.js`**（`../../plan/tech/26-terrain-art.md` §9 已列为建议前置），再改光照。
...
> ⚠️ 升版会让 `SAVE_APP_VERSION` 变化，按 v1.37.1 的设计**旧存档自动作废**——这是本项目既有规则，不是本方案引入的副作用（`./15-snapshot-and-save.md`）。
...
§7 全部命令通过；`test-wasm` 输出 `ALL_TESTS_DONE`；`bump-version --check` 零漂移；文档体检与跨文档检查无新增问题。
...
| 方案 | 边界 |
|---|---|
| [../../plan/tech/26-terrain-art.md](../../plan/tech/26-terrain-art.md) S1-4 / M3 | 该方案负责**地表反照率**（草绿→枯黄→积雪、纹理、素材）；本方案负责**光**（方向/强度/色温/阴影/氛围）。两侧**共用** `SimLighting.phase()` 与同一套 `cell.color` 失效机制，**严禁各自维护一套季节相位**。本方案先落地可让 S1-4 直接消费光相。 |
| [./41-seasons-climate.md](./41-seasons-climate.md) | 只提供季节/气温事实；本方案不新增时间基准、不修改 `tick_season`。 |
| [../../plan/tech/25-terrain-templates.md](../../plan/tech/25-terrain-templates.md) | 地表类别与法线来源不变；本方案只消费既有 `dzdx/dzdy`。 |
| [../../plan/tech/27-performance.md](../../plan/tech/27-performance.md) | 本方案的重着色成本需纳入其渲染侧基准；若未来网格分辨率提升，须重估 §4.1。 |
| [./50-frontend-overview.md](./50-frontend-overview.md) | 渲染管线新增两个氛围插入点，需同步文档（不动实体深度队列）。 |

---

## 11. 待确认决策点

1. **口径**：确认采用 §0.1 的“年周期光弧”（一年一圈、四季四象限）。若要昼夜+年调制，需先在内核引入“日”，属另一条路线。
