...
- 改动快照字段须跑 `snapshot-check.js` + `test-wasm.js` / `test-determinism.js` 回归（JSON 对拍门禁已于 v1.50.35 随 JSON 快照通道移除）。
...
- ⚠️ **★ M4 起真正的不变量是「四处同步」**：本工具只做静态登记核对；二进制编码/解码的回归由
  `test-wasm.js` / `test-determinism.js` 兜底（JSON 对拍门禁已于 v1.50.35 随 JSON 快照通道移除）。
  改动快照字段后**都要跑**。
- **常用命令**：
  ```bash
...
- **扫描范围**：根 AGENTS.md + 各局部 AGENTS.md + `docs/` 全部 markdown；有意排除 `../01-changelog.md`（历史记录）与 `*-plan-*` / `*-spec-*`（规划态）。
- **常用命令**：
  ```bash
...
- **扫描范围**：全仓 `*.md`（跳过构建产物与缓存目录）；同时跳过 `http(s)://` / `mailto:` / 纯 `#锚点` / `/` 开头的示例占位符。
