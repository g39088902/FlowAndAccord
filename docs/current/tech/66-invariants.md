...
本状态机描述「改动落地的合规校验」：提出改动后，沿确定性 / 数据一致性 / 行为语义 / 构建部署 / 代码组织 / 前端 DOM 六类不变量逐类自检，全过则可提交，任一红线违反即回退修复。六类不变量作为转移守卫写进条件（见文末快速自检清单）。
...
stateDiagram-v2
    ...
    B --> F : 确定性 / 数据一致性任一红线违反
    C --> F : 行为语义任一红线违反
    D --> F : 构建部署 / 代码组织 / 前端 DOM 任一红线违反
    F --> A : 修复后重新提改
```

| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| A | 提出改动 | 打算修改代码/配置/前端 | 启动六类自检 |
| B | 内核与数据自检 | 改动已对照清单 | D 类与 C 类不变量全过 |
| C | 行为语义自检 | D/C 类通过 | B 类不变量全过 |
| D | 工程与前端自检 | B 类通过 | P/O/F 类不变量全过 |
| E | 通过提交 | 六类全过 | 升版 + changelog 后进入终态 |
| F | 命中红线回退 | 任一不变量违反 | 修复后回到 A 重检 |
...
| D5 | **严禁修改 `config.simulationDt`**（1/60 游戏小时） | §4.3 | 数值积分发散，所有时间相关参数失效 |
...
| B14 | **外部市场隔离与单向流失（v1.13.0，v1.27.0 扩展）**：榷场互市不进入 `NodePool`，不设公地施密特触发器，由 B15 专用派发；★ v1.27.0 起水/粮采集断流时家户户主（账本黄金 ≥ `market_min_family_gold` 且体力达标）可由 `try_route_to_market` 直接改道榷场——仍是**家户账本远程结算付费**，不改变市场支付与黄金单向扣入 `LedgerRef::Void` 的通缩闭环；到达后先濒危自救再装袋购入 | ./22-market-pricing.md | 族人蹭吃蹭喝破坏公地平衡或黄金通缩机制失效 |
| B15 | **决策分支数组定长联动（18分支）**：内核 `BranchId::ALL`、`resolve_order`、`seen` 与前端 `DEFAULT_ORDER`、`VALID_BRANCH_ID` 严格定长联动 | §4.14 / ./22-market-pricing.md | 决策分支越界、反序列化 panic 或写盘校验失败 |
