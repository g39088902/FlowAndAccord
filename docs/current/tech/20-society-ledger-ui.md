# 20. 🏛️ 制度大盘 UI 实现（M1~M4）(`ledger-ui.js`)

> **模块索引**：[← 返回 ../README.md 全景索引](../README.md) · 主要源码：`frontend/js/ledger-ui.js` · 内核机制：[./07-ledger-and-polity.md](./07-ledger-and-polity.md)
> **关联文档**：[./19-ui-implementation.md](./19-ui-implementation.md)（UI 页面全景剖析）· [./21-frontend-dev-guide.md](./21-frontend-dev-guide.md)（前端开发实施指南）
> **里程碑状态**：M1~M4 已完整落地（M1 v1.0.0 / M2 v1.1.0 / M3 v1.2.0 / M4 v1.3.0）；M5 帝国上层政体（v1.44.7）见 [./07-ledger-and-polity.md](./07-ledger-and-polity.md)。
...
## 1. 多标签页制度枢纽 (Tabbed Society & Ledger Hub)
...
## 2. M2 界面实现：家庭旁路记账 + 分家抽资 + 丧父继承清算

### 2.1 旁路双环记账流水穿透抽屉 (Double-Entry Journal Viewer)
...
## 4. M4 界面实现：地区政体 + 国王尊号 + 夺位远征 + 长子继承

### 4.1 地区/王国政体面板 (`tab-region`)
- **4 大行政区卡片**（对应 4 处营地）：
...
## 5. 高保真 ASCII Wireframe 与视觉原型图

### 5.1 制度枢纽 4 标签页大盘布局 (Society & Ledger Hub)
