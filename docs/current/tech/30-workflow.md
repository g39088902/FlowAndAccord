...
stateDiagram-v2
    ...
    D --> E : bump-version --patch + 01-changelog.md + 最终 diff 审阅
    F --> A : 修复后重走工作区检查
    E --> [*] : 提交完成
```
...
先读根 `AGENTS.md` §4。快照主链路已采用 M4 二进制：字段变更还必须检查 `snapshot_bin/encode.rs`、`snapshot-bin.js`，并跑 `node tools/snapshot-check.js` + `test-wasm.js` / `test-determinism.js`（JSON 对拍门禁已于 v1.50.35 随 JSON 快照通道移除）。快照赋值位于 `world_snapshot.rs`。
...
| 仅文档 / AGENTS.md 内容 | 根 `AGENTS.md` §4.0.1、`./30-workflow.md` §G | `node tools/doc-maintenance-check.js`（不升版、不跑测试） |
...
- 运行与改动类型匹配的专项门禁；
- 运行 `node tools/doc-maintenance-check.js`；发布追加 `--strict`，提交按 `./30-workflow.md` 执行；
- 使用 `node tools/bump-version.js --patch` 统一升版，并用 `--check` 验证零漂移（**仅代码/配置/契约变化时**；纯文档变更跳过升版与测试，按 `./30-workflow.md` §G 执行）；
- 在 `docs/current/01-changelog.md` 记录行为或契约变化。
...
本仓库有时会有多个 Agent 同时工作（多人或多会话并行），为避免提交相互覆盖、diff 归属不清。`WORKBOARD.md` 的用途与规则（开工前登记 / 收工清理 / 过期兜底 / 提交前核对）以根目录 `AGENTS.md` §4.19 为权威定义，本节不再重复；提交前结合写字板区分自己与他人的改动，只提交自己登记的改动（见附篇一 §F）。
...
- [ ] 窗口结构或跳转关系变化已同步 [./19-ui-implementation.md](./19-ui-implementation.md)。
...
完成一次维护后，维护者应更新文档内容（如有变化）、把 `lastReviewed` 改为当天，并在机制变化时追加 `../01-changelog.md`。推荐日常运行普通模式，发布前或 CI 运行 `--strict`。
