...
# 2. 验证服务就绪（返回 200 即可）
sleep 2 && curl -s -o /dev/null -w "%{http_code}" http://localhost:3004/

# 3. 打开浏览器
# （工具调用）open_url_in_browser → http://localhost:3004
...
- **前端服务前置**：先确认 `:3004` 已有服务（`lsof -i :3004 -sTCP:LISTEN`），已在运行则**直接访问**，重复启动 `server.js` 会触发端口递增卡死问题（见根 AGENTS.md §2 步骤三）。
...
```bash
# 前置：lsof 确认 :3004 已监听
paw browser-action '{"action":"navigate","url":"http://localhost:3004","waitUntil":"networkidle"}' \
