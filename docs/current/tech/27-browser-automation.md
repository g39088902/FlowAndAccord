...
# 3. 打开浏览器
# （工具调用）open_url_in_browser → http://localhost:3002
```
...
>
> 🔴 **适用边界（先读）**：内置预览浏览器**不支持 File System Access API**，无法通过本项目的启动存档门禁（模拟一直暂停）。因此它**只适合纯视觉截图 / 布局校验**；凡涉及存档读写、模拟推进、长程演化的验证，**能用 Chrome 测试必须优先用 Chrome 测试**（playwright-cli `--browser=chrome`，见 §3；或手动开系统 Chrome 访问 `localhost:3002`）。
>
...
- **前端服务前置**：先确认 `:3002` 已有服务（`lsof -i :3002 -sTCP:LISTEN`），已在运行则**直接访问**，重复启动 `server.js` 会触发端口递增卡死问题（见根 AGENTS.md §2 步骤三）。
...
```bash
# 前置：lsof 确认 :3002 已监听
paw browser-action '{"action":"navigate","url":"http://localhost:3002","waitUntil":"networkidle"}' \
