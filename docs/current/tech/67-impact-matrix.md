...
M19.1 增量边界：`decisions/intent.rs`、`strategy.rs`、`primitive.rs`、`observation.rs` 仅提供类型/只读适配，不变更 Agent 存储、WASM 导出或 FABS。新增旧动作/分支时同步观察映射；接管期须重新审计全部 state/pending 写入点。详见 [./32-m19-architecture.md](./32-m19-architecture.md)。
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| POI 数量变更 (countCamps 等) | `ecology/` 初始化 / `poi.rs` ID 段位 / `index.html` 顶栏面板文案 / `./21-ecology-and-poi.md` / `config.rs`+`config.js` / `render.js` POI 列表 | §4.7 POI 数量须同步多处 |
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| `config.rs` 新增超参 | 命名 `const` 默认值 / `SimConfig` 字段 / `Default` 映射 (三处同文件) / `config.js` 对应字段 / `config-check.js` 自动覆盖 / `./14-config-reference.md` (自动生成) | §4.12 超参集中化：Rust 侧三处 + 前端一处 |
...
| T0 占地或曲线合法性规则 | `geo/query.rs` / `housing_system/settlement.rs` / 后续 `ecology/spawn.rs` 与路网走廊生成器 / `config.rs` + `config.js` / `./14-config-reference.md` | 房屋、道路、农业和防务应消费统一查询，不能复制坡度/禁行判据 |
...
| `.github/workflows/deploy.yml` 变更 | 4 个 Secrets 配置 / COS MIME 覆写 / 门禁顺序 / `./64-cicd.md` | §4.13 CI/CD 使用标准 rustup，非便携工具链 |
...
| 动态定价算法 (`market_unit_price`) | `world_snapshot.rs` 快照单价计算 / `ecology/` 现场交易计费 / `decisions/market.rs` / `config.rs` + `config.js` 超参 / ./22-market-pricing.md | 幂律定价纯函数，全链条调用须一致 |
...
| 外部商贸资金流失记账 | `ledger/journal.rs` TransferReason::Market / `ecology/` debit 家户黄金转至 Void / `./20-ledger-and-polity.md` | 黄金单向流失沉淀，形成通缩调节机制 |
...
```
SimConfig (config.rs + config.house-upgrade-cost.js，共 232 字段)
    │  序列化
    ▼
