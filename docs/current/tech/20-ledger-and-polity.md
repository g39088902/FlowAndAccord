# 20. 📒 账本与社会经济制度系统 (`ledger` + `bookkeeping`)
...
- **夺位远征（v1.9.0 起决策引擎驱动，见 [./31-decision-engine.md](./31-decision-engine.md)）**：决策分支 `B14SeekThrone`（生理层最高档）自主触发——在世成年男性非国王且存在空缺王位营地（有房者仅夺自家房屋所在营地、无房可夺任意）时，选定最近可夺位营地写入 `agent.expedition_target_camp` 并冲向目标（走现有寻路+运动系统坐标连续不闪现，施工进度冻结不回滚）；抵达且王位仍空缺写 `coronation_pending`，由世界 `execute_pending_coronations` 校验后 `set_king` 登基。
...
账本与婚姻登记子系统（`ledger_journal_capacity`）、宗族系统 M3（`clan_tribute_*` / `clan_mutual_aid_*`）、地区与王国系统 M4（`ledger_tax_*` / `ledger_relief_*`）见 [./14-config-reference.md](./14-config-reference.md) 第 10/11/12 分区。
