...
本状态机描述「一次改动的上下游同步」：从定位唯一真相源出发，经 Rust 生产者、WASM/快照适配、前端消费、存档恢复，最后过门禁与文档。任一环节出现 `undefined` / panic / 确定性分叉即回退补齐。边界是 §4 交互遗漏排查链。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> A
    A --> B : 先问"谁是唯一真相源"并改 Rust 生产者
...
| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| A | 定位唯一真相源 | 改 X 前先确认数据与行为的权威位置 | 真相源明确，开始改生产者 |
| B | Rust 生产者 | 真相源已定位 | 内核代码/配置改动完成 |
| C | WASM/快照适配 | 涉及 WASM 或快照字段 | 双副本同步且四处同步完成 |
| D | 前端消费 | 快照/配置已适配 | `_applySnapshot` 与渲染层消费正确 |
| E | 存档恢复 | 涉及存档契约 | `world_save` 与生成器版本门禁对齐 |
| F | 门禁与文档 | 各消费端就绪 | 专项门禁全绿且影响矩阵/changelog 更新 |
| G | 回退补齐 | 任一环节 `undefined`/panic/分叉 | 修复后回到 A 重走 |

**不变量**（违反即出 bug）：
- 快照字段必须四处同步（C2）：`snapshot.rs` / `world_snapshot.rs` / `snapshot_bin/encode.rs` / `snapshot-bin.js`+`rustworld.js` 缺一则前端 `undefined`。
- WASM 双副本同步（C3）：改 Rust 后 `sim_wasm.wasm` 必须复制到 `frontend/rust/` 与 `frontend/`，否则浏览器加载旧逻辑。
- 交互遗漏按固定链 `唯一真相源 → Rust 生产者 → WASM/快照适配 → 前端消费 → 存档/恢复 → 门禁与文档` 排查，优先查链路而非改调用方兜底（§4）。
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| `agent.rs` 行囊容量 (`carryCapacityResource`) | `ecology/` 装载逻辑 / `decisions/harvest.rs` 满额判定 / `decisions/branches.rs` 返家条件 / `snapshot.rs` AgentSnapshot / `rustworld.js` `_applySnapshot` / `render.js` Inspector 行囊展示 | §4.4 全链条联动：容量决定何时满、何时返、展示什么 |
| `ecology/` 装卸货速率 (`poiUnloadRateResource`) | `agent.rs` 行囊状态机 / `decisions/` 停留时长 / `ledger/journal.rs` Deposit 流水速率 / `render.js` 进度动画 | 卸货速率决定在家停留时长和账本入账节奏 |
| 新增资源类型 (第六类) | `agent.rs` 行囊字段 / `ecology/` 装载/卸货 / `poi.rs` PoiType / `decisions/` 全套分支 / `ledger/journal.rs` ResourceKind / `snapshot.rs` / `rustworld.js` / `render.js` / `config.rs`+`config.js` 超参 / `bookkeeping.rs` RESOURCE_ORDER | 资源类型是全系统横切概念，每一层都有枚举匹配 |
...
M19.1 增量边界：`decisions/intent.rs`、`strategy.rs`、`primitive.rs`、`observation.rs` 仅提供类型/只读适配，不变更 Agent 存储、WASM 导出或 FABS。新增旧动作/分支时同步观察映射；接管期须重新审计全部 state/pending 写入点。详见 [./12-m19-architecture.md](./12-m19-architecture.md)。
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| `ledger/journal.rs` ResourceKind 变更 | `bookkeeping.rs` RESOURCE_ORDER / `ecology/` 收付映射 / `housing_system/maintenance.rs` 烧柴 / `decisions/` 余额读取 / `snapshot.rs` LedgerBalanceSnapshot / `rustworld.js` / `render.js` / `ledger-ui.js` 五类余额展示 | ResourceKind 是账本系统的基础枚举，全系统横切 |
...
| `ledger/clan.rs` 宗族规则变更 | `world.rs` tick_clan 调用 / `birth.rs` 新生儿入族 / `ecology/` 始祖入族 / `snapshot.rs` ClanSnapshot / `rustworld.js` / `ledger-ui.js` 宗族页 / `config.rs` 族税/互助超参 | M3 宗族是按姓氏聚合的团体，与家户、地区并列 |
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| POI 数量变更 (countCamps 等) | `ecology/` 初始化 / `poi.rs` ID 段位 / `index.html` 顶栏面板文案 / `./08-ecology-and-poi.md` / `config.rs`+`config.js` / `render.js` POI 列表 | §4.7 POI 数量须同步多处 |
| `poi.rs` 新增 POI 类型 | `ecology/` 初始化/采收 / `decisions/` 寻路/采收分支 / `agent.rs` PrimitiveActionState / `graph.rs` 节点类型 / `snapshot.rs` / `rustworld.js` / `render.js` 图标/弹窗 / `config.rs` 产速/储量超参 | POI 类型是全系统横切概念 |
| `ecology/` POI 再生规则 | `poi.rs` tick_regenerate / `world.rs` tick POI 再生段 / `config.rs` regen_base_* / `render.js` 储量条动画 | POI 再生在 tick 最前端执行，影响后续所有采收决策 |
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| `agent.rs` 代谢规则 (饱食/水分/体力衰减) | `world.rs` tick 代谢段 / `decisions/branches.rs` 需求阈值 / `ecology/` 在家吃喝 / `config.rs` 代谢超参 / `render.js` 状态条 / `birth.rs` 如涉及孕期 | 代谢是 agent 生存的基础，决定所有需求触发 |
...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| `config.rs` 新增超参 | 命名 `const` 默认值 / `SimConfig` 字段 / `Default` 映射 (三处同文件) / `config.js` 对应字段 / `config-check.js` 自动覆盖 / `./05-config-reference.md` (自动生成) | §4.12 超参集中化：Rust 侧三处 + 前端一处 |
...
| `.github/workflows/deploy.yml` 变更 | 4 个 Secrets 配置 / COS MIME 覆写 / 门禁顺序 / `./26-cicd.md` | §4.13 CI/CD 使用标准 rustup，非便携工具链 |

### 1.11 外部市场与动态价格系统 (v1.13.0)

| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| 外部市场 POI 实体 (`poi.rs`) | `ecology/` 播撒与双库存再生 / `snapshot.rs` secondary_* / `world_snapshot.rs` / `rustworld.js` / `render_world.js` 绘制 / `render_inspector.js` 弹窗 | 市场是双库存且动态计价的特殊 POI，独立于 NodePool |
| 动态定价算法 (`market_unit_price`) | `world_snapshot.rs` 快照单价计算 / `ecology/` 现场交易计费 / `decisions/market.rs` / `config.rs` + `config.js` 超参 / ./09-market-pricing.md | 幂律定价纯函数，全链条调用须一致 |
...
| 外部商贸资金流失记账 | `ledger/journal.rs` TransferReason::Market / `ecology/` debit 家户黄金转至 Void / `./07-ledger-and-polity.md` | 黄金单向流失沉淀，形成通缩调节机制 |
...
```
SimConfig (config.rs + config.house-upgrade-cost.js，共 358 字段)
    │  序列化
    ▼
...
□ 四处同步（★ M4）：snapshot.rs / world.rs / snapshot_bin/encode.rs / snapshot-bin.js+rustworld.js 字段一致 (仅快照变更)，核对 `node tools/snapshot-check.js`（JSON 对拍门禁已于 v1.50.35 移除）
