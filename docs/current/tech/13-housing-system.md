...
- 房屋升级/修缮仅追加账本流水（Construction/Maintenance），不扣物理库存；耐久<50% 触发修缮且仅 owner/spouse 可修，无主房无人修直至坍塌。
- 空置房与有主同速率风化（`house_depreciation_rate×2`=0.04/s），坍塌后大门路网节点可被新立宅复用；成交保持成交时等级不变，买方搬入不足 50% 耐久自然激活修缮。
...
| 4 | 大庄园 | 🏰 | 160.0 | 终极形态 | 超大规模独立储备，解锁积累财富 |
...
- 气温 < 8℃ 时（由 `config.house_winter_cold_temp` 控制，不再受季节名硬绑定），非 0 级有主房屋每秒消耗 0.12 木材供暖；无主房不供暖。木材 < 10 时禁孕。
...
1. **出价下沉到决策引擎**：无房成年男性或已有房屋的户主在自己的决策相位（`(tick+id)%120==0`）命中 `B17BidHouse` 分支，一次性对所有符合条件的在售房写下升序目标集合 `pending_bid_house_ids`（不改变运动状态）；世界物理执行器 `execute_pending_bids` 按房屋 ID 升序逐个倾囊落地、成交一套即停，出价后进入 `houseAuctionBidCooldownTicks`(300) 全局冷却。
...
#### 2.7.2 v1.27.0 拍卖累计统计与 v1.34.0 历史应价队列 (容量 256)
...
★ **v1.34.0 全局应价与流拍历程沉淀**：
- 世界层维护容量为 256 的双端队列 `auction_history: VecDeque<HouseAuctionHistoryRecord>`，环形溢出截断；
...
- `ecology/`：POI 采收后回家卸货存入仓库。
- `ledger/`：家户体系以男性户主为锚，房屋与家户解耦但通过户主关联；★M2 升级竣工/修缮完工写入家户账本流水。
- `graph.rs`：立宅时接入路网或复用空置节点。
- `snapshot.rs`：房屋等级、耐久、仓储随快照下发。

## 5. 调参入口
房屋容量、升级门槛、施工时长、折旧修缮、供暖参数、立宅选址参数等见 [./05-config-reference.md](./05-config-reference.md) 第 6 分区。
