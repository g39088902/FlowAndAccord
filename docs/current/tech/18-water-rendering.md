...
单帧水系渲染管线的分层合成状态迁移：从底模河床消隐、湿砂漫滩、矢量水面到波光微沫，逐层叠加掩盖 13 米网格阶梯锯齿。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> BED_HIDE
    BED_HIDE --> SANDBANK : 网格 Quad 绘制后填充漫滩带
    SANDBANK --> VECTOR_H2O : 漫滩带之上闭合填充水面
    VECTOR_H2O --> CAUSTICS_FOAM : 水面后叠加迎光波光与岸线微沫
    CAUSTICS_FOAM --> PRESENT : 水体合成完毕交还主管线
    PRESENT --> [*] : 进入道路/实体层
```

| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| BED_HIDE | 底模河床消隐：`DeepWater`/`ShallowWater` 格改深褐河床土色，消除亮蓝阶梯 | 进入 `drawTerrainFeatures` 水系 pass | 网格 Quad 绘制后 |
| SANDBANK | 湿砂漫滩带：沿左右岸线外延 `w_bank≈10m` 填充漫滩多边形 | Quad 绘制完成 | 漫滩带填充完成 |
| VECTOR_H2O | 矢量水面：194 顶点闭合多边形 `rgba(54,158,202,0.62)` 填充 | 漫滩带之上 | 水面填充完成 |
| CAUSTICS_FOAM | 波光与微沫：Layer2.8 太阳波光 + Layer3 岸线微沫漂移（游鱼/卵石走墙钟独立） | 水面填充后 | 全部水层合成完毕 |
| PRESENT | 合成呈现：水体交还主渲染管线（边界墙已按 `ry` 升序深度排序） | 水层合成完毕 | 提交主管线 |

**不变量**（违反即出 bug）：
- 严禁全局细分网格（Zero Grid Subdivisions）：在固定 120×120 物理网格上用分层矢量覆盖解决锯齿，不得提升网格分辨率。
- 零运行时 GC 分配：投影顶点缓冲静态预分配（`Float32Array`），`render()` 循环内禁止对象分配，帧耗时增量 ≤ 0.08ms。
- 观感降噪铁律：凡「沿中心线/岸线走线、笔宽/间距与河宽成比例」的元素在窄河道退化成车道线，只有面状（水面填充/地表格色）与点状/小尺度（游鱼/波光）元素才能稳定成立。

## 1. 背景与现状视觉实证

在 v1.48.0 之前的项目版本中运行并随机生成河流地图（模板 `river_valley_v1`），通过高分辨率无头诊断采样捕获历史渲染特写；截图素材未随仓库分发，缺陷表现以 §1.1 文字描述为准。
...
## 3. 总体解决方案：四层沙盘水系渲染管线 (The 4-Layer Water Pipeline)
...
### 3.1 Layer 0：底模河床平缓消隐 (Submerged Riverbed Mesh)
...
### 3.2 Layer 1：平滑湿砂漫滩带 (Moist Sandbank Ribbon)
...
### 3.3 Layer 1.5：河床生态层 (RiverLife Submerged Layer) ★ v1.49.0 新增
...
### 3.4 Layer 2：连续矢量水面多边形 (Vector Water Surface)
...
### 3.5 Layer 2.8：迎光面太阳波光粼粼 (Sun Caustics Glint) ★ v1.49.0 新增
...
### 3.6 Layer 3：岸线微沫与碎沫漂移 (Shoreline Foam & Drift)
- **岸线微沫高光（Rim Foam）**：
...
### 3.9 ★ v1.48.2 沙盘边界墙深度排序 (Boundary Wall Depth Sort)
...
### 6.2 前端水系渲染重构（四层管线 + 抗锯齿 + 边界墙） ✅ v1.48.1 / v1.48.2 已完成
...
### 6.3 水系微观生态层（RiverLife） ✅ v1.49.0 已完成
