...
│   │   └── src/
│   │       ├── config.rs                   # ⚙️ SimConfig 结构体 (232 字段，纯净 derive(Default)，JS 唯一真相源)
...
├── frontend/
│   ├── js/
│   │   ├── config.js                       # ⚙️ 主配置 (window.SIM_CONFIG, 232 字段)
...
│   ├── code-map-check.js                   # ★ 代码地图一致性校验 (实际文件 vs ./90-code-map.md 登记 + 描述漂移检测)
│   ├── config-check.js                     # 前后端配置一致性校验 (含 config.house-upgrade-cost.js) + ./14-config-reference.md 自动生成
...
├── TODO.md                                  # 待办事项清单
└── docs/                                    # 📚 全部项目文档（当前/计划 → 产品设计/技术方案 → 分层编号）
    ├── README.md                              # 文档总导航
    ├── doc-maintenance.json                   # 文档维护清单契约配置
    ├── decision-viz-prototype.html            # 决策可视化交互原型
    ├── current/                               # 现状：代码中真实存在的行为
    │   ├── README.md                          # 现状总索引（含完整模块导航表）
    │   ├── 11-changelog.md                    # 版本演进记录
    │   ├── design/                            # 产品设计（玩家视角：看到什么、规则是什么）
    │   │   ├── 10-product-overview.md           # 定位、核心循环、八大看点、首局瞬间
    │   │   ├── 11-world-rules.md                # 地图模板、地形、道路涌现、四季
    │   │   ├── 12-life-and-society.md           # 马斯洛六层、生命周期、房屋五级、社会结构
    │   │   ├── 13-economy.md                    # 有限生态、真实搬运、家户账本、榷场定价
    │   │   └── 14-observation-ux.md             # 窗口模型、观察工具、操作与信息层级
    │   └── tech/                              # 技术方案（实现视角，编号十位 = 层）
    │       ├── 10-engine-architecture.md        # 总体：三层解耦、tick 顺序、数据流
    │       ├── 11-core-systems-fsm.md           # 三大核心系统状态机全景
    │       ├── 12-determinism.md                # 确定性原理、RNG 分域、门禁矩阵
    │       ├── 13-config-system.md              # SIM_CONFIG 免编译热调优
    │       ├── 14-config-reference.md           # 参数速查表（config-check 自动生成，勿手改）
    │       ├── 15-snapshot-and-save.md          # FABS 快照与存档全量契约
    │       ├── 20-ledger-and-polity.md          # 上层：账本、家户、宗族、王国、帝国
    │       ├── 21-ecology-and-poi.md            # 上层：23 处有限生态、采收与卸货
    │       ├── 22-market-pricing.md             # 上层：榷场互市与幂律定价
    │       ├── 30-agent-life-cycle.md           # 中层：生理代谢、繁衍、禀赋、死亡
    │       ├── 31-decision-engine.md            # 中层：马斯洛六层与 18 条分支（附设计思路）
    │       ├── 32-m19-architecture.md           # 中层：意图-策略-原语三层解耦规格
    │       ├── 33-housing-system.md             # 中层：五级房屋、折旧、空置房拍卖
    │       ├── 40-terrain-and-network.md        # 下层：地表单元、水系、路网与通行
    │       ├── 41-seasons-climate.md            # 下层：四季年轮与冬季供暖
    │       ├── 50-frontend-overview.md          # 表现层：渲染管线与深度队列铁律
    │       ├── 51-seasonal-lighting.md          # 表现层：年周期光弧与地形重着色
    │       ├── 52-water-rendering.md            # 表现层：矢量河面、漫滩与水体生态
    │       ├── 53-ui-implementation.md          # 表现层：页面全景 + 窗口结构与跳转
    │       ├── 54-society-ledger-ui.md          # 表现层：制度大盘四标签页
    │       ├── 55-frontend-dev-guide.md         # 表现层：前端开发实施指南
    │       ├── 60-build-and-run.md              # 工程：工具链、WASM 双副本、快速启动
    │       ├── 61-tools-guide.md                # 工程：tools/ 全工具速查
    │       ├── 62-diagnostics.md                # 工程：无头诊断 SOP
    │       ├── 63-benchmarking.md               # 工程：性能基准与确定性矩阵
    │       ├── 64-cicd.md                       # 工程：GitHub Actions → 腾讯云 COS
    │       ├── 65-browser-automation.md         # 工程：浏览器自动化与截图
    │       ├── 66-invariants.md                 # 工程：六类硬约束集中清单
    │       ├── 67-impact-matrix.md              # 工程：改 X 牵动哪些文件 + tick 顺序
    │       ├── 68-workflow.md                   # 工程：Agent 入口 + 提交检查单 + 文档维护
    │       └── 90-code-map.md                   # 附录：本文件
    ├── plan/                                 # 计划：在办设计与未落地方案
    │   ├── README.md                          # 计划总索引（含依赖顺序图）
    │   ├── design/                            # 产品设计（玩法方向）
    │   │   ├── 10-roadmap.md                    # 长期路线图 M10~M18
    │   │   ├── 11-everyday-life.md              # 人物日常生活细化
    │   │   ├── 12-social-relations.md           # 人际依赖与敌对 × 生产
    │   │   ├── 13-hormone-system.md             # 四轴十一激素调制层
    │   │   ├── 14-affinity-system.md            # 人际好感度系统
    │   │   └── 15-competitor-analysis.md        # 竞品与同类项目分析
    │   └── tech/                              # 技术方案（未落地实现）
    │       ├── 20-integration-contracts.md      # 跨专项共享契约权威
    │       ├── 21-memory-system.md              # 人物记忆与经验传承
    │       ├── 22-internal-market.md            # 售货员撮合与有限订单簿
    │       ├── 23-farmland-agriculture.md       # 农田与农业税
    │       ├── 24-hunting-defense.md            # 狩猎、流寇与武力公约
    │       ├── 25-terrain-templates.md          # 地图模板库与 D-B 子特征蓝图
    │       ├── 26-terrain-art.md                # 地形美术与世界景观
    │       └── 27-performance.md                # 仅保留未完成的性能优化
...
`decisions/intent.rs` 定义意图并转换已知来源的 Need；`strategy.rs` / `primitive.rs` 定义计划词汇；`observation.rs` 为 Agent 提供借用式执行事实与旧状态无损视图。旧 evaluate/routing/scheduler 继续执行，不存在第二个调度器。API 与存储边界见 [./32-m19-architecture.md](./32-m19-architecture.md)。
