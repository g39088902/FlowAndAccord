...
- ★ **水体是开放水循环**（6，v1.63.0）：降雨（全图落点）/ 泉眼（读 `WaterSource` POI 坐标）补源，越出地图边界即出流、坡面漫流粒子超时下渗 ⇒ 粒子数动态（上限 1600）；无静态水体但**有泉眼**的地形（山口 / 冲积扇）因此也参与本阶段。
...
> → `snapshot-bin.js` + `rustworld.js`。回归兜底是 `test-wasm.js` / `test-determinism.js`（JSON 对拍门禁已于 v1.50.35 随 JSON 快照通道移除）。
