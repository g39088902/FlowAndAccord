# 07. 📒 账本与社会经济制度系统 (`ledger` + `bookkeeping`)

> **模块索引**：[← 返回 ../README.md 全景索引](../README.md) · 主要源码：`crates/sim_core/src/spatial/ledger/`（8 子模块）+ `crates/sim_core/src/spatial/bookkeeping.rs`
...
>
> ⚡ **M8（v1.6.0）追加**：升级材料成本改**4×5 固定矩阵**（20 超参，`config.house-upgrade-cost.js` 权威值，Rust `config.rs` 平铺三处同步）；升 1 级水粮各 50、2 级木粮水各 75、3 级石木粮水各 100、4 级金石木粮水各 125，升级时从家户账本**一次性扣账**（Construction 流水）。本文件以下 M2~M4 记账机制描述中涉及「旁路观测/与物理仓库分离/等级备货目标/容量上限」的表述已过时，请以代码注释与 01-changelog v1.6.0 条目为准。
>
> ⚡ **v1.44.0 内部性能优化（行为逐拍等价，Phase 7 攻坚）**：`tick_household_split`（bookkeeping.rs）为 `Agent3D` 引入**纯性能缓存标志 `is_household_head`**（非业务真相源，可由 `hh.head == agent.id` 派生）——户主在世终生不分家，循环首部 O(1) 布尔短路，原幂等红黑树判断保留为兜底并在命中时**自愈回填**，旧档与继承/婚姻/分家等全部新增立户路径均无缝收敛，无需在立户调用点散落置位；`tick_clan_mutual_aid` / `tick_region_relief` 由「团体外层 × 全图家户内层」笛卡尔积**倒置为单次存续家户遍历**（先两浮点极贫门槛短路、再冷却、才反查归属核对族长/国王在位与族库/公仓资金），READ 后按（姓氏/camp_id, hid）排序保序派发，WRITE 原样，稳态零临时堆分配。确定性 6 套件全通；性能收益量化见 01-changelog v1.44.0 条目。

---

## 状态机

本文件描述家户（基层经济组织）与王国（地区政体）两套相对独立的生命周期状态机，分别由账本/家户 tick 与地区 tick 推进，决策引擎只读或驱动。

### 状态机 A：家户生命周期（`HouseholdRegistry`）

单个家户从立户、分家派生、户主死亡继承到绝嗣解散的完整生命周期，由 `bookkeeping::tick_inheritance` / `tick_household_split` 推进。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> Active : create() 立宅或成婚建户，active_households 自动登记
    Active --> Split : 已婚男性触发 Split 分家抽资，派生新家户
    Split --> Active : 分家完成，原户与派生户均存续（幂等跳过已立户者）
    Active --> Inherit : 户主死亡，同拍 settle_death_cargo 遗物归户
    Inherit --> Active : 有在世男性继承人（子），遗产平分后家户由继承人接续
    Inherit --> Dissolved : 无男性继承人（绝嗣），遗产充公/平分后同拍解散
    Dissolved --> [*]
```

| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| Active 存续 | 有男性户主、成员非空，登记于 active_households | create() 建户（立宅/成婚） | 触发分家、户主死亡或绝嗣 |
| Split 分家派生 | 分家抽资进行中，派生新家户 | 已婚男性满足分家条件 | 派生新户完成，原户回到 Active |
| Inherit 继承清算 | 户主死亡当拍，遗产分配与家户存续判定 | 户主死亡（is_alive=false） | 判定有继承人接续或无继承人解散 |
| Dissolved 解散 | 家户注销，从 active_households 移除 | 绝嗣（无男性继承人/成员清空） | 进入 [*]，资源已分配 |

### 状态机 B：王国继承（`RegionRegistry`）

单个营地的王位/王国政体状态，由 `tick_region` 初王/继承结算与决策引擎 `B14SeekThrone` 夺位远征共同推进。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> Vacant : 营地建立（始祖落位/孤儿营地），leader=None 账本冻结
    Vacant --> Pending : B14 非国王男性抵达空缺营地，写 coronation_pending
    Vacant --> Reigning : 初王顺位（arrival_order 首抵男性）/ 孤儿补王 set_king
    Pending --> Reigning : execute_pending_coronations 校验王位仍空缺 → set_king（+3 威望）
    Pending --> Vacant : 校验时王位已被占（目标易主），放弃折返
    Reigning --> Succession : 国王死亡，遗嘱入 history_kings，重排继承人
    Succession --> Reigning : 有在世男性顺位（长子→孙子→arrival 下一）→ set_king
    Succession --> Vacant : 无男性继承人（绝嗣），账本冻结
    Reigning --> Reigning : 夺位 B14 易主（原王废黜）或长子继承直接续位
```

| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| Vacant 空悬 | 无王，地区账本冻结（只进不出） | 营地建立 / 绝嗣 / 夺位失败 | 初王顺位、孤儿补王或 B14 抵达 |
| Pending 夺位待决 | 远征者抵达写 coronation_pending，待世界执行器校验 | B14 抵达空缺营地 | 校验通过登基或王位被占放弃 |
| Reigning 在位 | 国王在位，征税/救济/内帑活跃 | 初王/继承/夺位登基 set_king | 国王死亡进入 Succession 或被夺位 |
| Succession 继承清算 | 国王死亡，重排 arrival_order 选继承人 | 国王死亡（is_alive=false） | 选出继承人续位或绝嗣空悬 |

**不变量**（违反即出 bug）：
- 家庭储备唯一真相源是家户账本；一人同时只属于一个家户（`by_agent` 唯一索引），女性绝不能成立家户（丧父/丧夫也不行）。
- 宗族/地区无主（leader=None）时账本冻结（只进不出）；族税/公仓税全局统一时点征收（`clan_tribute_interval_ticks`=1800、`ledger_tax_interval_ticks`=4800）保证确定性。
- 继承先于分家（Split 幂等跳过已立户者）；户主死亡当拍先 `settle_death_cargo` 归户再 `tick_inheritance` 分配，避免遗物无处可归。
...
### 2.3 TransferRecord / TransferReason
- `TransferRecord`：单次转账记录（品类、数量、对手方、原因、时间戳）。
- `TransferReason`：转账原因枚举，已落地 `Deposit` / `Consume` / `Heating` / `Construction` / `Maintenance` / `Inheritance` / `Split` / `Tax` / `Tribute` / `MutualAid` / `Relief` / `Market` / `HousingPurchase` / `EstateShare` / `TransferTax` / `RoyalPrivy` 等。

### 2.4 LedgerRef（账本主体引用）
...
- **夺位远征（v1.9.0 起决策引擎驱动，见 [./11-decision-engine.md](./11-decision-engine.md)）**：决策分支 `B14SeekThrone`（生理层最高档）自主触发——在世成年男性非国王且存在空缺王位营地（有房者仅夺自家房屋所在营地、无房可夺任意）时，选定最近可夺位营地写入 `agent.expedition_target_camp` 并冲向目标（走现有寻路+运动系统坐标连续不闪现，施工进度冻结不回滚）；抵达且王位仍空缺写 `coronation_pending`，由世界 `execute_pending_coronations` 校验后 `set_king` 登基。
...
## 3. 源码分布（ledger 7 子模块 + bookkeeping）
...
## 4. ★ v1.47.0 逝者随身遗物归户（`settle_death_cargo`）
...
账本与婚姻登记子系统（`ledger_journal_capacity`）、宗族系统 M3（`clan_tribute_*` / `clan_mutual_aid_*`）、地区与王国系统 M4（`ledger_tax_*` / `ledger_relief_*`）见 [./05-config-reference.md](./05-config-reference.md) 第 10/11/12 分区。
