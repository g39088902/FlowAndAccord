...
│   │   ├── water_gpu.js                    # ★ 2026-09-26 水体粒子 **GPU 求解器** (window.WaterGPU：WebGPU compute 推进 **3D PBF**——补源/3D 重力+地面接触受力预测/3D 网格空间哈希（分层前缀和、3×3×3 邻域）/密度-λ/Δp/XSPH/淘汰，2 万粒子槽位；粒子 z 为真实自由度，地面接触按地形法向拆重力 ⇒ 顺坡下泄汇入洼地；★ 2026-09-27 补源仅降雨（删除清泉 POI 涌水）+ despawn 按「被水环绕程度」插值蒸发；结果经 staging mapAsync 回读写 rustWorldSim.fluidParticles + fluidParticleIds，供 water_particles.js 渲染；纯表现层、放弃确定性、不进快照/存档；无 WebGPU 由 main.js 硬门槛阻断启动；storage buffer 必须带 COPY_DST；详见 tech/33-runtime-fluid.md §8)
│   │   ├── water_particles.js              # ★ v1.62.0 水体粒子**渲染层** (window.WaterParticles：消费**前端 GPU 求解器**输出的粒子三维位置（原内核快照 Fluid section 已退役），逐粒子视觉抖动由「粒子槽位号 + 世界种子」稳定哈希派生（只算一次、稳态不闪）；复用 v1.61.4 观感常量与 sink 落笔路径；**不再自带求解器**、不消费水深字段；★ v1.63.0 粒子数逐帧可变 + 启用门槛取消，详见 tech/33-runtime-fluid.md)
