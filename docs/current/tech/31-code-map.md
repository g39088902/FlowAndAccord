...
│   │   └── src/
│   │       ├── config.rs                   # ⚙️ SimConfig 结构体 (386 字段，纯净 derive(Default)，JS 唯一真相源)
...
├── frontend/
│   ├── js/
│   │   ├── config.js                       # ⚙️ 主配置 (window.SIM_CONFIG, 386 字段)
...
│   │   ├── render_landscapes.js            # ★ S4-02 资源景观绘制接入层 (collectLandscapes 入队 + drawLandscapeChild/drawLandscapeShadowGround 分发；复用装饰图元/光照/季相，配置关态零开销回退；★ S4-03 被遮蔽子图元连同投影不入队；★ S4-04 detail 子图元 childActive q 过滤 + drawLandscapeGroundPatch 贴地色差片，填充样式预建常量零 GC；★ S4-05 berry/gold 点簇（round(10×q) 可见点数）+ quarry globalAlpha 渐强 + child._q 镜像；★ TA-07 入队与绘制共用 AccentLOD AABB（删重复 44/14 余量），签名加 it，贴地片阈值收编 accentLODGroundPatchMinPx)
