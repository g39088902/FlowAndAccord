# 55. 🛠️ 前端开发实施指南 (`frontend`)

> **模块索引**：[← 返回 ../README.md 全景索引](../README.md) · 适用范围：新增前端 UI / 社会制度界面开发
> **关联文档**：[./50-frontend-overview.md](./50-frontend-overview.md)（模块总览）· [./53-ui-implementation.md](./53-ui-implementation.md)（UI 页面全景剖析）· [./54-society-ledger-ui.md](./54-society-ledger-ui.md)（制度大盘界面实现）
...
> 以下为当前实际落地的快照结构（注意：`TransferRecordSnapshot.from/to` 为字符串化主体标识，非整数；`ClanSnapshot` 用 `member_ids` 而非家户 ID 列表；`RegionSnapshot.history_kings` 自 v1.12.0 起为 `HistoryKingSnapshot` 对象数组）。完整字段见 [./20-ledger-and-polity.md](./20-ledger-and-polity.md)。
