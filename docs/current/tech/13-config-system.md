# 13. ⚙️ JavaScript 动态数值配置系统 (`config.js`)
...
### 全量超参数抽取
- `SimConfig` 共 **232 个字段**，按 14 个分区组织（分区与字段数以 `crates/sim_core/src/config.rs` 注释及 [./14-config-reference.md](./14-config-reference.md) 自动速查表为准）：
  1. 引擎节拍与时间基准（3 字段）
...
- ⚠️ 改动本值会改变地形形态、POI 落位与全部确定性基线，并使旧存档因 `SAVE_APP_VERSION` 变更而废弃；
  调整后必须重跑全量门禁，并参考 `./63-benchmarking.md` 建立性能基准（格子数按 res² 增长）。
...
4. **速查表生成**：数值 100% 提取自 `config.js` 唯一真相源，自动更新生成 `./14-config-reference.md`。
...
## 调参入口
所有参数均在 `frontend/js/config.js` 中，字段说明见 [./14-config-reference.md](./14-config-reference.md)。
