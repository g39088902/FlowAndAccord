# 04. ⚙️ JavaScript 动态数值配置系统 (`config.js`)

> **模块索引**：[← 返回 ../README.md 全景索引](../README.md) · 主要源码：`frontend/js/config.js`、`frontend/js/rustworld.js`、`crates/sim_core/src/config.rs`

---

## 状态机

配置从「JS 真相源 → 序列化注入 WASM → 内核生效」流转，并由 `config-check.js` 契约门禁把关；任一校验失败即拒绝发布、回退修复。

```mermaid
stateDiagram-v2
    direction LR
    [*] --> A
    A --> B : 浏览器 Ctrl+F5 加载 WASM
    B --> D : rustworld.js::applyConfig 调 world_apply_config_buf 注入 SIM_CONFIG
    D --> C : 五类校验全过(孤儿/缺失/类型/速查表/空转)
    D --> E : config-check.js 任一项报错 exit 1
    C --> A : 继续热调优
    E --> A : 修复前后端契约后重跑
    C --> [*]
```
...
| 状态 | 含义 | 进入条件 | 退出条件 |
| :--- | :--- | :--- | :--- |
| A 真相源就绪 | `config.js` 持有 376 字段权威数值，Rust 不保留字面量常量 | 开发者编辑 config.js 或世界启动 | 浏览器刷新加载 WASM |
...
| E 门禁失败 | 前后端契约未同步，拒绝发布 | config-check.js exit 1 | 修复并重跑 |

**不变量**（违反即出 bug）：
- 前端 JS (`config.js`) 为仿真超参数唯一数值真相源，Rust 内核不保留数值字面量常量。
- `config.js` 字段集与类型必须与 `config.rs` 契约严格 100% 吻合。
- `node tools/config-check.js` 与 `node tools/test-wasm.js` 双绿方为可发布状态。

## 1. 模块定位

全部仿真超参数的统一配置入口。**386 个** `SimConfig` 字段由 `frontend/js/config.js` 及拆分配置（`config.house-upgrade-cost.js` / `config.decision-order.js`）驱动，经 `rustworld.js::applyConfig` 反序列化注入 Rust WASM 内存，实现免重新编译的热调优。Rust 逻辑层一律通过 `self.config.<字段>` 引用，禁止散落字面量。

## 2. 核心机制

### 2.1 全量超参数抽取
- `SimConfig` 共 **386 个字段**，按 16 个分区组织（分区与字段数以 `crates/sim_core/src/config.rs` 注释及 [./05-config-reference.md](./05-config-reference.md) 自动速查表为准）：
  1. 引擎节拍与时间基准（2 字段）
  ...
  7. 地形生成、地表查询与山口/河谷/草原/半坡/河谷聚落/台地/★ TB-03 冲积扇/盆地/湖畔盆地 profile（92 字段）
  8. 四季更迭与宏观气候（9 字段）
  ...
  15. 二手房屋市场、营地中介拍卖与麦穗竞价（8 字段）
  16. 四轴十一激素系统 · 动力学首批超参 (H-01 / H-02 / H-03)（58 字段）＋ 耦合与峰后窗口 (H-04)（12 字段）＋ 行为意愿通道 (H-08 / H-09)（10 字段）

### 2.2 免编译热调优
- 前端 `rustworld.js` 在加载 WASM 及重置模拟时，通过 `world_apply_config_buf` 将 `window.SIM_CONFIG` 动态序列化注入 Rust WASM 内存。
- 开发者直接编辑 `frontend/js/config.js` 并刷新浏览器（Ctrl+F5），即可即时生效，无需重编 WASM。
- `config.js` 每个字段均带中文行内说明。

### 2.3 POI 产速的本地创世偏好（v1.46.2）
...
## 3. 免编译热调优
- 前端 `rustworld.js` / `sim_worker.js` 在加载 WASM 及创建/重置模拟时，通过 `world_apply_config_buf` 将 `window.SIM_CONFIG` 动态序列化注入 Rust WASM 内存。
- 开发者直接编辑 `frontend/js/config.js` 并刷新浏览器（Ctrl+F5），即可即时生效，无需重编 WASM。
- `config.js` 每个字段均带中文行内说明。
...
### 3.2 数组类型字段与「Rust 无顺序」例外（v1.3.6 起）
...
### 3.3 配置校验工具 `tools/config-check.js`
零依赖纯 Node 脚本，作为**纯契约门禁**校验 `config.js` 与 `config.rs`：
1. **孤儿字段**：前端有 / Rust 无
2. **缺失字段**：Rust 有 / 前端无
3. **类型错配**：`usize/u64`、浮点、布尔与 `Vec<String>`/`Vec<u8>` 数组类型严格一致
4. **速查表生成**：数值 100% 提取自 `config.js` 唯一真相源，自动更新生成 `./05-config-reference.md`。
5. ★ **空转参数（v1.50.18 新增）**：Rust 有 / 前端有 / 类型也对，但 `crates/` 内核除 `config.rs` 自身外
   **零次读取** `.field`。这类字段调它不产生任何效果却会误导调参——前四条规则均检测不出，需此条兜底。
   命中后要么删除、要么在内核接线读取。

任一报错即说明前后端契约未同步，须先修复再发布。改参或字段后必跑。

### 3.4 参数速查表 `./05-config-reference.md`
...
## 4. 关键不变量
- `SimConfig` 当前有效字段数为 **386 个**（以 `config-check.js` 运行时输出为准）。
...
- `sim_wasm`：导出 `world_apply_config_buf` 函数接收配置。
- 所有 Rust 逻辑模块：通过 `self.config` 读取参数。
- `tools/config-check.js`：校验前后端一致性并生成速查表。

## 6. 调参入口
所有参数均在 `frontend/js/config.js` 中，字段说明见 [./05-config-reference.md](./05-config-reference.md)。
