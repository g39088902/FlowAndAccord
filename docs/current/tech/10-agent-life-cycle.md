...
- 婚姻登记的完整权责记录见 [./07-ledger-and-polity.md](./07-ledger-and-polity.md)。

### 2.5 生命周期与孕育状态机

> 状态与阈值对照源码 `agent.rs::tick_metabolism`、`birth.rs::resolve_newborns`、`decisions/branches.rs`（B16）与 `scheduler.rs::execute_pending_courtships`；时间常量权威定义在 `config.rs`（成年 1800s、孕期 200s、流产/产后冷却各 200s、遗骸风化 12s）。求偶/成婚的动作层状态见 [./11-decision-engine.md](./11-decision-engine.md) 的行动状态机总图。
...
## 5. 调参入口
生理代谢、容量、繁衍门槛、遗传参数、行囊容量等见 [./05-config-reference.md](./05-config-reference.md) 第 2、3 分区。
