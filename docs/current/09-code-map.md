...
> 本文件源码树与实际仓库 100% 对应（最后核验：v1.3.2）。
...
│   │   └── src/
│   │       ├── config.rs                   # ⚙️ SimConfig 结构体 (163 字段) + 默认常量
...
│   │           ├── bookkeeping.rs          # ★ M2 旁路记账 (Deposit/Consume/Heating + 继承 + 分家)
│   │           ├── world.rs                # World3DEngine 世界调度、四季温度、快照生成、配置注入、tick_bookkeeping/tick_clan/tick_region
...
├── frontend/
│   ├── js/
│   │   ├── config.js                       # ⚙️ 全局动态数值配置 (window.SIM_CONFIG, 163 字段)
│   │   ├── math.js                         # 3D 向量与投影变换
