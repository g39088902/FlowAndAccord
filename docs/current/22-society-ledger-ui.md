# 22. 🏛️ 制度大盘 UI 实现（M1~M4）(`ledger-ui.js`)

> **模块索引**：[← 返回 01-current.md 全景索引](../01-current.md) · 主要源码：`frontend/js/ledger-ui.js` · 内核机制：[12-ledger-system.md](./12-ledger-system.md)
> **关联文档**：[21-ui-page-overview.md](./21-ui-page-overview.md)（UI 页面全景剖析）· [23-ui-dev-guide.md](./23-ui-dev-guide.md)（前端开发实施指南）
> **里程碑状态**：M1~M4 已完整落地（M1 v1.0.0 / M2 v1.1.0 / M3 v1.2.0 / M4 v1.3.0）；M5 帝国上层政体（v1.44.7）见 [12-ledger-system.md](./12-ledger-system.md)。
