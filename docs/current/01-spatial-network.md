...
- **水系河谷与写意沙盘平滑管线（★ v1.48.2）**：T2 河谷（`river_valley_v1`）生成确定性闭合水体多边形（`River`）与双岸平滑几何（`RiverBank`）；前端通过四层微缩沙盘管线（河床底模消隐、平滑湿砂漫滩带、连续碧蓝矢量水面闭合填充、水陆交界表面张力微沫高光与中心微波）彻底消灭 13 米离散网格阶梯锯齿，零网格细分、零 GC 堆分配，耗时增量 $\le 0.08\text{ms}$。
