...
### 踏路成道与自然衰减（Stigmergy）
- 族人通行时累加踩踏耐久（`wear += 0.1`，上限溢出允许至 10.0，超过 5.0 无额外速度增益），往返双向共同加固。
...
- **端点对路径缓存与 0.25 级离散阶梯失效（★ v1.37.5 / v1.40.3）**：`LaneGraph3D` 维护 `(start, goal, prefer_hidden)` 的路径缓存，A\* 搜索综合道路基础限速与以 0.25 级（`road_wear_tier_step`）为阶梯步进的量化踩踏速度加成（`0.50x~2.20x`）及几何坡度；仅在立宅建路、动态调参、或车道踩踏/自然衰减跨越 0.25 级门槛时清空缓存，同桶内微增保持 100% 缓存命中，兼顾 Stigmergy 踏路成道宏观涌现与高运行效率。
