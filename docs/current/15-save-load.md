...
- **`app_version` 强制门禁与自动废弃（★ v1.37.1）**：`world_save.rs` 的 `SAVE_APP_VERSION` 随版本发布更新（当前 **1.37.1**）。`deserialize_save` 中作为内核硬性门禁校验（`save.app_version != SAVE_APP_VERSION` 直接返回 Err 拒绝），版本变更时旧档**自动废弃**。
