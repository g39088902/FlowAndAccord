...
1. **建设成本等效黄金折算 ($C_{const}$)**：
   - 依托外部市场（榷场互市）动态价格纯函数，动态计算水与粮的实时市价；木与石采用 `marketPriceBaseWood`(0.15) 与 `marketPriceBaseStone`(0.20) 基准价折算；
   - 累加房屋从 0 级至当前等级物化的所有基础建材，推导等效黄金成本 $C_{const}$（0 级仓库享有 `houseBaseFoundationCostGold` 5.0 保底地基价值）。
2. **双轨估价与买方开价**：
