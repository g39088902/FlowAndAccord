...
### 修建/升级者历史确权（v1.8.6）
- 每间房屋记录两个**历史确权**字段，展示于房屋 Inspector「🧱 修建/升级者」行：
  - `builder_id`（修建者/立宅人）：立宅实体化（`materialize_founded_houses`）时以立宅人 ID 固定，之后永不改写；
  - `last_upgrader_id`（最近升级者）：每次瞬时升级成功（`try_instant_upgrade`）时更新为该次主持升级的族人，从未升级时为 `None`。
- **二者均不随父系代际继承改变**：继承仅改写 `owner_id` / `generation` / `spouse_id`，修建/升级者字段随房屋永久保留——用于追溯宅邸最初修建人与历代升级功臣，与现户主解耦。
