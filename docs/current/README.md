...
> **版本**：v1.66.4（演进记录见 [./01-changelog.md](./01-changelog.md)）。
...
| 07 | [地形实现现状](design/07-terrain-implementation.md) | 产品视角的地形现状：两张地图、地表词汇、对玩法的影响、体验旋钮、已落地/规划/不做清单 |
| 08 | [房屋系统（产品视角）](design/08-housing-system.md) | 产品视角的房屋现状：五级形态与建材升级、自主决策链路、麦穗拍卖、体验旋钮、已落地/规划/不做清单 |
| 09 | [外部市场与动态价格（产品视角）](design/09-market-pricing.md) | 产品视角的市场现状：榷场互市、幂律定价、绝境兜底决策、黄金流失闭环、体验旋钮、已落地/规划/不做清单 |
...
| 11 | [决策引擎](tech/11-decision-engine.md) | 马斯洛六层、16 条分支、私有触发器、错峰节拍（附设计思路篇） |
...
| 18 | [水体渲染](tech/18-water-rendering.md) | 粒子水面运动模拟、平滑漫滩、游鱼（WebGL sink）、涉渡踏石 |
...
| 33 | [运行时水体求解器](tech/33-runtime-fluid.md) | 内核 PBF（深度平均域）粒子流体：★ v1.64.0 创世删除预设水系（播种恒空、水域格/水体/岸点不再产生）、降雨/泉涌补源（开放水循环的唯一水源）、受力与约束、地形坡度重力与 Manning 摩阻、FABS `Fluid` 快照协议、位精确存档、性能与水力侵蚀沉积（当前不触发） |
