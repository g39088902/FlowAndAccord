# 23. 🛠️ 前端开发实施指南 (`frontend`)

> **模块索引**：[← 返回 01-current.md 全景索引](../01-current.md) · 适用范围：新增前端 UI / 社会制度界面开发
> **关联文档**：[07-frontend-ui.md](./07-frontend-ui.md)（模块总览）· [21-ui-page-overview.md](./21-ui-page-overview.md)（UI 页面全景剖析）· [22-society-ledger-ui.md](./22-society-ledger-ui.md)（制度大盘界面实现）
...
> 以下为当前实际落地的快照结构（注意：`TransferRecordSnapshot.from/to` 为字符串化主体标识，非整数；`ClanSnapshot` 用 `member_ids` 而非家户 ID 列表；`RegionSnapshot.history_kings` 自 v1.12.0 起为 `HistoryKingSnapshot` 对象数组）。完整字段见 [12-ledger-system.md](./12-ledger-system.md)。
