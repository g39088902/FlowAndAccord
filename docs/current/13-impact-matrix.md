...
| 改动对象 | 必须同步改动 | 原因 |
|---|---|---|
| POI 数量变更 (countCamps 等) | `ecology/` 初始化 / `poi.rs` ID 段位 / `index.html` 顶栏面板文案 / `docs/current/02-ecology-poi.md` / `config.rs`+`config.js` / `render.js` POI 列表 | §4.7 POI 数量须同步多处 |
...
| 动态定价算法 (`market_unit_price`) | `world_snapshot.rs` 快照单价计算 / `ecology/` 现场交易计费 / `decisions/market.rs` / `config.rs` + `config.js` 超参 / 16-market-pricing.md | 幂律定价纯函数，全链条调用须一致 |
...
| 外部商贸资金流失记账 | `ledger/journal.rs` TransferReason::Market / `ecology/` debit 家户黄金转至 Void / `12-ledger-system.md` | 黄金单向流失沉淀，形成通缩调节机制 |
...
```
SimConfig (config.rs + config.house-upgrade-cost.js，共 240 字段)
    │  序列化
    ▼
