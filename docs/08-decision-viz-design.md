...
| 设计系统    | `style.css` + `09-ui-spec-and-ledger-design.md` §3.3 | 暗黑玻璃拟态、`.lineage-chip`、语义色板                                |
...
\| ▣ ③ 归属(1)            |  ┌─ b1 Branch 分支卡 ─┐                | - 源码锚点     |

\| ▣ ② 安全(4)            |  ┌─ b2 Branch 分支卡 ─┐  ← 一条纵列    | - config 键    |

\| ▣ ① 生理(3)            |  ┌─ b3 Branch 分支卡 ─┐  可拖拽换序    | - 关联子图     |

\| ─────────────          |  ┄ 分界线卡(①|②) ┄ ← 可拖动分层   | 跳转源码按钮   |

\| 说明 / 数据来源         |  ┌─ b4 Branch 分支卡 ─┐                |                |
...
* 中央：决策图视口（复用 `dag-view.js` 虚拟化），**13 张Branch 分支卡单列排布**——顶部评估入口、中部按优先级自上而下、底部状态机；**拖动卡片可在纵列内调整评估顺序**（顺序即优先级）；**卡片之间穿插 4 条「分界线卡片」，拖动分界线可重新划分"哪些需求在哪些层次"**（卡片层级 = 所在区间，联动左边框颜色、左栏计数、过滤与悬停高亮；全部本地持久化）。
...
* **决策树无连线**：13 张Branch 分支卡单列排布，**卡片上下位置即优先级**（命中即返回的短路语义），无需边连接；拖拽换序 = 调整优先级，突出 "排序" 而非 "拓扑"。
...
| 交互            | 方案                                                                                      |
| ------------- | --------------------------------------------------------------------------------------- |
| **纵列拖拽排序**    | 13 张Branch 分支卡单列排布；卡片 `pointerdown` 捕获 → 跟随指针移动 → 越过相邻卡片中心即交换位置（`branchOrder` splice + 单列重排）→ 松手吸附 |
...
| **固定锚点**      | 评估入口（◆）与状态机（⬡）为固定锚点，不参与排序，仅Branch 分支卡与分界线卡可拖动                              |
