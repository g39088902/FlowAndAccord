...
| :--- | :--- | :--- | :--- | :--- |
| **TA-05** ◐ | P0 样板闭环收尾：补 Chrome/Edge 真实存档 LOAD 链路（File System Access API）；高密拖动性能预算待用户取舍 | 中 | — | 植被系统存读档契约缺端到端证据；TA-06 同口径 LOAD 缺口并入本项 |
| **TA-09** | 完整场景样板：支脊山口与岩壁河谷实机交付（D-B1-8），构图方向已被用户接受、实机验收未开始 | 中 | 06 号阶段三 RiverCliff 注入（当前空壳 `accepted=false`） | D-B 地貌骨架停留在"代码交付但没人在真实种子上看过画面"；主路可读性、岩壁遮挡、树屋人穿插无验收 |
...
| **TB-04** | D-B 子特征注入器：T1 注入山脚湖/瀑布/密林，T2 注入牛轭湖/峭壁/河岸林带，按 seed 哈希概率 | 高 | 06 号阶段二；各子特征按 06 号 R.3 解锁 | **关键路径**——TA-09 岩壁、S4-X1~X4 专属景观、TB-06 瀑布全部依赖它 |
...
1. **无前置、性价比高**：TA-13（季节地表）、TA-10（台地美术，TB-02 已就绪）、TA-15（飘叶）、TA-05 LOAD 补测（只需 Chrome/Edge）。
2. **关键路径**：TB-04（阶段三注入器）→ 解锁 TA-09 岩壁样板 + S4-X1~X4 专属景观 + TB-06 瀑布。
...
**若只挑三项性价比最高**：TA-13（玩家一眼看出春夏秋冬）、TA-10（内核已就绪）、TA-05 LOAD 补测（关掉存读档最后一个 NOT_RUN）。**关键路径**：TB-04 是解锁一串下游任务的瓶颈，但工作量最大。
...
**2026-09-14：构图方向已获用户明确接受；实机验收未开始，TA-09 / D-B1-8 保持未完成。** 本记录是该样板的交付权威入口。
...
**前置核对基线**：源码 `77fffc7`，应用版本 1.50.50，地形生成器版本 8，存档结构版本 7。当前关键卡点：`geo/terrain.rs` 的 RiverCliff 仅有枚举与空几何注入，`accept_or_rollback` 保持 `accepted=false`。

**后续实机证据包（待采集）**：两场景各覆盖四季×四方位×低高俯角×远中近景（每场景 96 视图）；记录 seed/profile/tick/版本/提交/WASM 双副本 SHA256/配置/Chrome 版本/窗口与 DPR/相机参数。当前无合格岩壁种子或样板存档，不填写猜测值。物理门禁记录与视觉证据分开归档。
...
**已知偏差**：TA-04-8 首轮实测中高密·拖动场景最差视角超标（+5.8~+14.8 ms/帧），逐类分解显示增量主体在 `drawTerrainCell`（TB-01 fBm 起伏侧壁）与 `drawAccentEntity`（TA-11 石簇/芦草强化），TA-04 自身范畴约 +3~5 ms/帧达标。该场景处置（按场景修订预算 / 对 TB-01/TA-11 追加优化）**待用户取舍**。
...
**TA-05 遗留缺口（◐）**：Chrome/Edge 真实存档 LOAD 链路 NOT_RUN（预览浏览器无 File System Access API）；TA-06 同口径 LOAD 缺口并入。验收 URL：`http://localhost:3000/?seed=42&nogate=1&ta06=1`，植被观察台 `ta06-acceptance.js` 长期保留。TA-06/07 的完整验收记录与像素指标见证据包 `assets/ta06-evidence-2026-09-22/` 与 changelog v1.50.64/v1.50.65 条目。
