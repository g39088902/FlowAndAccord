...
> ★ **渲染架构决策（2026-09-17）**：**所有内容上 WebGL，不再使用 Canvas 2D**。双 Canvas（`sim-canvas-gl` + `sim-canvas`）**仅为本轮过渡形态**；装饰层与实体层将整体迁入同一 WebGL 管线，随后退役 2D 覆盖层与 `fallback-handler.js` 的 2D 回退分支（[31 号迁移方案 §8](./31-canvas-to-webgl-migration.md) 阶段三~五）。直接后果：**原 TA-08 的 Canvas 2D 遮挡近似策略（模型内排序 / 实体拆子项 / 屏幕空间兜底）全部取消**，遮挡改由 GPU 深度缓冲解决（TA-08 任务已删除视为完成，验收矩阵并入 [31 号 §8.5](./31-canvas-to-webgl-migration.md)）；TC-03 由"条件触发"改为**既定路线**；§6.2、§6.7、§9.3 口径已同步。
...
| TA-05 ◐ 2026-09-14 | P0 样板闭环：一株落叶树、一株落叶灌木、一块岩石，验证四季、四方位旋转与帧耗时——视觉/季相/受光/生命周期（除 LOAD）已通过；LOAD 存档链路 NOT_RUN 待 Chrome 补测；性能仅现状归档（未改代码无差分），高密拖动预算处置仍待用户取舍；**保持未完成状态** | P0，验收记录见 §11.4；方案与遗留事项见 [09 植被样板验证](./09-vegetation-verification.md) | 中 | TA-03、TA-04 |

**波次二 · 普及优化与组合地貌样板**

| 编号 | 任务 | 原代号 / 详见 | 难度 | 依赖 |
| :--- | :--- | :--- | :--- | :--- |
| TA-06 ✔ 验收通过（2026-09-22，v1.52.3） | 三类乔木轮廓（阔冠/疏冠/锥形常绿）与三类灌木变体（落叶多茎/花灌/低矮常绿），稳定哈希派生、不增持久化字段——**实现与静态/数值验证已落地**（v1.50.64：物种派生层 + 三乔木/三灌木骨架 + 花灌木花朵图元 + `render_bush.js` 前置拆分 + 冠幅/实高单一来源联动；`frontend-check` / `code-map-check` / `cross-doc-check` / `doc-link-check` / `bump-version --check` 全绿，临时断言 6 组全过）；**2026-09-22 WebGL 实况验收补做**：96 视图矩阵（四季×四方位×两俯角×三缩放）+ 6 变体近景 + 受光分离 + 生命周期 + 景观共用通道（57 对零失配）+ 高密世界全部取证，归档 `docs/plan/tech/assets/ta06-evidence-2026-09-22/`；长期调试工具**植被观察台** `ta06-acceptance.js`（调试监视器内入口）+ seed42 七株身份表 + 用户人工验收通过；**性能 A/B 按「已无性能问题」取消**（同 TA-07-10 口径）；**Chrome 真实存档 LOAD 链路仍 NOT_RUN**（须 Chrome/Edge File System Access API） | P1，详见 §6.3；方案与任务序列见 [TA-06 实施方案](../../../TA-06-TODO.md) | 中 | TA-05（建议按 §0.2 合并 LOAD 补测） |
| TA-07 ✔ **已取消视为完成**（2026-09-22） | 细节分级 LOD（远/中/近三档，阈值带滞回）、完整投影包围体剔除、局部几何模型缓存——**实现与静态/数值验证已落地**（新建 `accent-lod.js` 集中解析层：特征尺度 CSS px / 三档滞回判档 / 解析式屏幕 AABB / `kindBounds` 一级保守常数；模型层真值包围体 `bounds` + 分级几何 `farClusters`/`segTier`/`stoneMain`（`accentModelStyleVersion` 5→6）；装饰·灌木·草丛·阴影·景观·队列六处消费点收口，删净旧 `44/14` 启发余量与 `crownR×1.8` 经验包络；入队前两级剔除 + 深度项传 AABB 与锚点 `ex/ey`；临时断言 4 组全过——含 72 万投影点全部落在解析 AABB 内的保守性验证）。**取消口径（2026-09-22，经用户确认）**：本任务的原生驱动力是性能（高密世界下的入队/绘制成本），而**当前已无性能问题**；仅剩的 TA-07-9（Chrome 视觉验收）与 TA-07-10（六场景性能 A/B）随任务取消不再追补。**实现保留、不回退**——判档与剔除全在 CPU 侧、与渲染后端解耦，在默认 WebGL 路径下照常生效（见 §6.7 落地现状）。任务序列见 [TA-07 实施方案](../../../TA-07-TODO.md)（保留作历史记录） | M3、P1，详见 §6.7 | 中 | TA-06 |
| TA-08 ✔ 已删除视为完成（2026-09-17） | 遮挡边界实测**与验收矩阵**：重定向后仅剩测量工作、**无功能开发任务**，经用户确认删除并视为完成；原「模型内枝簇排序 / 实体拆有限子项 / 屏幕空间兜底」三策略**已取消**——遮挡改由全量 WebGL 深度缓冲解决（TC-03），验收矩阵并入 [31 号 §8.5](./31-canvas-to-webgl-migration.md)（原 TA-08-blocking-measurement.md 已删除） | P1，详见 §6.7 | 中 | TA-07 |
| TA-09 | 完整场景样板（待交付）：支脊山口与岩壁河谷，把地表色彩、岩层、成组植被、道路可读性、植被受光纳入同一幅画面；对应 D-B1-8，按本表前置后置交付，不阻塞 06 号阶段一代码交付及阶段二 | 旧 §9-1，详见 §4.4/§7 | 中 | TB-01、06 号阶段三 RiverCliff（TA-08 已删除视为完成） |
...
| TA-12 ✅ v1.50.53 | 世界坐标锁定的低对比度地表纹理（草斑/土纹），缩放旋转不重随机（确定性纹样模型 + 跨格预裁剪 + 共用受光色档 + 生命周期/性能收口；Chrome 验收四方位摘要不变、水系排除、吞吐零回退；性能增量超标如实记录待取舍；机制详见 `frontend/AGENTS.md` §1.1 terrain-texture.js 行） | S1-2 | 中 | — |
...
| TA-18 | 地形与装饰分块缓存、远景减量、相机旋转期性能测量，视觉缓存设内存上限 | M3，详见 §10.2/§11.3 | 中 | TA-07（任务已取消；实现仍落地，本项按**既有实现**为前置） |
...
| TC-03 ◐ **已定为实现路径**（2026-09-17） | GPU 渲染层迁移（★ 阶段一/二已落地：v1.50.77 地形层由 WebGL 承载；★ **架构决策：全量 WebGL、不再使用 Canvas 2D**，双 Canvas 仅为过渡形态）。剩余范围 = **装饰层 / 实体层整体迁入同一 WebGL 管线 + Canvas 2D 覆盖层与 2D 回退管线退役**，不再"条件触发"。方案见 [31 号迁移方案](./31-canvas-to-webgl-migration.md) §8 阶段三~五 | L3、P3 | 高 | 现行性能数据（§11.3） |

### 1.3 推荐实施波次

1. **波次一（当前）**：TA-01 → TA-02 → TA-03/04 → TA-05，先把一株植物的四季做透；不新增 FABS 字段、不等待内核任务。
2. **波次二**：TA-06 做普及（TA-07 的 LOD/剔除底座**已落地但任务已取消视为完成**，2026-09-22：性能驱动力随「已无性能问题」消失，见 §1 台账；原列于其后的 TA-08 亦已删除视为完成）；TB-01 可与之并行；随后 TA-09（支脊山口/岩壁河谷实机样板，另需 06 号阶段三 RiverCliff；构图草案可提前）、TA-10（待 TB-02）。
3. **波次三**：TA-11~TA-18 环境补全，与 TB-02/TB-03/TB-04 并行推进；纹理、季相地表（TA-12/13）与标注避让（TA-16）无内核依赖，可随时插入。
4. **波次四**：TB-05/TB-06、TC-01/TC-02 另立项排期；**TC-03（全量 WebGL 迁移）为既定路线**，按 31 号方案阶段三~五顺序推进（装饰层 → 实体层 → Canvas 2D 退役），其前置为现行性能数据（§11.3；原 TA-08 实测基线随任务删除不再采集）。
5. **依赖原则**：TC-03 不再是"条件触发"的应急项，而是渲染层的既定演进终点；波次三的装饰类任务（TA-11/TA-15/TA-17/TA-18）在排期时应优先考虑**避免在 Canvas 2D 侧继续追加一次性优化投入**，几何与筛选逻辑（模型层、LOD、遮罩）保持与渲染后端解耦，以便迁移直接复用。
...
| 跨图层「远物压近物」 | Canvas 2D 无深度缓冲，房屋/POI/族人各按数组原序整层绘制 | ✅ 已解决并持续扩展：`drawWorldEntities()` 统一深度队列，v1.50.11/v1.50.14/v1.50.20 起地形格、水系段、道路、侧壁也全部入队，按 `project3D().depth` 远 → 近绘制 | ★ 2026-09-17 后口径变更：新增世界实体在过渡期仍挂同一队列，但**遮挡的最终判据改为 GPU 深度缓冲**，队列在迁移后仅剩批次提交/透明排序职责；高树穿插遮挡由 TC-03 迁移解决（不再在 2D 侧做策略） |
...
- ◐ 选中对象始终可辨认、可点击：拾取逻辑仍按 `sim.*` 原序遍历，与绘制队列解耦。★ 2026-09-17 口径变更：树冠遮人**不再在 Canvas 2D 侧做局部淡化/轮廓**（原 TA-08 策略 C 已取消），改由 TC-03 迁移后在 WebGL 侧以「关深度测试描边 pass + 叶簇半透明」实现，验收矩阵见 [31 号 §8.5](./31-canvas-to-webgl-migration.md)。
...
**TA-09 交付分两步**：① 构图草案可提前制作，用于评审色板、主次地标、植被成组和道路留白，标为视觉目标，不勾选 D-B1-8；② 实机样板须等待 TB-01 支脊及 06 号阶段三 `RiverCliff` 分别通过门禁（原前置 TA-08 遮挡实测已删除视为完成）。固定两组真实 seed/profile 与同版本存档，记录应用/生成器版本、配置、窗口与 DPR，按 §11 覆盖四季、四方位、低高俯角、远中近景及模拟推进；核对岩壁不压盖近路、选中对象可辨、实体接地及帧预算。两场景全部通过才勾选 TA-09 / D-B1-8，物理门禁证据独立保留。岩壁尺度准入以 06 号 §5.4.D 为准，不用人工挑选的漂亮截图代替种子矩阵。★ 遮挡相关项（岩壁压路、选中对象可辨）的**判据以 [31 号 §8.5 验收矩阵](./31-canvas-to-webgl-migration.md) 为准**；若 TA-09 实机交付排在 TC-03 迁移之后，则直接按 WebGL 管线口径验收，不再补测 2D 基线。
...
素材由内置 image_gen 生成并修订。评审回复为“接受构图方向”，仅接受主次关系、色板与道路留白，不代表接受实机性能或物理尺度。首稿误带的农田、围墙与院地已移除；保留稿不作为已实现美术资产或渲染能力证明。
...
| TA-08 | ✔ 已删除视为完成（2026-09-17）：重定向后仅剩测量工作、无功能开发任务，验收矩阵并入 31 号 §8.5 | 无（遮挡判据改以 [31 号 §8.5](./31-canvas-to-webgl-migration.md) 为准） |
...
### 6.2 渲染路线：局部三维骨架（历史选择）→ 全量 WebGL（★ 2026-09-17 定为目标形态）

首期选择的是**局部三维枝干骨架 + 椭球叶簇 + Canvas 投影绘制**（TA-03 已落地）：有三维形态与世界空间受光，沿用当时的覆盖层画布、相机、实体队列与拾取逻辑。★ v1.50.77 起地形（含沙盘侧壁）已由 `frontend/js/webgl/` 绘制在底层 `sim-canvas-gl`，装饰/实体暂留 Canvas 2D 覆盖层 `sim-canvas`，两块画布经 `fallback-handler.js` 同帧合成——**这是过渡形态**。

★ **2026-09-17 架构决策**：**全量 WebGL、不再使用 Canvas 2D**。装饰层与实体层整体迁入同一 WebGL 管线（[31 号方案 §8](./31-canvas-to-webgl-migration.md) 阶段三~五），迁移完成后退役 2D 覆盖层与 2D 回退管线。本节的局部三维骨架、季相曲线、受光公式与 LOD 筛选**全部作为 CPU 侧几何与参数来源保留**，只更换绘制后端。

| 路线 | 能解决的问题 | 代价与适用范围 |
|---|---|---|
| 继续纯二维精灵，补季相和动态渐变 | 最快修复落叶、颜色和亮部方向 | 旋转视角仍像纸片，适合作为低画质表现 |
| 局部三维骨架与叶簇（**已落地**：TA-03） | 冬季枝形、旋转体积、分簇受光 | 已实现；其几何与模型层是 GPU 迁移的输入，绘制后端将被替换 |
| **全量 GPU 渲染管线（★ 目标形态）** | 深度缓冲、复杂遮挡、大规模实例植被、统一绘制后端 | 装饰/实体并入同一 WebGL 管线（31 号 §8 阶段三~五）；迁移后 2D 覆盖层与回退管线退役，不再存在「在 2D 覆盖层上叠加第三块画布」的问题 |

样板的四季、旋转与帧耗时验证仍有效；**后续不再以「性能超标或遮挡成为阻碍」为启动条件**——迁移已定，§9.3 由"评估"改为既定实施路线。
...
### 6.7 模块拆分、模型缓存与遮挡边界（TA-01 ✅ v1.50.23、TA-07 ✔ 已取消视为完成·实现保留；TA-08 已删除视为完成）
...
- `render_world.js`：继续负责跨实体队列（过渡期）；★ 2026-09-17 起**不再要求把地被、影子、枝干和冠层拆成有限子项**——该拆分（原 TA-08 策略 B）随 GPU 深度缓冲方案取消。
...
★ **TA-07 落地现状（v1.50.65；★ 2026-09-22 任务已取消视为完成——原生性能驱动力随「已无性能问题」消失，Chrome 视觉与性能验收不再追补，实现原样保留）**：
> **取消说明（2026-09-22）**：下列四项均为**代码既成事实**，在默认 WebGL 路径下照常生效（判档/剔除全在 CPU 侧，与渲染后端解耦；`accent-renderer.js` 只消费筛选结果）。取消的是「TA-07-9 Chrome 视觉验收 + TA-07-10 六场景性能 A/B」这两项**证据补采**工作，不是实现。四条防回退红线见 [frontend/AGENTS.md](../../../frontend/AGENTS.md)。
...
**遮挡边界口径（2026-09-17 定稿）**：现有根部足迹排序不能保证高树与坡地/房屋的真实穿插。★ **2026-09-17 起遮挡处理不再走 Canvas 2D 近似路线**（原「模型内按深度排枝簇 + 跨实体拆有限子项 + 屏幕空间兜底」三策略已取消；原「TA-08 只做实测」一步随任务删除不再执行，Canvas 2D 基线证据包不再采集）：

1. ~~**TA-08 只做实测**~~（✔ 2026-09-17 随任务删除取消——原计划的旋转切片证据包不再采集）；
2. **技术解由 GPU 深度缓冲承担**：装饰/实体整体并入同一 WebGL 管线（TC-03 / [31 号 §8](./31-canvas-to-webgl-migration.md) 阶段三~五），逐像素深度取代画家算法近似；
3. **选中遮挡补偿**改为 WebGL 侧「关深度测试描边 pass + 叶簇半透明」，验收矩阵见 [31 号 §8.5](./31-canvas-to-webgl-migration.md)；
4. **迁移后不再存在 2D 覆盖层**，「禁止叠加第三块画布」的约束随之升级为「不再有 2D 覆盖层」。

落叶和阴影仍按地表足迹处理，不能因为骨架三维化而重新引入半截入土；该口径在 GPU 迁移后依旧适用（贴地约束属几何事实，与绘制后端无关）。
...
### 9.3 TC-03 · 全量 WebGL 迁移（★ 2026-09-17 定为既定路线，L3）

★ **阶段一/二已落地（v1.50.77~v1.50.82）**：地形与沙盘侧壁已由 `frontend/js/webgl/` 绘制在底层 `sim-canvas-gl`；帧率已解限，不再锁定 60FPS。

★ **架构决策（2026-09-17）**：**所有内容上 WebGL，不再使用 Canvas 2D**。TC-03 不再是"条件触发"——装饰层（Tree/Bush/Boulder/RockCluster/GrassTuft/景观子图元/贴地投影）、实体层（族人/房屋/POI/道路/特效）整体迁入同一 WebGL 管线，迁移完成后退役 `#sim-canvas` 2D 覆盖层与 `fallback-handler.js` 的 2D 回退分支。方案与任务分解见 [31 号迁移方案 §8 阶段三~五](./31-canvas-to-webgl-migration.md)；遮挡问题的技术解由 GPU 深度缓冲承担（TA-08 任务已删除视为完成，验收矩阵并入 31 号 §8.5）。

实施建议：先做「一片装饰群 + 一座房屋 + 一名可选中人物」并入现有 WebGL 地形管线的切片原型，对比 Canvas 2D 覆盖层的帧耗时、内存、镜头与命中行为后按阶段三~五推进。复用局部几何、材质、季相定义与 LOD 筛选（这些全部留在 CPU 侧），不绑定具体三维库；**拾取与标签层是主要回归风险点**，须同步验证。

**长期退出标准**：景观变化能够解释资源、迁徙或聚落历史；同一存档回放得到一致的世界事实；迁移后的渲染层保留低画质档位（**同一 WebGL 管线内的 LOD/质量档，而非独立 2D 降级路径**）和原有观察交互。
...
- ◐ **最小遮挡样板**（原 TA-08，✔ 2026-09-17 删除视为完成——重定向后仅剩测量工作、无功能开发任务，Canvas 2D 基线不再采集）：一面岩壁、一座房屋、一棵树、一段道路、一个族人；验收判据保留为本条目，★ 判据以 [31 号 §8.5 验收矩阵](./31-canvas-to-webgl-migration.md) 为准，在迁移后 WebGL 管线上验收（Chrome 中固定种子/存档、窗口与 DPR，覆盖四方位、低高俯角、远中近景、四季及模拟推进）。岩壁不能错误盖住近处道路，树冠不能遮断选中信息，实体不能悬空或入土。遮挡的实现在 TC-03（31 号 §8 阶段三~五），不在本项内。
...
### 11.4 Accent 专项验收（TA-01~TA-07；TA-08 已删除视为完成）
...
| 分级与剔除 | 远/中/近三档可辨且不混淆；缩放穿越阈值零闪烁；视口边缘无弹跳、无半株突现、**漏画 0 例**（TA-07 追加，见 §6.7 与 [TA-07-TODO](../../../TA-07-TODO.md) §6；★ 2026-09-22 起该项**不再作为独立验收补做任务**——TA-07 已取消视为完成，实现与静态/数值验证成果保留） |
| 生命周期 | 暂停季相与飘叶冻结；读档/回溯可重建；换世界缓存结果与冷重建一致、不串入旧数据；动态光开关往返正常；高倍速下不频闪；**LOD 滞回档位随世界对象换新而自然失效**（TA-07 追加） |
| 性能 | 按 §11.3 同设备同负载对照，新增绘制 p95 增量目标 ≤ 3 ms；首轮实测（TA-04-8）：3/4 场景达标，高密·拖动最差视角超标且主体归因非 TA-04 范畴，处置待用户取舍（§11.3）。★ TA-07 六场景 A/B（P1~P6）**已随 TA-07 任务取消（2026-09-22）不再执行**——取消理由即「当前已无性能问题」；TA-06 本轮性能 A/B 同样按此口径取消（见 §11.4 TA-06 验收补做记录） |

**TA-05 验收记录（◐ 2026-09-14，内置预览浏览器 + `?nogate=1`，种子 42，v1.50.52，HEAD `1ef2660`，零代码改动）**：三对象 = 落叶 Tree#0 / 落叶 Bush#77 / Boulder#44（常绿对照 Tree#36，高地补充 Tree#12）；夹具以同 id/kind/scale/rotation/tint 克隆改坐标走生产管线，选点过 `LandscapeMask.accentHidden` 判据（夹具方法见 09 号 §3）。①四季与边界 PASS：同株春芽(芽量0.64)/夏满冠/秋金黄/冬枝可辨，冬落叶树叶量 0.03~0.04（规格 0~0.05）、常绿 0.94 保持轮廓，四季交界 1−ε/ε 两侧连续无跳变，先变色后减叶，岩石材质不随季相变色，`yearPhase` 回环（春季中部）与季分箱边界分别覆盖（26 点 × 4 对象数值采样）。②96 视图主矩阵（四季×四方位×两俯角×三缩放，细节三档全覆盖）+ 受光分离（azimuthOffsetDeg 0/90/180/270 投影翻转）+ 光近视线退化 + 视口四边 + 整圈旋转扫掠 PASS，无悬空/入土/裁断/跳层。③生命周期：真实快照推进、暂停/继续、REWIND 精确回滚、RESET 同种子冷重建一致、换种子不复用旧世界、1024x 无频闪、交互/共享模型、动态光开→关→开 全部 PASS；**LOAD 文件存档 NOT_RUN**（预览浏览器无 File System Access API，须 Chrome 补测）。④性能：未改渲染代码无 A/B 差分，现状绝对耗时已归档（初始世界统一队列 p95 14.6~16.7ms；高密 16.0~27.9ms 方差大，面板隐藏 + Worker 驱动产帧 + 1024x 演化扰动口径受限，吞吐不可靠不产结论）——TA-04-8 高密·拖动超标处置仍待用户取舍（§11.3）。**TA-05 保持未完成**：余 LOAD 补测 +（如需严格判定）Chrome 可见面板环境性能复测。方法论与遗留事项：[09 植被样板验证](./09-vegetation-verification.md)。
...
**TA-06 验收台追加（◐→◐ 2026-09-22，v1.52.1，纯前端表现层调试工具，零持久化字段、零内核/快照变更）**：① **新建 `frontend/js/ta06-acceptance.js`（318 行，仅 `?ta06=1` 激活，无参数 IIFE 入口直接退出零 DOM 零 rAF）**——左上浮动面板含季节锁定（自动/春/夏/秋/冬，季中 u=0/.25/.5/.75，`Object.defineProperty` 覆盖 `sim.currentSeason/seasonProgress` 读取 + 锁植被季相/SimLighting 相位/地形烘焙色）、光向 0/90/180/270 + 动态光 checkbox（与原生 `#chk-dynamic-light` 双向联动）、相机 rotZ 四方位 / rotX 缓0.72/标1.05/陡1.30 / 缩放 0.6~6、7 株样板一键飞往（zoom3、注视冠部 lift=bounds.zMax*0.45）、物种 DOM 标注层（rAF；阔/疏/锥·灌/花/青+id，样板带★，zoom<0.8 淡出，视口外隐藏，**被遮罩株过滤**）、4Hz 读数行（季节/progress/u/zoom/rotZ/光向 az·elev/视口内植被数）；`index.html` 在 auction-ui.js 之后注册。② **定稿 seed 42 七株身份表**（全部 `LandscapeMask.accentHidden=false` 复核）：tree_broad #3 @(-4.3,-126.2) s0.715 / tree_sparse #22 @(106.6,-122.6) s0.955 / tree_conifer #8 @(25.1,218.2) s1.117 / bush_multiStem #67 @(-98.5,102.9) s0.992 / bush_flowering #72 @(-14.4,-164.5) s1.125（春花朵首选，白/黄花点）/ bush_flowering #80 @(163.0,87.9) s1.172（含 1 粉花点）/ bush_lowEvergreen #74 @(-17.0,82.2) s1.105；补充株 #60（粉花点最多 3）/#68/#81（2 粉）；废弃被遮罩株 #2/#0/#31/#78/#65/#70/#62（原点附近营地/POI/道路保护区，可作遮罩对照）。③ **内置浏览器自测全过**（`?seed=42&nogate=1&ta06=1`，GL 全量管线）：GL Terrain/Accent/Shadow 三 Pass 初始化成功、Worker 开局种子 42、控制台零 JS 错误零 404；季节锁定读数精确（春 u=0、冬 u=0.750）；飞往定位精确（#8 标签居中）；春 zoom8 #72 冠面 3 个浅色花点清晰；冬季 #72/#69 秃枝无叶无花、#3 锥形常绿与 #74 低矮常绿保叶冷暗绿、草枯褐、石头不变；秋季落叶株黄褐变色、常绿树仍深绿；光向 0/90/180/270 花色明暗翻转（az0 最亮）；rotZ90/rotX陡/zoom6/标注开关均生效；动态光 checkbox 与原生联动验证通过；`?accentgl=0` Canvas 对照路径正常；无参数页 `?nogate=1` 无面板/无标注层/零错误/零 404（零影响确认）。④ **花朵视觉结论**：GL 与 Canvas(?accentgl=0) 两路径花点颜色逐值一致、同源 `accentLitFill`（Canvas fillStyle 与 GL sink `_litFinal` 数值相同）；花色为白(246,242,232)/淡粉(226,190,186)/淡黄(238,224,168)低饱和三色板，背光偏暗是受光设计非缺陷；花点屏半径 <`accentFlowerMinPx`(1.2) 省略，zoom≥~3.3 稳见（特写 6 最清晰）；#72 只有白/黄花点（粉色花位被 v=0.29 抑制），淡粉花点最多是 #60（zoom5 下 3 粉）。⑤ **景观抽查实测（TA-06-9 取证）**：seed42 `LandscapeModel.groups()`=18 组、48 株景观灌木（deciduousBush 30/evergreen 10/floweringBush 8）；8 株原生花灌木中 #62/#65/#70 被 `accentHidden` 隐藏，未遮罩为 #60/#68/#72/#80/#81。⑥ **仍 NOT_RUN（故本项保持 ◐）**：Chrome/Edge 真实建档→存档→读档 LOAD 链路（`?nogate=1` 依法不能替代）、§11.3 性能 A/B 四场景（P1~P4，无变体基线端缺失）、96 视图全矩阵由验收人按 [TA-06-TODO §9](../../../TA-06-TODO.md) 走查。人工验收指南见 [TA-06-TODO.md §9](../../../TA-06-TODO.md)；验收 URL：`http://localhost:3000/?seed=42&nogate=1&ta06=1`。
...
**TA-06 验收补做记录（◐ 2026-09-22，v1.52.0，内置预览浏览器 + `?nogate=1`，默认 WebGL 路径）**：按「WebGL 技术实况」重做验收——本轮先确认**装饰几何的单一来源仍是 Canvas 绘制函数**（`render_accents.js` / `render_bush.js` / `render_grass.js` 的绘制函数在 GL 下逐帧执行，仅出口由 `ctx` 换成 `WebGLAccentLayer` sink；`accent-renderer.js` 只做三角化 + 解析式 AA + 深度对齐，**不复制几何与受光公式**），故 TA-06 的画面成果（三乔木轮廓 / 三灌木变体 / 花朵图元 / 冠幅·实高单一来源 / 季相 profile 单一入口）**在默认路径上本已生效**，本轮工作是**取证**与**文档口径修正**，非重新接入。① **页内数值断言 10 项全 PASS**（`?seed=42`）：物种分布 ±3%（乔木 0.4135/0.3360/0.2505、灌木 0.5555/0.2350/0.2095）、非植被三种 `undefined`、`multiStem` 40 个不同 id 与旧基准逐值一致、真值包围体 ≤ 一级粗剔常数（300 id × 5 kind 零越界）、花位 9 点上限与三色槽位均匀且非花灌木 `flowers === null`、**`L#` 景观通道与 accent 通道同 seed 物种逐对一致（57 对失配 0）**、`resetCache()` 暖冷缓存一致、四季叶量（落叶冬 0.03/0.04 vs 夏 1.0；常绿全年 ≥0.94）、花历（花灌木春 0.855、其余三季 0）、三档真实覆盖随缩放单调。② **视觉矩阵 96 视图**（种子 8 `river_valley_v1`，四季 × 四方位 × 两俯角 × 三缩放）+ **8 张拼图** + **6 变体近景 12 张**（春/冬同机位，核对冬季裸枝与常绿轮廓）+ **受光分离 8 张**（A 固定光向转相机：四张回读 `lightDir` 完全相同而最亮质心大幅移动 ⇒ 亮部不黏屏幕固定位置；B 固定季相变光向：`azimuthOffsetDeg` 0/90/180/270 + `SimLighting.markDirty()` 后回读方位角每档 +90°、四张截图 md5 全不同）+ **生命周期 3 张**（A→B→A 换世界：seed8 `river_valley_v1` → seed1 `mountain_pass_v1` → seed8）+ **景观通道 3 张**（Wood 12 / Berry 7 / Water 4 子图元）+ **高密世界 2 张**（`grassland_plain_v1` 1025 装饰）。逐张像素指标（greenRatio / brownRatio / meanRGB / 最亮质心）见证据包 `metrics/matrix.json`；季节可辨性由 meanRGB 夏→冬显著转暗、秋褐峰值与冬褐谷值、以及数值层叶量扫掠共同支撑（**greenRatio 不是落叶程度的干净代理**——地形基底本身偏绿，如实说明）。③ **未做项**：真实 Chrome File System Access **LOAD 链路 NOT_RUN**（预览浏览器无该 API，建议与 TA-05 缺口合并补测）；**性能 A/B（原 TA-06-11）经用户确认取消**（理由「当前已无性能问题」，同 TA-07-10 口径）；TA-06-1 Chrome 无变体截图基线仍为缩减态（本轮矩阵不可当作变体 A/B 基线）。④ **已知偏差（用户决定不改代码，仅记录）**：WebGL 阴影代理 `shadow-pass.js` 的冠簇竖直压扁取常数 0.85 而非模型 `sk.crownSquash`（0.72/0.62/0.55），且 `sk.footprintR` 在 GL 下随 `render_shadows.js` 早退而失去视觉作用——阴影图属「保守代理近似」，0.85 偏大反而保证不漏影；TA-06-8 的「阴影冠幅与实体一致」在 WebGL 侧由代理读 `sk.crownR`/`sk.trunkH` 满足，仅扁压系数非同源。⑤ **门禁与交付**：纯文档 + 证据包（零代码改动），按根 AGENTS.md §4.0.1 仅文档变更例外——不升版、不重编译 WASM；证据包归档 [assets/ta06-evidence-2026-09-22/](./assets/ta06-evidence-2026-09-22/report.md)（42 文件 / 约 5.8 MB：`manifest.json` + `report.md` + `metrics/` + `visual/`）。**TA-06 仍记 ◐**（LOAD 未验证 ⇒ 不勾选完成）。

**TA-07 取消记录（✔ 2026-09-22，纯文档，零代码改动）**：见 §1.2 台账 TA-07 行与 §6.7 取消说明。要点：① **取消理由是性能驱动力消失**（「当前已无性能问题」），而非实现有问题；② **实现与静态/数值验证成果全部保留**（`accent-lod.js` 六处消费点唯一口径、模型层 `bounds`/`farClusters`/`segTier`/`stoneMain`、入队前两级剔除与深度项传 AABB/锚点），且**在当前默认 WebGL 路径下照常生效**（判档与剔除全在 CPU 侧、与渲染后端解耦，`accent-renderer.js` 只消费筛选结果）；③ 未做的 **TA-07-9（Chrome 视觉验收）与 TA-07-10（六场景性能 A/B）不再追补**，四条防回退红线（口径唯一 / 入队端与绘制端同一 AABB / 不改深度与画序 / 不改受光与季相公式）继续有效，见 [frontend/AGENTS.md](../../../frontend/AGENTS.md)；④ 本次同步复核（顺带）：一级保守常数与模型真值包围体在 300 id × 5 kind 零越界、三档真实覆盖随缩放单调（zoom 0.6 far 60 / zoom 8 near 65）——复核数据并入 TA-06 证据包 `metrics/fixture-assertions.json`。
