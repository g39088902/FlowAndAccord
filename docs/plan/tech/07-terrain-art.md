...
> 本文为规划态：D-A 基础（Tree/Boulder/Bush）已落地；RockCluster/GrassTuft、连续季相、局部三维骨架、S3 样板、标注避让与季节地表材质均尚未实现。
...
- **与 [动态季节光照方案](../../current/tech/17-seasonal-lighting.md)**：该方案负责**光**（光向/强度/色温/阴影/天空氛围），本文负责地表与植被**反照率**；两侧共用同一季节相位（`SimLighting.phase()`）与同一套 `cell.color` 失效机制，**严禁各自维护一套季节时钟**。光照方案先落地时，季节调色直接消费其光相。
...
| 截图显示秋季，但地表仍以大片绿色为主 | 着色函数不含季节因子 | ◐ 树冠已有三档季相（v1.50.21）；**地表** `computeElevationColor` 仍未消费 `sim.currentSeason`，且 `cell.color` 被 `rustworld.js` 缓存，改色必须显式失效 | 季节地表材质（TA-13）；连续植被季相（TA-02） |
...
| 季节地表 | ⏳ 未实现（地表着色不读季节）；树冠三档季相已落地，连续化见 §6.3 | 春嫩、夏深、秋枯、冬（覆雪待气候契约），规则见 §4.3 |
...
- **两条季节链路分开做、共用时钟**：TA-13 负责地表反照率（草绿→枯黄→覆雪预留），TA-02/§6.3 负责植被季相；二者都消费 `SimLighting.phase()`，禁止自建第二套季节时钟。
...
将现有 `SimTreeTint` 原位扩展为植被季相的唯一生产者，增加拟议接口 `sample(accent, sim, profile)`；迁移期保留 `tint()` 兼容调用，迁移完成后统一清理，避免两套年历并存。输入继续采用 `currentSeason` 与 `seasonProgress`，缺字段时沿用现有回退。**不要用已平滑的光相决定落叶数量**，光照仍直接读取 `SimLighting` 的当前状态。年相位定义与连续性要求以 §4.3 为唯一准则。
...
| 季节颜色、水面微动 | ◐ 季节/气温事实已随快照下发（`season`、`season_progress`、`temperature`），树冠三档季相已落地；地表视觉映射未做 | 截图验收固定视觉时刻；暂停与回放时不漂移；改色须让 `cell.color` 缓存失效 |
...
- ⏳ 覆盖远/中/近三档镜头、四季，以及初始聚落和高密度聚落（四季地表尚无差异，验收需等 TA-13/TA-02）。
