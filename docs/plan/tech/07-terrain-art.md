...
> ★ **渲染架构决策（2026-09-17）**：**所有内容上 WebGL，不再使用 Canvas 2D**。双 Canvas（`sim-canvas-gl` + `sim-canvas`）**仅为本轮过渡形态**；装饰层与实体层将整体迁入同一 WebGL 管线，随后退役 2D 覆盖层与 `fallback-handler.js` 的 2D 回退分支（[31 号迁移方案 §8](./31-canvas-to-webgl-migration.md) 阶段三~五）。直接后果：**原 TA-08 的 Canvas 2D 遮挡近似策略（模型内排序 / 实体拆子项 / 屏幕空间兜底）全部取消**，遮挡改由 GPU 深度缓冲解决（[TA-08 已重定向](../../../TA-08-blocking-measurement.md)）；TC-03 由"条件触发"改为**既定路线**；§6.2、§6.7、§9.3 口径已同步。
...
| TA-08 ◐ 已重定向（2026-09-17） | 遮挡边界实测**与验收矩阵**：一树一屋一人旋转切片实测（前置诊断）；原「模型内枝簇排序 / 实体拆有限子项 / 屏幕空间兜底」三策略**已取消**——遮挡改由全量 WebGL 深度缓冲解决（TC-03）。本任务收缩为测量 + 验收标准携带者，详见 [TA-08](../../../TA-08-blocking-measurement.md) | P1，详见 §6.7 | 中 | TA-07 |
| TA-09 | 完整场景样板（待交付）：支脊山口与岩壁河谷，把地表色彩、岩层、成组植被、道路可读性、植被受光纳入同一幅画面；对应 D-B1-8，按本表前置后置交付，不阻塞 06 号阶段一代码交付及阶段二 | 旧 §9-1，详见 §4.4/§7 | 中 | TB-01、TA-08 实测、06 号阶段三 RiverCliff |
...
| TC-03 ◐ **已定为实现路径**（2026-09-17） | GPU 渲染层迁移（★ 阶段一/二已落地：v1.50.77 地形层由 WebGL 承载；★ **架构决策：全量 WebGL、不再使用 Canvas 2D**，双 Canvas 仅为过渡形态）。剩余范围 = **装饰层 / 实体层整体迁入同一 WebGL 管线 + Canvas 2D 覆盖层与 2D 回退管线退役**，不再"条件触发"。方案见 [31 号迁移方案](./31-canvas-to-webgl-migration.md) §8 阶段三~五 | L3、P3 | 高 | TA-08 实测基线 |

### 1.3 推荐实施波次

1. **波次一（当前）**：TA-01 → TA-02 → TA-03/04 → TA-05，先把一株植物的四季做透；不新增 FABS 字段、不等待内核任务。
2. **波次二**：TA-06 → TA-07 做普及与性能底座，随后 TA-08（遮挡实测与验收矩阵）；TB-01 可与之并行；随后 TA-09（支脊山口/岩壁河谷实机样板，另需 06 号阶段三 RiverCliff；构图草案可提前）、TA-10（待 TB-02）。
3. **波次三**：TA-11~TA-18 环境补全，与 TB-02/TB-03/TB-04 并行推进；纹理、季相地表（TA-12/13）与标注避让（TA-16）无内核依赖，可随时插入。
4. **波次四**：TB-05/TB-06、TC-01/TC-02 另立项排期；**TC-03（全量 WebGL 迁移）为既定路线**，按 31 号方案阶段三~五顺序推进（装饰层 → 实体层 → Canvas 2D 退役），其前置为 TA-08 实测基线+现行性能数据。
...
| 跨图层「远物压近物」 | Canvas 2D 无深度缓冲，房屋/POI/族人各按数组原序整层绘制 | ✅ 已解决并持续扩展：`drawWorldEntities()` 统一深度队列，v1.50.11/v1.50.14/v1.50.20 起地形格、水系段、道路、侧壁也全部入队，按 `project3D().depth` 远 → 近绘制 | ★ 2026-09-17 后口径变更：新增世界实体在过渡期仍挂同一队列，但**遮挡的最终判据改为 GPU 深度缓冲**，队列在迁移后仅剩批次提交/透明排序职责；高树穿插遮挡由 TA-08 实测取证、由 TC-03 迁移解决（不再在 2D 侧做策略） |
...
- ◐ 选中对象始终可辨认、可点击：拾取逻辑仍按 `sim.*` 原序遍历，与绘制队列解耦。★ 2026-09-17 口径变更：树冠遮人**不再在 Canvas 2D 侧做局部淡化/轮廓**（原 TA-08 策略 C 已取消），改由 TC-03 迁移后在 WebGL 侧以「关深度测试描边 pass + 叶簇半透明」实现，验收矩阵见 [TA-08 §2](../../../TA-08-blocking-measurement.md)。
...
**TA-09 交付分两步**：① 构图草案可提前制作，用于评审色板、主次地标、植被成组和道路留白，标为视觉目标，不勾选 D-B1-8；② 实机样板须等待 TB-01 支脊、TA-08 遮挡实测及 06 号阶段三 `RiverCliff` 分别通过门禁。固定两组真实 seed/profile 与同版本存档，记录应用/生成器版本、配置、窗口与 DPR，按 §11 覆盖四季、四方位、低高俯角、远中近景及模拟推进；核对岩壁不压盖近路、选中对象可辨、实体接地及帧预算。两场景全部通过才勾选 TA-09 / D-B1-8，物理门禁证据独立保留。岩壁尺度准入以 06 号 §5.4.D 为准，不用人工挑选的漂亮截图代替种子矩阵。★ 遮挡相关项（岩壁压路、选中对象可辨）的**判据以 [TA-08 §2 验收矩阵](../../../TA-08-blocking-measurement.md) 为准**；若 TA-09 实机交付排在 TC-03 迁移之后，则直接按 WebGL 管线口径验收，不再补测 2D 基线。
...
| TA-08 | ◐ 已重定向（2026-09-17）：遮挡近似三策略取消，本项收缩为**旋转切片实测 + 验收矩阵**；尚未开始实施 | 按 TA-05→TA-06→TA-07→TA-08 波次完成相关底座与 §11.4 验收，不能借本次草案标为完成；实测结论作为 TC-03 迁移输入 |
...
### 6.7 模块拆分、模型缓存与遮挡边界（TA-01 ✅ v1.50.23、TA-07、TA-08 ◐ 已重定向）
...
**遮挡边界必须实测**：现有根部足迹排序不能保证高树与坡地/房屋的真实穿插。★ **2026-09-17 起口径变更**——遮挡处理不再走 Canvas 2D 近似路线（原「模型内按深度排枝簇 + 跨实体拆有限子项 + 屏幕空间兜底」三策略已取消，见 [TA-08](../../../TA-08-blocking-measurement.md)）：

1. **TA-08 只做实测**：先测一树一屋一人的旋转切片，建立问题证据包与 before/after 基线，作为迁移输入；
2. **技术解由 GPU 深度缓冲承担**：装饰/实体整体并入同一 WebGL 管线（TC-03 / [31 号 §8](./31-canvas-to-webgl-migration.md) 阶段三~五），逐像素深度取代画家算法近似；
3. **选中遮挡补偿**改为 WebGL 侧「关深度测试描边 pass + 叶簇半透明」，验收矩阵沿用 TA-08 §2；
...
★ **架构决策（2026-09-17）**：**所有内容上 WebGL，不再使用 Canvas 2D**。TC-03 不再是"条件触发"——装饰层（Tree/Bush/Boulder/RockCluster/GrassTuft/景观子图元/贴地投影）、实体层（族人/房屋/POI/道路/特效）整体迁入同一 WebGL 管线，迁移完成后退役 `#sim-canvas` 2D 覆盖层与 `fallback-handler.js` 的 2D 回退分支。方案与任务分解见 [31 号迁移方案 §8 阶段三~五](./31-canvas-to-webgl-migration.md)；遮挡问题的技术解由 GPU 深度缓冲承担（[TA-08](../../../TA-08-blocking-measurement.md) 仅保留实测与验收矩阵）。
...
- ⏳ **最小遮挡样板**（TA-08）：一面岩壁、一座房屋、一棵树、一段道路、一个族人；在 Chrome 中固定种子/存档、窗口与 DPR，覆盖四方位、低高俯角、远中近景、四季及模拟推进。岩壁不能错误盖住近处道路，树冠不能遮断选中信息，实体不能悬空或入土。★ 本条**判据以 [TA-08 §2 验收矩阵](../../../TA-08-blocking-measurement.md) 为准**：迁移前在 Canvas 2D 现状采基线，迁移后在 WebGL 管线复测；遮挡的实现在 TC-03（31 号 §8 阶段三~五），不在本项内。
