...
| F 归档 | 结果写入 docs/current/11-changelog.md，bump-version | 验收完成 | 优化关闭，进入现状文档 |
...
4. **取消按需求层级再错峰**：保持固定 dt、既有个人决策相位、层级评估与结算时序；不能通过延后需求评估取得性能收益。与新增行为的验证边界见[融合设计 §7](./20-integration-contracts.md#7-事实快照与验证)。
