...
| 七 · 独立模板 | 草原 S7-01/02 已完成，S7-03 起待交付；半坡林地、连续河谷待交付 | 草原需一；林地需密林规则（可由 S7 先交付）；河谷需二 | 是 / 中 | §4.1/4.2、阶段七 TODO |
...
| 9 | 河谷聚落 | 上游河道、谷底冲积带、两侧连续山坡 | 水、耕地、道路都沿谷底集中 | 新增 profile `river_valley_settlement_v1` | ✅ 内核已交付（v1.50.48 S7-06 骨架 + v1.50.49 S7-07 主河水系/浅滩/水池，探针 60 种子 §1.4 门禁 0 违例，见 §4.3）；`random` 候选待 S7-10 全链路验收 | §4.3 |
...
| 16 | 半坡林地 | 单侧缓坡山体、坡面林地、坡脚泉溪与局部露岩（辅助至多二） | 林地纯视觉可穿越、坡度产生真实慢行；房屋沿坡脚与林缘形成带状聚落 | 新增 profile `hillside_woodland_v1` | ✅ 内核骨架已交付（v1.50.46，S7-04；探针 124 种子 §1.4 门禁 0 违例）；林地装饰已交付（v1.50.47，S7-05：梯级密林 + 取水点隔离，见 §4.2 末）；`random` 候选待全链路验收 | §4.2 |
...
> **排期口径（2026-09-13）**：3～16 号模板已全部纳入文首 R.3 阶段计划；表中「排期：阶段 X」指实施窗口与依赖关系，开工仍按「状态机」A→F 逐张过门禁。阶段编号是推荐顺序而非强制串行（R.4），前置就绪即可提前开工；阶段七按专项 TODO 推进；草原骨架已交付，视觉验收不通过时保留显式 profile 供调试，禁止加入 random，不撤销已有实现记录。
...
| D-C：高级装饰 | 泉水景观群、资源区景观群、标注避让 | D-B1；特定景观群依赖相应注入器，不等待 R0/全部 D-B2 | 中期 M3 | ⏳ 未实施 |
