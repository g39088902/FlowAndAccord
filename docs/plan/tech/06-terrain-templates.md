...
> **定位**：本文是地图模板、地形要素与玩法约束的规划权威；已实现机制以 [14 地形与路网](../../current/tech/14-terrain-and-network.md)为准，材质、光照、素材、遮挡与视觉性能见 [07 地形美术](./07-terrain-art.md)。统一生成器的可执行技术设计见根目录 [TERRAIN_FIELD_COMPILER_DESIGN.md](../../../TERRAIN_FIELD_COMPILER_DESIGN.md)。
> **复核基线**：2026-09-24；源码 `TERRAIN_GENERATOR_VERSION=21`、存档 `SAVE_FORMAT_VERSION=7`、FABS `FORMAT_VERSION=4`。文中旧阶段的版本号是历史验收基线，不得当作当前源码版本；实施按变更性质判断版本递增，不照抄旧稿中的目标版本号。
> **阅读入口**：[R.0 统一生成器方向](#r0-统一生成器方向) → [R.1 当前状态](#r1-现状快照)（含 [R.1.1 未完成步骤速览与必要性](#r11-未完成步骤速览与必要性)）→ [R.3 阶段计划](#terrain-roadmap) → §2.1 模板清单 → §5/§6 施工契约 → §18 验收。近期任务明细见 [TODO](../../../TODO.md)。阶段二（STAGE2-1~8）与阶段七（S7-01~10）均已收官，专项任务文档随收官归档删除，收口证据见 [14 号 §8.2](../../current/tech/14-terrain-and-network.md)；阶段七探针门禁基线与固定种子验收表收编于 §18.8，交付证据见 [changelog](../../current/01-changelog.md)。
...
> **本节是全文唯一的排期权威**（2026-09-12 重组，2026-09-24 增加 UGC 迁移波次）：整合原 §2.2 实现路线、§5.9 推荐提交顺序、§5.1 提交组框架与 §19 景观对接的排期信息，原位置只保留细节契约或指针，避免多处维护漂移。排期原则：**从易到难、按依赖解锁**；现有 D-B1 / R0 / D-B2 / P1 仍是历史模板交付边界，统一生成器 UGC-00~05 是后续替换生成入口的独立迁移边界。每个边界只改变一种物理或基础设施能力，禁止把多个波次合并为一次大改。

### R.0 统一生成器方向

> **本节决定后续地形代码的形态；详细接口、伪代码、体素后端和小任务执行卡见根目录 [TERRAIN_FIELD_COMPILER_DESIGN.md](../../../TERRAIN_FIELD_COMPILER_DESIGN.md)。**

当前“少量地图骨架 × 地貌子特征 × 景观风格”继续保留，但 profile 不再是生成算法的分支入口。目标结构为：

```text
seed + TerrainRecipe
  → Field Graph（高程、地层、硬度、降水、湿度、汇流、沉积、地下水）
  → 固定顺序地理过程（结构变形、侵蚀、汇流、沉积、水位、土壤与植被）
  → 通用约束评估（连通、可建、浅滩、绕行、取水、草地/沙地因果）
  → Terrain semantics（GeoCell / WaterBody / Feature / Corridor）
  → HeightfieldBackend（迁移兼容）或 VoxelBackend（目标后端）
```

硬约束：

1. 新地图优先新增 recipe、节点参数和玩法约束；不得为每个地图新增一套 `generate_*_profile()`。
2. 只有新物理过程才新增通用算子；台地、盆地、冲积扇和火山湖应由可复用场算子与水文过程组合。
3. 旧 profile 作为存档和回归 alias 保留，直到对应 recipe 通过固定 seed 矩阵和有效世界门禁。
4. 体素是目标底层表示；第一阶段通过 `HeightfieldView` 继续向现有 `GeoCell`、路网、房屋和生态提供二维查询。
5. 统一生成器不进入 tick 热路径；静态体素由 seed、recipe hash 和生成器版本重建，动态修改另存 chunk delta。

迁移任务使用 `UGC-00`～`UGC-12`：基线与字段诊断 → Field IR/高度场后端 → 通用水文过程 → 全部静态模板迁移 → VoxelBackend → FABS/WebGL → 地层与结构 → 静态地下水 → 不确定性与剖面诊断 → 多层寻路 → 外部数据适配 → 专业物理扩展。每个任务的输入、修改文件、禁止范围、稳定错误码和退出条件以根目录设计文档为准。
...
| 阶段五 / 六 · P1 | 台地 `plateau_v1`（v1.50.54 TB-02）、盆地 / 冲积扇 / 火山湖（v1.50.55 TB-03）**已发布并入 random**（v1.50.68 起 8 路，§5.6.1）；火山湖独立规格已回填 §5.6 | TB-04 盆地群峰环抱与峡谷出水口远期重构另立 [10 号文](./10-basin-mountain-encirclement.md) |
| 其余模板 / R0 / T4 | 规划登记与局部规格 | 八a R0、八b OxbowLake、八c FootLake/RidgeWaterfall、九a~九d 远期模板、T4 动态水文——速览见 R.1.1，分项解锁见 R.3 |

<a id="r11-未完成步骤速览与必要性"></a>
### R.1.1 未完成步骤速览与必要性
...
```mermaid
flowchart TD
    B["已落地：T0/T1/T2/D-A · 阶段二基座 · D-B1 代码<br/>D-C 通用 · 阶段七三模板 · P1（台地/盆地/冲积扇/火山湖）"]
...
| UGC-00 | ⬜ **新增规划**：冻结旧生成器基线、字段 hash、WASM/native 对拍和分层诊断格式 | ⓪ 基座 | 否 / 低 | 根目录统一生成器设计 §8、§10 |
| UGC-01 | ⬜ **新增规划**：Field IR、通用算子、土壤/水分/地表材料与调色字段和 HeightfieldBackend；先迁移草原与山口 recipe | UGC-00 | 是 / 中 | 根目录统一生成器设计 §4~§6 |
| UGC-02 | ⬜ **新增规划**：汇流、侵蚀、沉积、水位和水体语义投影 | UGC-01 | 是 / 高 | 根目录统一生成器设计 §4.4、§5、§8 |
| UGC-03 | ⬜ **新增规划**：河谷、半坡、台地、盆地、冲积扇、火山湖全部改为 recipe，并验收“有水才长草、干燥区转沙地”字段约束 | UGC-02 | 是 / 高 | 根目录统一生成器设计 §4.3、§6、§8 |
| UGC-04 | ⬜ **新增规划**：稀疏 VoxelBackend、32³ chunk、HeightfieldView 和网格提取 | UGC-03 | 是 / 高 | 根目录统一生成器设计 §7 |
| UGC-05 | ⬜ **新增规划**：静态 chunk 重建、动态 delta 存档、FABS/WASM/WebGL 接入 | UGC-04 | 是 / 高 | 根目录统一生成器设计 §7、§8 |
| UGC-06 | ⬜ **新增规划**：地层柱、材料属性、储水量、渗透性和体素材料投影 | UGC-05 | 是 / 中 | 根目录统一生成器设计 §4.5、§8 |
| UGC-07 | ⬜ **新增规划**：断层、褶皱、不整合的确定性位移场 | UGC-06 | 是 / 中高 | 根目录统一生成器设计 §4.5、§8 |
| UGC-08 | ⬜ **新增规划**：静态地下水、土壤湿度、草地/沙地因果链 | UGC-06 | 是 / 中 | 根目录统一生成器设计 §4.3、§4.6、§8 |
| UGC-09 | ⬜ **新增规划**：剖面诊断、候选比较和 confidence 场 | UGC-06 | 否 / 中 | 根目录统一生成器设计 §4.7、§8、§10 |
| UGC-10 | ⬜ **新增规划**：多层查询、洞口/竖井连接和地下寻路 | UGC-07、UGC-08 | 是 / 高 | 根目录统一生成器设计 §7.3.1、§8 |
| UGC-11 | ⬜ **新增规划**：DEM、地层接触点、钻孔和断层线只读适配器 | UGC-06、UGC-07 | 否 / 中高 | 根目录统一生成器设计 §7.3.2、§8 |
| UGC-12 | ⬜ **长期候选**：多相流、反应输运、热、力学和地震反演独立后端 | UGC-08、UGC-10 | 是 / 极高 | 根目录统一生成器设计 §1.2、§8 |
...
| 六 · P1 | ✅ **已交付**（v1.50.55 TB-03：盆地 `basin_oasis_v1` / 冲积扇 `alluvial_fan_v1` / 火山湖 `volcanic_lake_v1`，静水全链路 + 60/60 种子矩阵；火山湖独立规格已回填 §5.6；冲积扇辨识度改善第一批随 v1.50.68 落地） | 二；建议五先行，不是硬依赖 | 是 / 中高 | §1.2、§5.6 |
...
- **统一生成器关键路径**：UGC-00 → UGC-01（草原/山口 recipe）→ UGC-02（水文过程）→ UGC-03（既有静态模板迁移）→ UGC-04（体素后端）→ UGC-05（存档与 WebGL）。UGC-01 通过前不新增新的模板专用几何模块；UGC-03 通过前旧 profile 继续作为兼容 alias。
...
- 剩余阶段开工时逐项拆分；已交付的 TB-01 支脊、S7-02 草原骨架及 TB-02/TB-03 P1 模板不重新建任务。火山湖规格已随 TB-03 落地（§5.6 要点 1），TB-04 群峰环抱与峡谷出水口另立 [10 号文](./10-basin-mountain-encirclement.md)。
...
- **项目路线图与实施计划**：R.0 统一生成器方向 · R.1 现状快照（含 R.1.1 未完成步骤速览与必要性）· R.2 路线总览 · R.3 阶段计划 · R.4 依赖与并行要点 · R.5 门禁与退出标准速查 · R.6 原子任务与交付规则
...
- **第一部分 · 地图模板（核心）**
  ...
  - **5. 地图模板实现蓝图**：5.1 提交组与边界 · 5.2 单一真相源与稳定 ID · 5.3 创世流水线 · 5.4 D-B2 四种子特征 · 5.5 D-B1 纯视觉扩展 · 5.6 P1 四个已落地 profile 的历史算法记录与 UGC 映射 · 5.7 快照/FABS/前端/存档同步 · 5.8 有界失败与验收矩阵 · 5.9 提交顺序（已并入文首 R.3）；UGC-00~05 的执行卡见根目录统一生成器设计
...
| **profile / recipe** | 兼容层中的模板 ID 与统一生成器中的数据配方。旧名如 `mountain_pass_v1` / `river_valley_v1` 保留作存档 alias；新实现由 `TerrainRecipe` 的字段节点、过程节点和玩法约束组成。空间目标不同可以新增 recipe，但不得为每张地图复制生成算法 | §2、§9.1、根目录统一生成器设计 |
...
采用 **少量地图骨架 × 地貌子特征 × 成套景观风格**，并把骨架编译为统一的字段图。现有实现继续使用 Rust 静态高度场；目标后端是稀疏体素，迁移期由高度场兼容视图服务旧查询。首期不等待完整实时流体；创世阶段允许使用固定迭代次数的侵蚀、汇流与沉积过程，让河道、河阶、湖盆和冲积扇从字段关系中产生。

| 层次 | 职责 | 示例与边界 |
|---|---|---|
| 地图骨架 / recipe | 主要字段、过程和玩法目标 | 山口、河谷、台地、盆地、冲积扇；新增地图先组合通用节点，只有新的物理过程才新增代码 |
...
山脚湖是局部子特征；**火山湖**围绕湖岸、陆路出口和连续可建带组织独立骨架，不能用 T1/T2 的局部山脚湖替代。`volcanic_lake_v1` 已随 TB-03 落地并进入 `random`；后续 UGC-03 会把它改写为 `Cone + Crater + WaterLevel + Outlet` recipe。已登记的平地草原、半坡林地（§4.1/§4.2）保留为 recipe 迁移对象，不因增加景观风格重复命名。

### 1.3 高度场、分辨率与有效世界边界

- 当前高度场默认 `terrainGridRes=256`（v1.50.70 由 160 提升），约 764m 世界对应采样间距约 2.996m。影响通行的沟、岸、坡应跨越数个网格，并以**离散高程、坡度及完整走廊**验收；连续公式峰值不能代替离散结果。统一编译器的 Field2 首先对齐该分辨率，VoxelBackend 再按 32³ chunk 生成三维密度和材质。
- 岩缝、细水丝、草簇等小尺度细节由表现层派生。渲染细分不得创造与内核不一致的可走地面；每边分辨率翻倍约使格数增至四倍，须先测生成、查询、快照与绘制成本，不默认全图加密。
- 高度场适合丘陵、陡坡、台地、盆地、沟谷；真垂直崖面、悬挑、洞穴和多层通行由目标 VoxelBackend 表示。迁移期仍用真实陡坡和顶部 HeightfieldView 服务旧路网，不绘制暗示可穿行的天然石桥；多层体素在多层导航完成前不自动成为 Agent 可行走区域。
...
| 3 | 火山湖 | 中心湖、外缘山坡、环湖缓岸与陆路出口 | 房屋沿干燥缓岸分布，道路绕开水面 | 独立骨架 `volcanic_lake_v1`；山脚湖/牛轭湖仍是局部子特征 | ✅ 已落地（v1.50.55 · TB-03：双出口 + 双岸点共享池；UGC-03 迁移为 recipe） | §3.5、§5.6 |
...
> 局部湖泊走 T1/T2 子特征注入；整图「火山湖」仍是独立骨架（§1.2），两者不要混淆：T1 鞍部低地的「山脚湖」、T2 主河弯道切割的「牛轭湖」。两者均生成静态 `WaterBody` 特征（`resource_pool_id = 0`，**不可采水**，不创建 `WaterAccessPoint` / `PrimitivePoi`，不参与 HUD 水库存量统计）。**现行算法与拒绝条件以 §5.4.A（`FootLake`）与 §5.4.C（`OxbowLake`）为准**，本节只保留玩法意图。
...
|---|---|---|---|
| 火山湖 | 湖泊、外缘山坡、环湖缓岸 | 房屋沿干燥缓岸分布，道路绕开水面 | ✅ 阶段六已落地；`volcanic_lake_v1` 复用水体和可建区查询，不能用局部山脚湖替代整图构图。**不含湿地/泥泽**，不依赖动态排水；UGC-03 迁移为 `Cone + Crater + WaterLevel + Outlet` recipe |
...
## 5. 地图模板实现蓝图（D-B ～ 静态 Profile 扩展，兼容 UGC 编译器）

> **服务对象**：历史模板扩展的施工契约；统一生成器新增地图的主施工契约是根目录 [TERRAIN_FIELD_COMPILER_DESIGN.md](../../../TERRAIN_FIELD_COMPILER_DESIGN.md)，本节保留旧 profile、子特征和稳定 ID 的兼容规则。
...
历史模板扩展按以下四类独立提交组实施，**禁止合并为一次大改**；这不是串行排期。统一生成器迁移使用独立的 UGC-00~05 边界，不能把旧模板注入器和编译器迁移合并。优先级与排期见文首 R.3，R0 只阻塞牛轭湖：
...
| **P1** | 台地、盆地、山前冲积扇、火山湖（四个静态 profile 已落地） | 是（逐个验收） | 是 | 历史实现已通过各自矩阵；后续 UGC-03 逐个迁移为 recipe 后重新验收再替换生成入口 |
| **UGC-00~05** | 基线与字段诊断 → Field IR/高度场后端 → 通用水文过程 → recipe 迁移 → VoxelBackend → FABS/WebGL 接入 | 兼容 alias 保留 | 按波次改变 | 每波次通过字段 hash、约束、WASM/native 对拍、性能和存档门禁；执行卡见根目录统一生成器设计 |
...
P1 不实现海水、航运、湿地、洞穴或季节水位。火山湖的大湖是本节可采的新水面，必须复用现有 `WaterPool`、`WaterAccessPoint`、`sync_water_pois()` 和按 `agent.id` 的结算顺序；台地/冲积扇不得因地形标签直接发放资源或肥力收益。

✅ **TB-03（v1.50.55）三模板已落地**：`alluvial_fan_v1` / `basin_oasis_v1` / `volcanic_lake_v1` 均按本节规格实现并通过 60/60 种子物理矩阵与 Chrome 样板验收；实施算法、参数窗口与实测踩坑沉淀见各模块（`geo/alluvial_fan.rs` / `geo/basin.rs` / `geo/volcanic_lake.rs` / `geo/static_water.rs`）与 [14 号文](../../current/tech/14-terrain-and-network.md)。落地要点修订（以实现为准）：

1. **火山湖独立规格（`volcanic_lake_v1`，本节新增回填）**——湖半轴 0.10~0.16×world 两轴独立抽样、湖形 = 椭圆 + 有限低频径向扰动；分带高程：湖床（静水 `WaterBody` 特征 #1，水位 = 干岸基准 − 1.2m）→ 干岸平台（0.45×r_out 宽、岸线外 10m 安全退距 NO_BUILD）→ 环湖盆岭（sin² bump 峰值 12m，两端导数为零，不可用单调外坡否则形成环形断崖墙）→ 外缘平地；两个分离陆路出口（角向归零 85%、间隔 ≥126°）与两个分离湖岸取水点共享 `WaterPool #1`。
2. **火山湖取水岸点数量例外**——取水岸点数 = 2（由 `water_source_poi_count(profile, countWater)` 决定）；`countWater` 保留水供给预算基数语义（池总量/再生按 `countWater` 乘算，岸点不再乘入），既有模板语义不变。
...
`resolve_profile` 的 `random` 映射随各模板 M1/M2 验收分批扩容：v1.50.40 五路（T1/T2/草原/半坡/河谷聚落）→ v1.50.54 六路（+台地）→ v1.50.55 九路（+冲积扇/盆地/火山湖，`(seed ^ 0x5052_4F46_494C_4531) % 9` 各 ~11.1%）→ **v1.50.68 八路（砍需求：删除河谷聚落、台地更名 `plateau_v1`，`(seed ^ 0x5052_4F46_494C_4531) % 8` 各 ~12.5%；同一种子的 random 映射随之改变）**；`flat_baseline` 永不入池。
...
> 推荐提交顺序已与 §2.2 路线图、§5.1 提交组统一整合为文首「**项目路线图与实施计划**」R.3 阶段计划（含难度、依赖与提交组映射），本节不再维护第二份顺序清单。约束不变：提交组边界与退出条件以 §5.1 为准，每阶段门禁见 §5.8 与 §18，阶段内要求（支脊按 §5.4.E 规格及探针准入、物理/装饰改动分别提交、样板不冒充验收、火山湖先补独立骨架规格等）已并入 R.3/R.4。
...
|---|---|---|---|---|
| UGC-00~03 | Field Graph、通用水文过程与 recipe 迁移；输出字段、地貌特征和水体语义 | UGC-00 基线与固定 seed 矩阵 | 07 号只消费字段/材质/水体事实；装饰仍走独立 accent RNG | ⬜ 规划，执行细节见根目录统一生成器设计 |
| UGC-04~05 | 稀疏体素 chunk、网格提取、HeightfieldView、静态重建和动态 delta | UGC-03 全部静态模板通过约束门禁 | 31 号 WebGL 统一管线消费 chunk mesh；LOD 不改变语义字段 | ⬜ 规划，不能提前写入现状文档 |
...
近期旧模板交付（阶段一～八）不包含下列扩展；UGC-02 之后的统一编译器会在创世阶段逐步加入有界侵蚀、汇流和沉积过程，但 R.3 的远期模板仍只登记方向与前置，不代表 T4/航运已经具备施工规格：

- 实时流体、地下水、洪水和枯丰水期模拟；创世阶段的固定迭代侵蚀/汇流/沉积属于 UGC-02，不能扩展为 tick 内动态模拟；
...
- 以 GPU/3D 引擎升级替代 T0 的查询、存档、确定性和路网基础工作；VoxelBackend 必须通过 HeightfieldView 和统一查询接口接入；
