...
> **阅读入口**：[R.1 当前状态](#r1-现状快照) → [R.3 阶段计划](#terrain-roadmap) → §2.1 模板清单 → §5/§6 施工契约 → §18 验收。近期任务明细见 [TODO](../../../TODO.md)；阶段二（生成组合基座与有界回退）与阶段七（插队模板批次）已收官，专项任务文档 STAGE2-TODO.md 与 STAGE-07-TODO.md 已随之归档清理，收口证据见 [14 号 §8.2](../../current/tech/14-terrain-and-network.md) 与 changelog v1.50.52。
...
| 阶段二 | STAGE2-1 配置加回（v1.50.39）、STAGE2-2 水系影响带收敛（v1.50.40）、STAGE2-3 流水线框架（v1.50.45）、STAGE2-4 几何/ID 校验 + STAGE2-6 生存诊断（v1.50.47）、STAGE2-7 `flat_baseline` 显式基线（v1.50.48）、STAGE2-5 有界降级集成（v1.50.49）、STAGE2-8 收口（v1.50.49 文档收口）**全部交付**；合法基线等价与失败降级分层验收完成（证据见 14 号 §8.2） | STAGE2-3 完整几何事务域（features/水体/中心线/覆盖意图/halo 备份）随阶段三注入启用；局部拒绝随几何事务域落地 |
| T1 骨架扩展 | TB-01 支脊与多尺度地貌已交付，生成器版本已升为 5；现状见 14 号与 [07 号 §7.2](./07-terrain-art.md) | 不再重复安排支脊开发；后续流水线重构须保持当前 T1 输出 |
| 平地草原 | S7-01~S7-03 全部交付（探针基座 / 内核骨架 / 草甸装饰）；**阶段七（插队模板批次 S7-01~S7-10）已随 v1.50.52 全链路收口**：半坡林地（S7-04/05）、河谷聚落（S7-06/07）、配置联动（S7-08）、FABS/快照/前端视图适配（S7-09）、确定性回归与版本收口（S7-10）；3 张新 profile 已入存读档白名单与 `random` 候选池（2→5 路均衡） | 无——阶段七闭环；湖畔盆地等后续模板按 R.3 分项解锁 |
| 阶段四 · D-C | 通用资源景观套件、几何遮罩避让、五类配方样板（Water/Wood/Berry/Stone/Gold）、标签布局引擎与聚合/双目标停靠兜底（S4-01~S4-08 全部收口交付，v1.50.46~53，详见 [16 号文](../../current/tech/16-frontend-overview.md) §2.16～§2.18；特定地貌专属景观群与院地任务见 [STAGE-04-SUBFEATURE-LANDSCAPES-TODO](../../../STAGE-04-SUBFEATURE-LANDSCAPES-TODO.md)） | 无——通用阶段四闭环；特定地貌专属景观群（S4-X1~X4）随对应注入器逐项交付 |
| 其余模板 / D-B2 / R0 / T4 | 规划登记与局部规格 | 按 R.3 分项解锁，未完成不得标为已发布 |

### R.2 路线总览（依赖解锁图）

```mermaid
flowchart TD
    B["已落地 T0 / T1 / T2 / D-A / D-B1"] --> S2["二：生成组合基座收口"]
    B --> G["七：草原装饰与验收（骨架已交付）"]
    B --> F["三：密林山坡等视觉散布规则"]
    F --> H["七：半坡林地"]
    B --> DC["四：D-C 通用景观群与标注避让"]
    S2 --> C["三：RiverCliff（支脊已由 TB-01 交付）"]
    S2 --> P["五/六：台地、盆地、冲积扇、湖畔盆地"]
    S2 --> V["七：连续河谷"]
    S2 --> R["八a：R0-1 → R0-4"]
    R --> O["八b：OxbowLake"]
    S2 --> W["八c：FootLake / RidgeWaterfall"]
    S2 --> K["九a：地表喀斯特"]
    S2 --> SEA["九a：海湾淡水/海水契约"]
    SEA --> PEN["九a：半岛陆桥契约"]
    SEA --> FI["九c：静态峡湾"]
...
实线仅表示能力前置，不代表日期或所有批次间的先后顺序。同一框中的模板仍各自立项与验收；九a～九d 保留远期窗口，静态首版不强制等待 T4。TA-09 场景样板的实际前置见 R.4 / §19，构图评审与实机交付记录见 [07 号 §4.4.1](./07-terrain-art.md#441-ta-09-构图评审与实机交付记录)。
...
| 二 · 共用基座 | **✅ 全部交付（v1.50.39~49）**：STAGE2-1~STAGE2-8 全部收口；合法基线等价与失败降级分层验收完成（证据见 14 号 §8.2） | 一；原子任务顺序见 R.6 | 重构不改；新增拒绝/降级另拆 / 中 | §5.3、§5.8 |
...
| 七 · 独立模板 | **✅ 全部交付（v1.50.39~52）**：草原 S7-01/02/03、半坡林地 S7-04/05、河谷聚落 S7-06/07、配置联动 S7-08、FABS/快照/前端适配 S7-09、确定性回归与版本收口 S7-10（专项 STAGE-07-TODO 已随收官归档清理） | 无——3 张新 profile 已过 §18 全链路验收并加入 `random` 候选池 | 是 / 中 | §4.1/4.2/4.3、阶段七收口记录 |
...
- 剩余阶段开工时逐项拆分；已交付的 TB-01 支脊及 S7-02 草原骨架不重新建任务。湖畔盆地须先完成「环湖可建带、陆路出口、淡水来源、与盆地绿洲的区别」规格，再安排实现。
...
- **项目路线图与实施计划**：R.1 现状快照 · R.2 路线总览 · R.3 阶段计划 · R.4 依赖与并行要点 · R.5 门禁与退出标准速查 · R.6 原子任务与交付规则
...
| 4 | 台地聚落 | 平缓台面、台缘陡坡、坡脚水源、缓坡入口 | 高处开阔用地与下行取水/采集成本之间的取舍 | 新增 profile `plateau_settlement_v1` | ✅ **已发布**（v1.50.54 TB-02：SDF 圆角矩形台面 + 陡壁过渡带 + 双入口缓坡 + 坡脚泉溪与双接入路网 + 随机池准入，60/60 种子全通） | §5.6 |
...
| 7 | 盆地绿洲 | 封闭/半封闭盆地、汇水低地、中心泉池 | 聚落围绕低地水源分布 | 新增 profile `basin_oasis_v1` | ✅ 已落地（v1.50.55 · TB-03，单岸点共享池） | §5.6 |
| 8 | 峡湾聚落 | 深窄海湾、两侧陡坡、谷口、少量缓岸 | 水域造成强分隔，谷口成为陆路关键节点 | 静态海水/淡水与陡坡走廊 | ⏳ 排期：阶段九c（规格前置） | §4 |
| 9 | 河谷聚落 | 上游河道、谷底冲积带、两侧连续山坡 | 水、耕地、道路都沿谷底集中 | 新增 profile `river_valley_settlement_v1` | ✅ **已发布**（v1.50.48 S7-06 骨架 + v1.50.49 S7-07 主河水系/浅滩/水池 + v1.50.52 S7-10 全链路收口：存读档白名单 + `random` 候选池入列，见 §4.3） | §4.3 |
...
| 13 | 山前冲积扇 | 山口出口、扇形缓坡、分散浅沟 | 山地资源流向平原的过渡地带 | 新增 profile `alluvial_fan_v1` | ✅ 已落地（v1.50.55 · TB-03，干浅沟软地禁建） | §5.6 |
...
| 盆地绿洲 | 封闭或半封闭盆地、汇水低地、外缘山坡与中心泉池 | 聚落围绕低地水源分布，外缘资源与中心生活区形成明确距离梯度 | 阶段六（P1）；复用共用可建区与生存诊断契约，不依赖湖畔盆地先完成，泉池走 `WaterPool`（见 §5.6） |
| 峡湾聚落 | 深窄海湾、两侧陡坡、谷口与少量缓岸 | 水域造成强分隔，有限岸带与谷口成为陆路关键节点 | 阶段九c；需要静态海水边界、陡坡走廊、安全岸线与内陆淡水，动态水文和航运均不作为首批前提 |
| 河谷聚落 | 上游河道、谷底冲积带、两侧连续山坡和浅滩/桥位预留 | 水、耕地、道路都沿谷底集中，跨谷与上坡扩张形成权衡 | 阶段七；以两岸河谷为基础，逐步加入更连续的谷地骨架（新增 profile `river_valley_settlement_v1`，设计见 §4.3） |
...
- **配置联动与全链路收口记录（v1.50.51~52 · S7-08/S7-10）**：S7-08 将草原形态参数（残丘幅度/幅径比、泉洼深度/半径等）集中化为 37 个 SimConfig 字段（四路同步，世界输出逐位不变）；S7-10 收口——存读档白名单入列 `grassland_plain_v1`、`random` 候选池 2→5 路均衡（`(seed ^ 0x5052_4F46_494C_4531) % 5`）、T1/T2 24 种子跨构建对拍逐字节一致、3×60 种子矩阵 §1.4 门禁 0 违例（草原 maxSlope 9.61°~16.33°、buildable ≥14392、detour_p95 1.08）。证据见阶段七 S7-08/S7-10 归档实测记录。
...
- **全链路收口记录（v1.50.52 · S7-10）**：存读档白名单入列 `hillside_woodland_v1`、`random` 候选池 2→5 路均衡；半坡 60 种子矩阵复验 §1.4 门禁 0 违例（maxSlope 22.04°~27.22°、buildable ≥13268、>30° 恒 0）；T1/T2 24 种子跨构建对拍逐字节一致。证据见阶段七 S7-10 归档实测记录。
...
| **P1** | 台地聚落、盆地绿洲、山前冲积扇；湖畔盆地待独立规格 | 是（逐个验收） | 是 | 每个模板单独通过有效世界矩阵后才加入 `random` 候选 |
...
- **阶段二跨构建证据**：剩余纯重构基准取包含 TB-01 与 S7-02 的冻结当前提交（记录实际提交 ID）；已完成 STAGE2-2 的历史对拍保留原基准，记录基准/候选提交、双副本 WASM SHA256、配置与固定种子；复用 §5.1 的物理字段规范化比较方法，显式 T1/T2 各跑 seed 0–59，开关开/关均比较高程、坡度、地表、flags、肥力、水系、POI 与路网。新旧构建各自确定性不能代替此比较。任一物理差异即不通过“基座自身不改世界”；必要的行为修正应另拆物理变更提交、递增生成器版本并重新验收，不能混入等价重构。
- ◐ **STAGE2-3（v1.50.45）流水线框架已落地，完整几何事务域随阶段三注入启用**：编排器 `generate_with_config` 迁至 `geo/terrain.rs`，按本节管线展开为私有阶段 0–9（`resolve_profile` / `reset_static_terrain_state` / `generate_base_relief`〔原 `generate_with_profile`，只铺高程、坡度/地表/肥力/flags 定稿上移第 6 步 `finalize_slope_and_surface`〕/ `apply_profile_static_hydrology`〔hydrology.rs〕/ `plan_subfeatures` / 第 5 步几何管线 5a–5d / `finalize_slope_and_surface` / `validate_static_terrain_geometry`〔先接线稳定 ID 校验，STAGE2-4 扩充〕/ `generate_base_accents` / `append_subfeature_accents`〔空实现〕）；第 10/11 步由 `World3DEngine` 创世序列执行（调用点不变）。阶段间临时数据走 `GenesisScratch`（T2 共享河几何 + 草原软地掩码，不进快照/存档）。等价验证：临时对拍 180 组（T1/T2/草原 × seed 0–59）物理字段逐位指纹 100% 全等 + 临时时序断言验证 0–9 步严格按序（脚本用后删除）；`TERRAIN_GENERATOR_VERSION` 保持 5。**缺口**：本节「回滚必须是事务」要求的完整支撑域备份（features / 水体 / 中心线 / 覆盖意图 / 差分 halo）尚未实现——当前 `SubFeatureWorkspace` 仅备份高程并支持整块回滚，属阶段三生产注入时的启用范围。
- ◐ **生存诊断已实现、未接拒绝（STAGE2-6，v1.50.47）**：`spatial/survival_diagnosis.rs::diagnose_survival()` 独立只读——按实际配置枚举营地（不硬编码 4 个），逐营地单源 Dijkstra（微秒整型权重）检查水/粮/市场三类路网可达与往返成本（口径复用生产寻路：`terrain_time_cost` 软地/浅滩折算 + `Δz×grade_coef` 坡度折算）；预算由现有配置推导（两端满仓自饮自食 ⇒ 往返 ≤ 2×capacity/代谢速率，名义消化 1.0，市场口径同粮），无新增超参。失败码 `SpawnDisconnected` / `SurvivalCostExceeded`；矩阵校准：4 profile × seed 0–59 全通过（水 max 349.8s / 粮 169.5s / 市 441.7s < 500s 预算），掐断资源车道可准确检出。是否拒绝并重试由本节有界回退环（STAGE2-5）决定。
...
| `basin_oasis_v1` | 椭圆盆地在 q≤1 内下凹，边界用有限支撑平滑带回接；`depth*(1-q²)` 仅作内部候选，不能延伸至全图或在 q=1 留导数折角；中心小泉池半径 10–14m，池外 12m 为干燥岸环 | 泉池深水禁行；只允许岸环放置水源 POI；外缘坡度过大禁建 | `WaterBody #1` 绑定唯一 `WaterPool #1`，所有水源 POI 共享它；不能同时保留额外独立清泉库存 | 多岸点扣减守恒；湖面与库存分离；营地到水点往返低于生存阈值 |
...
1. **湖畔盆地独立规格（`lakeside_basin_v1`，本节新增回填）**——湖半轴 0.10~0.16×world **两轴独立抽样**；湖形 = 椭圆 + 有限低频径向扰动（相对幅度 ≤0.12，半径恒正、无岛屿）；分带高程：湖床（静水 `WaterBody` 特征 #1，水位 = 干岸基准 − 1.2m）→ 干岸平台（0.45×r_out 宽、平坦可建、岸线外 10m 安全退距 NO_BUILD）→ **环湖盆岭**（sin² bump 峰值 12m，两端导数为零——不可用只升不降的单调外坡，否则岭缘留下环形断崖墙）→ 外缘平地；**两个分离陆路出口**为盆岭垭口（角向归零 85%，间隔 ≥126°）；两个分离湖岸取水点（岸线外 12m）共享 `WaterPool #1`。核心检验：湖体真实阻断直穿、环岸陆路全宽连通、双出口可达。
2. **盆地绿洲取水岸点数量例外（§5.6 数量语义修订）**——小泉池（半径 10~14m）塞不下 `countWater` 个满足 POI 间距的岸点：`basin_oasis_v1` 的**实际清泉 POI 数 = 1**、`lakeside_basin_v1` = 2，由 `water_source_poi_count(profile, countWater)` 决定；**`countWater` 保留水供给预算基数语义**（池总量 = `stockMaxWater×countWater`、再生 = `regenBaseWater×countWater`，岸点不再乘入预算）。既有六张模板的数量与预算语义不变。`countWater=0` 走空池 → 生存门禁明确失败/降级。
...
4. **实测踩坑沉淀**——盆地不做泉池区整片平坦化（盆底曲率 F(q) 混合环会产生 33°+ 陡坡环阻断出口，改为泉池邻域噪声强抑制 + 水位深偏移 2.0m）；冲积扇山口锚点必须在图缘线上且扇头侧缘保留 110m 最小过渡弧宽（否则扇头形成横贯扇面的 NO_WALK 墙）；固定生成期锚点不可作建房候选门禁（会落在扇侧陡缘/岸环禁建带），门禁一律按最终定稿格网扫描 `validate_footprint`。

### 5.6.1 random 候选池现状

`resolve_profile` 的 `random` 映射随各模板 M1/M2 验收分批扩容：v1.50.40 五路（T1/T2/草原/半坡/河谷聚落）→ v1.50.54 六路（+台地）→ **v1.50.55 九路（+冲积扇/盆地绿洲/湖畔盆地，`(seed ^ 0x5052_4F46_494C_4531) % 9` 各 ~11.1%）**；`flat_baseline` 永不入池。
...
- ✅ 初始营地、关键资源和市场位于预期陆路连通分量，往返成本不超过生存诊断上限——诊断函数（STAGE2-6，v1.50.47）+ 拒绝/重试接入（STAGE2-5，v1.50.49 有界构造器门禁链 `Geometry:`→`RoadNetwork:`→`Survival:`）均已落地；生产路径统计（STAGE2-8 收口）：默认配置 4 profile × seed 0–59 共 240/240 首试通过。
- ✅ 现有无新地貌基线在关闭地形 profile 后保持行为等价——`flat_baseline` 显式 profile 已交付（STAGE2-7，v1.50.48，生成器版本 6 → 7）：可生成/可存读档/可续演、不进 random、生存诊断 seed 0–59 零失败；旧 T0 等价对照存在**基准缺口**（旧 T0 生成源已删除，见 §5.8 基线先验收条目的声明与新冻结基准）。降级目标实测：收紧 `terrainMaxWalkSlope=1.9°` 后 231/240 种子降级发布至 flat_baseline（STAGE2-8 收口统计）。
...
- ✅ 生成失败时在有界重试后使用通过校验的简化 profile，并记录失败原因——STAGE2-5（v1.50.49）有界降级环落地：阶梯（禁结构子特征 → 移除可选支脊 → `flat_baseline`）+ 策略去重 + 诊断记录（`creation_diagnostic`）+ 降级发布经事件流明示（不冒充原模板成功）；收口统计与差分证据见 [14 号 §8.2](../../current/tech/14-terrain-and-network.md)。
...
- ✅ 生成时过滤禁行格与深/浅水（`Boulder+RockFace` 为受控例外）；这不证明避开稍后生成的道路、房屋或 POI。相关视觉避让由 D-C 的确定性后处理验收，不能勾选为已有能力。
- ✅ 装饰密度与配置 `terrainAccentDensity` 呈线性关系（density=0 时无装饰；density=2 时数量约 2x 默认）。
- ✅ 装饰并入统一深度队列后按相机深度正确遮挡（v1.50.2），不再「远树压近树」或「近树被远人穿透」。
- ✅ FABS Section 21 历史对拍通过；当前快照门禁使用 §18.6 的命令，已移除的 JSON 对拍不再列为待执行项。
