...
>
> **状态**：T0 统一地表查询、T1 山口聚落、T2 两岸河谷水系与 D-A 地表装饰系统**均已落地**（T1/T2 于 v1.47.5；D-A 于 v1.49.1，v1.49.2/v1.49.3/v1.50.10 持续打磨）；生成器版本 **4**（v1.50.17 经 T1-R 主脊通行力修复后由 3 递增）；T1/T2 **子特征注入**、D-B/D-C 高级装饰为**新增规划（v1.48.0）**；其中阶段一 D-B1（RockCluster/GrassTuft、特征/快照骨架、子特征选择器与总开关）**代码交付已完成**（v1.50.29→35，D-B1-9 收口验收见 §18.5），注入器仍在后续阶段；T4 动态水文、桥梁与土地演化**未实现**（2026-09-13 起 §2.1 全部 16 张地图模板已纳入文首 R.3 排期，T4 相关模板排至阶段九c）。**T1-R 主脊通行力缺口已修复**（2026-09-11 审查发现、同日修复：主脊最大坡度由约 24° 提升至 36°~41°，产生真实硬禁行与绕行代价；实测记录、参数契约与门禁见 §9.3.1）。
...
| 一 | D-B1 | 代码交付：RockCluster/GrassTuft 生成与绘制 ＋ 特征/快照骨架（FABS Section 22、`FORMAT_VERSION` 2→3）＋ 子特征选择器与总开关接线（字段加回见 R.5）；**旧 T1/T2 高程/地表/路网/POI 逐字节不变**；顺带收口 §18.4 装饰缓存拆分验证；场景样板作为关联美术交付按 07 号 TA-09 波次后置，不阻塞代码交付及阶段二；✅ **代码交付完成**（D-B1-9 收口，验收记录见 §18.5） | 低 | D-A ✅ | 否 | §5.5、§5.2、§17.1 |
...
| 三 | D-B2 | 首批子特征：T1 不对称支脊（按 §5.4.E 施工，候选参数先过种子矩阵，不直接向共享 RNG 插入抽样）、T2 河谷峭壁 `RiverCliff`；配套视觉型裸岩露头、河岸林带；**物理改动与装饰改动分别提交** | 中 | 二 | 是 | §5.4.D、§5.3 |
...
- **D-B1 可单独上线**：旧 T1/T2 物理输出逐字节不变是其退出条件（§5.1）。✅ **已收口（D-B1-9，2026-09-13）：阶段一代码交付完成**（跨构建差分与 R.5 门禁记录见 §18.5），可进入阶段二；D-B1-8 / TA-09 实机场景样板依赖 07 号 TB-01、TA-08 及本方案阶段三 `RiverCliff`；构图草案可提前评审，保持独立待交付，样板通过前不将阶段一全部交付标为完成。视觉样板不替代物理验收。
...
| 15 | 平地草原 | 低幅起伏平原、草甸地表、孤立残丘与泉溪洼地（辅助至多二） | 无天然屏障的开放开局：选址与通行近乎自由，聚落结构完全由水源分布与踩踏涌现 | 新增 profile `grassland_plain_v1` | ✅ 内核骨架已交付（v1.50.40，S7-02）＋ 草甸装饰辨识度闭环已交付（v1.50.45，S7-03；`random` 候选待全链路验收） | §4.1 |
| 16 | 半坡林地 | 单侧缓坡山体、坡面林地、坡脚泉溪与局部露岩（辅助至多二） | 林地纯视觉可穿越、坡度产生真实慢行；房屋沿坡脚与林缘形成带状聚落 | 新增 profile `hillside_woodland_v1` | ✅ 内核骨架已交付（v1.50.46，S7-04；探针 124 种子 §1.4 门禁 0 违例；林地装饰 S7-05、`random` 候选待全链路验收） | §4.2 |
...
### 4.1 平地草原（`grassland_plain_v1`）· 设计登记（2026-09-12）· 排期（2026-09-13：阶段七·插队位）· 内核骨架已交付（2026-09-13：v1.50.40 / S7-02）
...
6. **离散尺度准入（编码前先跑临时探针）**：默认步长 Δ = 764/119 ≈ 6.42m，上述 0.25×18m 的抬升带仅 4.5m，不能用连续函数导数代替实际格点中心差分。探针使用与生产相同的坡度、地表和走廊判据，扫描 H={6,9,12}m、宽={18,24,30}m、抬升比例={0.25,0.5,0.75}、切向角 0°/30°/60°/90°、x/y 两轴网格相位 0/0.25/0.5/0.75Δ；平滑带完整计入，包含格界与斜向案例。候选均不达标时先调整局部形状/尺度，禁止直接提高全图分辨率掩盖失败。
...
首版只扩展 `mountain_pass_v1` 骨架，不新增 profile 或占用八类子特征的 kind/ID；由独立固定盐值和无状态 hash 派生参数，不向既有 `relief_rng` 或世界 RNG 插入抽样。随物理改动递增生成器版本；`terrainAccentSubFeatures` 只控制既有子特征选择器，不控制本骨架扩展。失败降级须有独立的 `disable_spurs` 标记，保留 seed 与原主脊参数，先去掉支脊再尝试 §5.8 的简化 profile，日志记录该标记。

- **几何规格**：1–2 条支脊，第二条可被保护规则拒绝；在主脊远离鞍部的两侧错开连接点，向外侧平原伸出。候选长度 80–140m、高程增量 8–16m、高斯横向半宽 24–40m。局部坐标 u 沿支脊、v 横跨支脊，增量采用 `A·exp(-(v/w)²)·sin²(πu/L)`（0≤u≤L），端点增量及一阶导数归零；横向 2w–3w 用 smoothstep 衰减到零，避免有限 AABB 截断跳变。起点位于主脊坡面，连续叠加到原高程，不削平主脊。上述数值仅为探针候选，达标后按配置联动固化。
- **保护范围**：以原主脊坐标的鞍部位置为中心，沿脊距离 ≤ 原 `saddle_width` 的整条横穿山口带保持原高程；支脊完整支撑域不得进入该带或越出地图。候选被拒绝即跳过，不重抽共享随机数。首版不叠加额外多尺度物理噪声；噪声作为后续独立物理提交，复用本节保护与矩阵，细小岩纹只属视觉层。
- **接线与回滚**：先生成原 T1 骨架，再叠加支脊，最后统一重算坡度/地表/flags；局部坡度验证读取包含差分一格邻域的原高程副本。生成营地/POI/路网后执行全局验收，失败完整重建无支脊 T1，不保留失败世界的实体、缓存或 RNG 状态。
- **固定矩阵与准入**：显式 T1 seed 0–59，各对照同 seed 无支脊结果；覆盖一条/两条、两侧及参数端点，记录实际接受数与失败原因。至少 10 个种子有支脊通过（不足顺序扩展至 seed 255）；所有交付世界须保持山口合法连续路线、主脊原有阻断作用、可行走连通分量不增加，营地至水/粮/市场可达且通过阶段二生存成本诊断。可建面积至少为无支脊对照的 90%（规划准入值）；另记录绕行比及资源往返成本，不以“全局连通”替代生存验收。存读档续演、分批 tick、跨世界缓存按 §18 验证。
- **前端边界**：从最终地表显示支脊，不恢复已删除的 Ridge/Saddle 特征轮廓，不让装饰树石提供额外阻挡；TA-09 在该骨架通过物理门禁后再做实机场景验收。
...
- ✅ **跨构建差分（§5.1 契约全项执行）**：基准提交 `5f09236`（v1.50.28，阶段一之前）vs 最终候选提交 `f21e573`（v1.50.35，D-B1-1～7 产物）；WASM SHA256：基准 `640c3cedb9dd82a23e12bc42a83361bc7a8741b6f08d942ac1658030d7f26b93`、候选 `bd3b7699e907e44801bd67255c6f351fe449d881f3a8e7a5d995ecb4619dd1a4`（`frontend/rust/` 与 `frontend/` 双副本同值）。冻结配置 = 基准提交前端 `config.js` + `config.decision-order.js` + `config.house-upgrade-cost.js` 合并（232 字段，SHA256 `5192330c1b00eab65e7646bf8a235e2bafabf79b6d708cc0179242edd00e3469`），候选端仅额外覆盖 `terrainAccentSubFeatures=false/true`，旧端不注入该字段。显式 `mountain_pass_v1` / `river_valley_v1` × 固定种子 `1,2,3,7,42,100,123,456,789,1024,2026,65535`，创世完成未推进 tick 取 FABS 全量帧。高程+完整地表格（14400 格 × 海拔/坡度/地表/肥力/水体归属/标志 6 物理字段）、完整路网节点与车道（几何+拓扑）、完整 POI 记录，经规范化导出（对象键递归排序、数组保序、f32 直读不取整）逐字节比较：**基准 vs 候选关 / 基准 vs 候选开 / 候选关 vs 候选开 三组各 24/24 全等，任一物理差异 0**。`terrain_generator_version` 两侧均为 4、`SAVE_FORMAT_VERSION` 未动。允许项符合预期：候选端 accents 数量增多（D-B1-5 RockCluster/GrassTuft，85±3 → 157±1），`terrain_sub_features` section 恒空（阶段一无注入）。临时比较脚本按 §4.10 于收口后删除，本记录保留基准、配置、矩阵与结果摘要。
- ✅ **R.5 门禁（全部对应最终候选产物）**：`cargo test --lib`（0 用例 0 失败，§4.10 有意结果）；WASM release 编译 + 双副本同步（SHA256 同上）；`test-wasm.js`（ALL_TESTS_DONE：同种子确定性 / 长程无 NaN 越界 / 存读档连续 / 版本不兼容拒绝）；`test-determinism.js`（6/6 套件全通）；`config-check.js`（Rust 233 = 前端 233，含 `terrainAccentSubFeatures` 消费点）；`frontend-check.js`（全绿）；`cross-doc-check.js`（冲突 0 · 漂移 0）。
- ✅ **阶段一代码交付完成**，可进入阶段二（生成组合基座）；D-B1-8 / TA-09 场景样板按 07 号波次后置，样板通过前不将阶段一全部交付标为完成。
