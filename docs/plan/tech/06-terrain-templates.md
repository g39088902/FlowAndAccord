...
> **复核基线**：2026-09-22，应用 v1.52.6；源码 `TERRAIN_GENERATOR_VERSION=14`（RiverCliff 注入器落地）、存档 `SAVE_FORMAT_VERSION=7`、FABS `FORMAT_VERSION=4`。阶段三最新交付见 R.1.1 / §5.4.D，旧验收记录保留各自版本基线；实施按变更性质判断版本递增，不照抄旧稿中的目标版本号。
