...
| B 选定扩展路径 | 依 §5.1 四类提交组（D-B1/R0/D-B2/P1）独立发布，排期见 §5.9 | 登记后选路径 | 生成骨架或 D-B1 装饰单独上线 |
...
按以下四类独立提交组实施，**禁止合并为一次大改**；这不是串行排期。优先级见 §5.9，R0 只阻塞牛轭湖：
...
1. **场景样板与 D-B1**：先完成支脊山口、岩壁河谷两种固定场景的构图与美术样板；纯视觉样板不冒充物理验收。按需接入特征/快照骨架及 RockCluster/GrassTuft，新增字段必须有消费点；旧 T1/T2 高程、地表、路网、POI 保持不变。
2. **生成组合基座与首批子特征**：实现 §5.3 的阶段化生成、水系局部整形、统一派生、确定性选择与 §5.8 有界回退。先交付不对称支脊与河谷单侧岩壁，配套裸岩坡、河岸林带；支脊须先补独立施工规格和种子矩阵，不能直接向共享 RNG 插入抽样。物理改动与装饰改动分别提交。
3. **P1 台地**：先完成不规则台缘、连续台面、明确缓坡入口，再打通取水、建房、道路、存读档和表现；离散坡度、遮挡和多种子验收通过后才加入 `random`。
4. **第二批骨架与景观组合**：盆地绿洲、山前冲积扇逐个实施；湖畔盆地先补全独立骨架规格。景观风格不自动新增 profile。平地草原、半坡林地保留候选资格，分别在草甸辨识度、林地遮挡与成本验证后排期。
5. **后续静态扩展**：地表喀斯特先限丘峰与可达洼地；海湾、半岛另补海水不可采饮、内陆淡水及陆路连通契约。动态水文、洞穴、航运不随模板自动引入。
6. **复杂水系专项**：山脚湖可按 §5.4.A 独立实施；瀑布先解决分辨率和遮挡样板。R0-1～R0-4 仅为牛轭湖的硬前置，不阻塞岩壁、台地和其他静态骨架。完成 R0 后再实现 §5.4.C 的裁弯、封口与资源保护。
7. 每个代码阶段独立更新现状文档与 changelog，按根 AGENTS.md 升版、同步 WASM 双副本并执行适用门禁。纯规划修订不升版，不把建议写成已落地能力。
