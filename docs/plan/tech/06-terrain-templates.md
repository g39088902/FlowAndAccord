...
```text
seed + TerrainRecipe
  → Field Graph（高程、硬度、降水、湿度、汇流、沉积）
  → 固定顺序地理过程（侵蚀、汇流、沉积、水位）
  → 通用约束评估（连通、可建、浅滩、绕行、取水）
...
迁移任务使用 `UGC-00`～`UGC-05`：基线与字段诊断 → Field IR/高度场后端 → 通用水文过程 → 全部静态模板迁移 → VoxelBackend → FABS/WebGL 接入。每个任务的输入、修改文件、禁止范围、稳定错误码和退出条件以根目录设计文档为准。
