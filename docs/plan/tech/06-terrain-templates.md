...
| 六 · P1 | ⬜ **未开工**：盆地、冲积扇、湖畔盆地逐项实施；湖畔盆地先补独立规格 | 二；建议五先行，不是硬依赖 | 是 / 中高 | §1.2、§5.6 |
...
此表登记实施窗口，不宣称所有远期模板已有完整算法。湖畔盆地规格必须明确环湖可建带、陆路出口、淡水来源及与盆地的区别；未通过规格与有效世界验收的模板不注册空实现、不加入 random。
...
- 剩余阶段开工时逐项拆分；已交付的 TB-01 支脊及 S7-02 草原骨架不重新建任务。湖畔盆地须先完成「环湖可建带、陆路出口、淡水来源、与盆地的区别」规格，再安排实现。
...
`resolve_profile` 的 `random` 映射随各模板 M1/M2 验收分批扩容：v1.50.40 五路（T1/T2/草原/半坡/河谷聚落）→ v1.50.54 六路（+台地）→ **v1.50.55 九路（+冲积扇/盆地/湖畔盆地，`(seed ^ 0x5052_4F46_494C_4531) % 9` 各 ~11.1%）**；`flat_baseline` 永不入池。
