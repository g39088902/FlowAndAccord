...
> **整理日期**：2026-09-12（瘦身：清除已落地过程性记录；同日重组：散落的实现路线/提交顺序/排期整合为文首「项目路线图与实施计划」并新增目录）；方案优化基线 v1.48.0，现状基线 v1.50.21。
...
flowchart TD
    ...
    S3 -.-> S4
    W3 -.-> S4
    S1 -.-> G1["候选插队 平地草原（依赖 GrassTuft）"]
    S3 -.-> G2["候选插队 半坡林地（依赖密林山坡）"]
    S6 --> FAR["远期 喀斯特 / 海湾 / 半岛 / 沙漠绿洲 / T4 动态水文 / 海岛"]
```
...
| 六 | P1 | 盆地绿洲 `basin_oasis_v1` → 山前冲积扇 `alluvial_fan_v1` 逐个实施；**同期补全湖畔盆地独立骨架规格**（不注册空实现，见 §1.2） | 中高 | 二（建议五先行） | 是 | §5.6、§1.2 |
| 七 | — | **候选插队位**（2026-09-12 设计登记、未正式排期）：平地草原 `grassland_plain_v1`（草甸辨识度依赖 GrassTuft，仅需阶段一）；半坡林地 `hillside_woodland_v1`（林地散布复用「密林山坡」，需其先行落地）；分别在草甸辨识度、林地遮挡与成本验证后排期 | 低中 | 一 / 密林山坡 | 是 | §4.1、§4.2 |
...
| 九 | — | **远期扩展**：地表喀斯特（先限丘峰与可达洼地）、海湾、半岛（各补海水不可采饮、内陆淡水与陆桥契约）、沙漠绿洲（需干旱地表/蒸散表现）；T4 动态水文/桥梁/土地演化（前置：动态状态存档与决策恢复）；海岛（依赖航运与补给模型） | — | 各自前置 | — | §4、§19、§21 |
...
| 字段 | 加回时点 | 唯一消费点 |
| :--- | :--- | :--- |
| `terrainAccentSubFeatures` | 阶段一（D-B1） | 子特征注入总开关；置 false 时 §5.3 第 4–5、9 步为空 |
...
| 3 | 湖畔盆地 | 中心湖、外缘山坡、环湖缓岸与陆路出口 | 房屋沿干燥缓岸分布，道路绕开水面 | 独立骨架待设计；山脚湖/牛轭湖仅为局部子特征 | ⏳ 未排期 | §3.5、§5.4.A/§5.4.C |
...
| 6 | 海湾聚落 | 受陆地环抱的海湾、缓岸、岩岬 | 沿避风岸发展的聚落、绕湾陆路 | 静态扩展候选（需海陆边界与淡水语义） | ⏳ 未排期 | §4 |
...
| 10 | 半岛聚落 | 三面临水的陆地、狭窄陆桥、内陆高地 | 陆桥成为进出瓶颈 | 静态扩展候选（需海水/淡水与陆桥契约） | ⏳ 未排期 | §4 |
...
| 14 | 喀斯特地貌 | 石灰岩丘峰、洼地、落水洞/泉眼 | 地表路径绕开岩丘，水源集中在泉眼 | 静态地表扩展候选（洞穴与地下河另立项） | ⏳ 未排期 | §4 |
...
| 湖畔盆地 | 湖泊、外缘山坡、环湖缓岸 | 房屋沿干燥缓岸分布，道路绕开水面 | 未排期；按 §1.2 设计独立环湖骨架、连续岸带与陆路出口，复用水体和可建区查询；不能用局部山脚湖替代整图构图。**不含湿地/泥泽**，不依赖动态排水 |
...
| 海湾聚落 | 受陆地环抱的海湾、缓岸、岩岬与内陆淡水来源 | 沿避风岸发展的聚落、绕湾陆路与岸线空间取舍 | 静态扩展候选；海水不可直接作为淡水资源，航运另立项，先完成海陆边界和淡水语义 |
...
| 半岛聚落 | 三面临水的陆地、狭窄陆桥、内陆高地或淡水点 | 陆桥成为进出瓶颈，岸线资源与有限扩张腹地形成选择 | 静态扩展候选；先确认海水、淡水、陆桥连通及极端阻断时的生存诊断 |
...
| 喀斯特地貌 | 石灰岩丘峰、洼地、落水洞/泉眼与可建台地 | 地表路径绕开岩丘，水源集中在泉眼或洼地；形成辨识度高的局部地标 | 静态地表扩展候选；洞穴、多层地下河和地下交通不进入首批，必须先定义地表水去向与安全禁入边界 |
| 平地草原 | 低幅起伏平原、草甸地表、孤立残丘与泉溪洼地（辅助至多二） | 无天然屏障的开放开局：选址与通行近乎自由，聚落结构由水源分布与踩踏涌现 | 未排期；新增 profile `grassland_plain_v1`，草甸视觉依赖 D-B GrassTuft 先行落地（设计见 §4.1） |
| 半坡林地 | 单侧缓坡山体、坡面林地、坡脚泉溪与局部露岩（辅助至多二） | 林地纯视觉可穿越、坡度产生真实慢行；房屋沿坡脚与林缘形成带状聚落 | 未排期；新增 profile `hillside_woodland_v1`，林地散布复用「密林山坡」子特征规则（设计见 §4.2） |

这些组合只约束初始环境。聚落兴衰、道路流量和家庭选择由模拟涌现，不预设某一岸必富、某个山口必成王都；水体模板也不自动赋予航运、渔业或无限淡水。

### 4.1 平地草原（`grassland_plain_v1`）· 设计登记（2026-09-12，未排期）
...
### 4.2 半坡林地（`hillside_woodland_v1`）· 设计登记（2026-09-12，未排期）
...
D-B1 的视觉子特征可以单独上线；D-B2/P1 任何一项失败均只禁用该特征或 profile，**不能**在运行中修改道路、移动居民或回抽世界种子。三角洲、海岛、峡湾暂不进入本轮；动态河流、洪水、桥梁及地下系统另立项。海湾、半岛、地表喀斯特可作为后续静态扩展，不强制等待 T4，但须先补齐各自水陆、淡水与可达性契约，不能提前做成“看起来能走”的假设施。
