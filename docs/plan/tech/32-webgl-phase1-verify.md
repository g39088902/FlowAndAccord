...
> ★ **后续进展**：Phase 2（地形层迁移）已于 v1.50.77 落地（双 Canvas 架构，地形由 `sim-canvas-gl` WebGL 层绘制，实体仍走 Canvas 2D 覆盖层），v1.50.80~82 帧率解限。本文为阶段一 PoC 验证的历史记录，其中「待人工合并」「强制回退 Canvas 2D」等临时状态均已失效；当前行为以 [frontend/AGENTS.md](../../../frontend/AGENTS.md) 为准。
