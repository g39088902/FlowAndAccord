# 地图模板规划：模板库、地形要素与未落地蓝图

> **状态**：T0 / T1 / T2 / D-A 已落地；本文保留**模板设计目标、模板总表、未落地组合库、D-B 子特征注入蓝图、R0 河道表示迁移与验收门禁**。
> **已落地的共用技术基座**（地表单元、地貌特征、水体与资源池、装饰、地表查询、生成顺序、路网、取水、快照、前端绘制与配置）已迁入现状文档 [../../current/tech/40-terrain-and-network.md](../../current/tech/40-terrain-and-network.md)；视觉方案见 [./26-terrain-art.md](./26-terrain-art.md)。
...
> **本文定位**：地形领域**唯一权威文档**——以地图模板为主轴，同时承载「模板清单与地理玩法」与「支撑其实现的技术方案」两部分。2026-09-11 由原 `./25-terrain-templates.md`（方向设计）与 `26-plan-terrain-implementation.md`（实施技术方案）合并而成，26 号已删除；合并时按当前代码事实校正了原 26 号中已过时的落地状态（D-A 装饰已落地、`render_terrain.js` 已拆分、配置已达 232 字段）。同日二次整理：改为以地图模板为核心，技术章节按「服务对象」重新归位并整体重排章节号（对照表见 §0.3）。
> **整理日期**：2026-09-11（合并日 + 模板化重组日）；方案优化基线 v1.48.0，现状基线 v1.50.16。
> **入口**：[文档导航](../../README.md) · [长期路线图](../design/10-roadmap.md) · [地形美术与世界景观提升](./26-terrain-art.md)。
> **依据**：[./20-integration-contracts.md](./20-integration-contracts.md)、[./26-terrain-art.md](./26-terrain-art.md)、[../../current/tech/51-seasonal-lighting.md](../../current/tech/51-seasonal-lighting.md)、[../../current/tech/52-water-rendering.md](../../current/tech/52-water-rendering.md)。
> **现状以** `../../current/tech/40-terrain-and-network.md`、`../../current/tech/13-config-system.md`、`../../current/tech/15-snapshot-and-save.md`、`../../current/tech/50-frontend-overview.md` 与 `../../current/11-changelog.md` 为准。
...
本文是**地形种类、生成关系、通行、选址和水资源语义**的规划权威；[景观提升计划](./26-terrain-art.md)负责材质、光照、素材、遮挡、缓存与视觉性能。土地适宜性由地形提供，农田产权、投资与产出仍由[农业规划](./23-farmland-agriculture.md)负责。
...
8. 每个阶段结束后更新 `../../current/tech/40-terrain-and-network.md`、`../../current/tech/13-config-system.md`、`../../current/tech/15-snapshot-and-save.md`、`../../current/tech/50-frontend-overview.md`、`../../current/tech/67-impact-matrix.md` 与 `../../current/11-changelog.md`，并按根 `AGENTS.md` 升版本、同步两份 WASM。
...
本次合并**不修改运行时行为、应用版本或存档格式**。实施时按根 [AGENTS.md](../../../AGENTS.md) 执行升版、WASM 双副本、配置、快照与确定性门禁，并将已交付内容同步至现状文档（`../../current/tech/40-terrain-and-network.md`、`../../current/tech/13-config-system.md`、`../../current/tech/15-snapshot-and-save.md`、`../../current/tech/50-frontend-overview.md`、`../../current/tech/67-impact-matrix.md` 等）及 `docs/current/11-changelog.md`。
