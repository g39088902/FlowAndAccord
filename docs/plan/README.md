...
| 07 | [地形美术与世界景观](tech/07-terrain-art.md) | 部分落地；§1 为任务总台账（TA/TB/TC 编号+难度+依赖），§6 为 Accent 季相、动态受光、LOD 与遮挡边界方案；★ 2026-09-17 渲染架构决策：**全量 WebGL、不再使用 Canvas 2D**（TC-03 改为既定路线，原 2D 遮挡近似策略取消；TA-08 任务已删除视为完成，验收矩阵并入 31 号 §8.5） |
