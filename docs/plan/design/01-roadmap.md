...
| 地形美术与植被季相 | S1/S2/P2 渲染基底与统一深度队列、D-A 地表装饰系统（v1.49.1）、TA-01 装饰三件套（v1.50.23）、TA-02 连续植被季相 `SimTreeTint.sample()`（v1.50.24）、TA-03 局部三维枝干骨架 + 两遍式树冠（v1.50.25~v1.50.27）、TA-04 世界光向动态受光 + 贴地投影（v1.50.32~v1.50.46）、动态季节光照（v1.48.0）（详见 §8.1 专项 21） |
...
地形与画面表现的专项方向见 [地形美术与世界景观提升规划](../tech/07-terrain-art.md)及[地图模板总纲](../tech/06-terrain-templates.md)：渲染基底（S1/S2/P2）、地表装饰系统（D-A）、植被专项波次一（TA-01~03：装饰三件套、连续季相、三维枝干骨架与两遍式树冠，v1.50.23~v1.50.27）与波次二受光（TA-04 世界光向动态受光 + 贴地投影，v1.50.32~v1.50.46）已落地，TA-05 样板闭环已按用户口径收口（2026-09-22，功能已齐、仅缺测试验证与取舍）；地图模板总表已扩至 16 张，其中 T0/T1/T2 已实现（T1 主脊通行力修复见 v1.50.17），2026-09-12 新登记平地草原 `grassland_plain_v1` 与半坡林地 `hillside_woodland_v1`；**2026-09-13 起 16 张模板全部纳入 [地图模板规划 R.3 阶段计划](../tech/06-terrain-templates.md)**（阶段五～八为主体批次、九a～九d 为远期批次）。后续待做：D-B 子特征注入阶段二，随后按 R.3 依次推进台地/盆地/冲积扇/火山湖等 P1 模板与 T4 动态水文。地形方向以地图模板为主轴组织，负责模板清单与地理规则/技术契约；落地明细以 [现状 · 地形与路网](../../current/tech/14-terrain-and-network.md) 与 changelog 为准。
...
内部市场的专项方案见 [内部市场方案：售货员撮合、场地托管与有限订单簿](../tech/03-internal-market.md)：交易由集市场地中介、自主兼职售货员（真人族人）撮合完成，买卖双方实物托管后挂单，按价格/时间/可选关系破平/ID 配对，黄金与资源强守恒；濒危家庭可用木石金换水粮、富余家庭可用余水余粮换木石金，首期全图只设一个独立居民市集。该方案已取代早期 AMM 恒定乘积池口径（见 §8.1 口径变更注），尚未实现或排期。
...
**目标**：以有风险的狩猎连接家庭生存，以可预警、可止损的外部危机形成公共庇护契约。玩法权威见 [07 狩猎、防卫与公共庇护](./07-hunting-defense.md)，执行契约见 [05 技术方案](../tech/05-hunting-defense.md)。

- **生态狩猎**：按实际食物/保暖缺口比较采集、采购与狩猎；依据身体能力与装备准入，有限追逐、生态恢复、真实搬运与保存共同约束收益。
- **流寇与防守**：外部有限入侵与基础逃生、庇护、自主守卫同时交付；真实接触、破门和装运造成损失，撤退也能结束危机。内生犯罪与俘虏治理后置。
- **公共庇护**：家庭保留必要照料能力，公仓在真实预算内优先援助伤亡家庭，再按有效护民贡献奖励；败退、无政体与预算不足都有明确结果。
- **生命力与历史**：可恢复伤势不改变自然寿命；伤亡、物资损失和公共支付来自实际结算，并进入人物、家户与事件观察。
...
| **测试策略** | 临时验证脚本/断言用后即删；长期验证使用 `test-wasm.js`、`test-determinism.js`、`frontend-check.js` 等门禁 |
...
| 21 | 地形美术与世界景观提升 | ◐ S1/S2/P2、D-A 与 TA-01~04 已落地（TA-04 世界光向动态受光 v1.50.32~v1.50.46）；TA-05 已按用户口径收口（2026-09-22）、TA-06 已落地、TA-07 取消视为完成、TA-08 已删除；★ 2026-09-24 决策（UGC 基本功能优先）：TA-09/D-B1-8、TA-10 已删除，TA-13/15/17/18、TB-05/06、TC-01/02 与专属景观/院地细节全部撤销排期，待统一生成器调通后完全重新设计；唯一主线为 UGC-00~12 与既定 TC-03 | [07 地形美术](../tech/07-terrain-art.md) |
| 22 | 地图类型与统一生成 | ◐ T0/T1/T2/D-A 已落地，仅作迁移期兼容与回归；★ 2026-09-24 起取消一切逐模板生成分支、注入器与专用几何算法计划，地图类型仅作为配方与验收目标，后续只排 UGC 统一场编译器、通用水文过程与体素后端（UGC-00~12） | [06 地图类型与统一生成](../tech/06-terrain-templates.md)<br/>可执行设计：[统一地形场编译器设计](../../../TERRAIN_FIELD_COMPILER_DESIGN.md)<br/>已落地基座：[现状 · 40 地形与路网](../../current/tech/14-terrain-and-network.md) |
...
| 测试策略 | 临时验证脚本/断言用后即删；长期验证走 `test-wasm.js` / `test-determinism.js` / `config-check.js` / `frontend-check.js` | 全部 |
...
实际排期由 [规划索引](../README.md) 管理，不把设计修订写成已实现功能。
