...
> **状态**：规划态，类型和目录均为待实施设计，不代表现有 API。
>
> **源码审查基准**：v1.46.6；范围与里程碑见[总体规划](./19-plan-agent-intent-strategy-decoupling.md)。本文是类型、控制协议、分支映射、时序及验收的详细权威定义。
>
> **基础范围**：行为等价的职责迁移。预排批量队列、个性化选策、全局抢占和 UI 三栏不纳入基础交付。
