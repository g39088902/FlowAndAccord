...
> **状态：✔ 已取消视为完成（2026-09-22，经用户确认）。** 取消理由：本任务的原生驱动力是**性能**（高密世界下装饰入队/绘制成本），而当前已无性能问题；未做的仅剩 TA-07-9（Chrome 视觉验收）与 TA-07-10（六场景性能 A/B）两项**证据补采**，随任务取消不再追补。**实现保留、不回退**：TA-07-1~8 + TA-07-11 已于 v1.50.65 落地，判档与剔除全在 CPU 侧（`accent-lod.js` + 模型层 `bounds`/`farClusters`/`segTier`），与渲染后端解耦，在默认 WebGL 路径下照常生效；四条防回退红线见 [frontend/AGENTS.md](frontend/AGENTS.md)。下游 TA-18 的前置改为「TA-07 的**实现**」，[07 号 §1.2 台账](docs/plan/tech/07-terrain-art.md)与 [TODO.md](TODO.md) 已同步。（先例：TA-08 于 2026-09-17 删除视为完成。）
> **（原文保留，作历史记录）◐ 部分实施（2026-09-14 实施，v1.50.65，HEAD 后续提交）。** 规划态源码事实为 2026-09-14 逐行核对结论（应用版本 v1.50.61、HEAD `c884c94`）；**实施后事实以各任务「实施记录」为准**。下文 §0~§7 为原方案与验收口径，**未勾选项（§5 任务序列中的 TA-07-8 后半、TA-07-9、TA-07-10）不再执行**。
...
| **TA-06** | ⏳ 未实施（方案已出 [TA-06-TODO.md](TA-06-TODO.md)，2026-09-14；2026-09-22 更新：实现已落地 v1.50.64/65 并转入验收补做） | 台账 TA-07 依赖 TA-06。TA-06 将交付：`render_bush.js` 拆分、**冠幅/实高/足迹改模型单一来源**（`sk.crownR` / `sk.footprintR`）、`extentOf` 由骨架求值、三乔木轮廓（冠幅 4.8~10.5 差异显著）、`accentModelStyleVersion` 4→5 | **本次依赖关系已失效**——TA-07 于 2026-09-22 取消视为完成，且 TA-06 的模型真值早已落地（`accentModelStyleVersion` 6），故「严格按 TA-06 → TA-07 顺序」不再适用；TA-06 的验收补做独立推进 |
...
**★ 实施状态速览（2026-09-14，v1.50.65；★ 2026-09-22 任务取消视为完成——下表为取消前的实施快照，保留作历史记录）**：

> **取消口径**：TA-07-1~8、TA-07-11 的实现与静态/数值层验证成果**保留**（代码未回退）；其中 TA-07-8 的「四事件端到端 / 内存峰值 / DPR 无关性」与 TA-07-9 / TA-07-10 属**证据补采**，随任务取消不再执行。**不得**据此文勾选任何「已完成验收」结论。
...
- [~] **TA-07-8 生命周期、确定性与内存断言（约 0.5 日，依赖 7）** —— ◐ 原状：纯函数性 / 暖缓存==冷重建 / 滞回收敛已断言；**四事件端到端、内存峰值、DPR 无关性 随任务取消（2026-09-22）不再执行**。
...
- [~] **TA-07-9 Chrome 视觉验收 + 证据包（约 1 日，依赖 8）** —— ⬜ **已随任务取消（2026-09-22）不再执行**（原生性能驱动力消失；视觉回归已由 TA-06 的 WebGL 验收矩阵覆盖同一批图元与档位）。以下为原验收口径，保留备查。
...
- [~] **TA-07-10 性能 A/B（约 1 日，依赖 9）** —— ⬜ **已随任务取消（2026-09-22）不再执行**：取消理由即「当前已无性能问题」，性能证据的采集目的已消失。以下为原 A/B 口径，保留备查（若日后重新出现高密负载性能诉求，可按此口径重启）。
