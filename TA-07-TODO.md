...
| TA-08 | ⏳ 未实施（台账依赖 TA-07） | 无跨实体遮挡拆分 | TA-07 的真值包围体与分级几何是 TA-08 遮挡实测的底座；**不得**在 TA-07 内顺手做跨实体深度拆分或子项化 |
...
| 三档细节分级重定义（远景树形与叶量 / 中景主要枝簇 / 近景细枝叶片），阈值带滞回消除缩放闪烁 | 跨实体遮挡拆分、模型内枝簇深度排序改造（TA-08）；分块缓存与远景减量（TA-18） |
...
11. **不越界实施**：跨实体遮挡拆分（TA-08）、分块缓存与远景减量（TA-18）、离屏精灵预渲染、GPU 迁移（TC-03）一律不在本任务内。
