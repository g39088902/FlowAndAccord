...
> **难度**：高（跨多个前端渲染文件，需 Canvas 2D 遮挡近似优化）  
> **依赖**：TA-05 → TA-06 → TA-07（LOD + 模型缓存 + 剔除）→ TA-08  
> **目标**：通过测量与优化策略消除真实穿插遮挡（如岩壁压路、树冠遮人），在 Canvas 2D 框架内达到可接受的质量阈值，若无法达成则触发 TC-03 GPU 渲染层迁移评估。
...
当前 Canvas 2D 渲染系统已建立统一相机深度队列（`render_depth_queue.js::drawWorldEntities`），实现了基本的“远 → 近”绘制顺序。但存在以下遮挡问题尚未解决：

- **真实穿插遮挡**：高树冠层可能完全遮蔽选中族人信息，导致点击/识别失败；
- **地形误遮挡**：岩壁斜面错误覆盖近处道路或房屋入口，造成通行误解；
- **画家算法局限**：Canvas 无深度缓冲，仅靠单一 `depth` 值排序无法处理复杂重叠几何（如树枝穿插屋瓦、人物穿越树干）。
...
| TC-03 | 条件触发：若 TA-08 性能无法达标，则启动 GPU 渲染层迁移评估 |
