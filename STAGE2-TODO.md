...
> **状态**：STAGE2-1/2 已交付；3～8 未完成。已有 T1 支脊与草原骨架必须保留，当前生成器版本为 5、存档格式为 7、FABS 为 3。
...
| STAGE2-3 | 0–11 步流水线框架与完整几何事务接口 | ⏳ | 2 | 对当前基准等价；第 5/9 步仍无生产注入 |
...
- [ ] 按 06 号 §5.3 解耦 0–11 步。提供几何校验、生存诊断和重试的接口，先用无行为变化的适配层接线，不把尚未实现的 4/6/7 当成本任务的隐含依赖。
- [ ] 几何事务覆盖完整支撑域和坡度差分 halo，备份高程、features、水体、中心线、覆盖意图及元数据；局部失败全部回滚，第 6 步统一定稿坡度/地表。不能只备份高程。
- [ ] 保留当前 T1 主脊/支脊与 T2 分类、RNG 和集合顺序；第 5/9 步生产注入仍为空。现有草原分支通过同构建回归。
